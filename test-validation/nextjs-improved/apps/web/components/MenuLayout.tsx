...
export default function MenuLayout({
  ...
  const handleMenuAction = (action: string, item: MenuItem): void => {
    if (onMenuAction) {
  ...
  const renderMenuItem = (item: MenuItem, level: number = 0): React.ReactElement => {
    if (item.separator) {
  ...
  const renderHorizontalMenu = (): React.ReactElement => (
  ...
  const renderVerticalMenu = (): React.ReactElement => (
    <Box
      width="64"
  ...
  const renderMobileMenu = (): React.ReactElement => {
    const menuContent = (
  ...
  const renderMobileToggle = (): React.ReactElement => (
    <Button
      variant="outline"
      size="sm"
      onClick={handleClick}
  ...
  const getMainContentMargin = (): string => {
