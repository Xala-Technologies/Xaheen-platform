...
export const responsiveUtils = {
   ...
   */
  getResponsiveClasses: (config: Record<string, string>): string => {
   ...
   */
  getContainerMaxWidth: (breakpoint: string): number => {
    const config = responsiveConfig.breakpoints[breakpoint];
   ...
   */
  getSpacingScale: (breakpoint: string): number => {
    const config = responsiveConfig.breakpoints[breakpoint];
   ...
   */
  getResponsiveSpacing: (baseSpacing: number): Record<string, number> => {
...
export const mediaQueries = {
  ...
  up: (breakpoint: string): string => {
    const config = responsiveConfig.breakpoints[breakpoint];
    return config ? "template string" : '';
  },

  down: (breakpoint: string): string => {
    const config = responsiveConfig.breakpoints[breakpoint];
    return config && config.maxWidth ? "template string" : '';
  },
