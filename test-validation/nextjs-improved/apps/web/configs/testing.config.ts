...
const baseJestConfig: Config.InitialOptions = {
  ...
  moduleNameMapping: {
    '^@/(.*)$': '<rootDir>/src/$1',
    '^@xaheen-ai/design-system$': '<rootDir>/node_modules/@xaheen-ai/design-system',
  },
  testMatch: [
 ...
 */
const localizationConfig: LocalizationTestConfig = {
  locales: ['en', 'nb', 'fr', 'ar'],
  fallbackLocale: 'en',
  testKeys: [
    'common.loading',
...
const testUtils = {
   ...
   */
  mockPlatform: (platform: 'mobile' | 'desktop' | 'web') => {
   ...
   */
  mockDeviceCapabilities: (capabilities: Partial<{
    touch: boolean;
    hover: boolean;
    keyboard: boolean;
    camera: boolean;
   ...
   */
  testAccessibility: async (element: HTMLElement): Promise<void> => {
   ...
   */
  testNorwegianLocalization: (component: any, key: string) => {
   ...
   */
  testPerformance: async (url: string): Promise<void> => {
 ...
 */
const platformTestConfig: PlatformTestConfig = {
