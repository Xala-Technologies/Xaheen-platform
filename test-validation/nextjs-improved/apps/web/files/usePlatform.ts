...
export function usePlatformNavigation() {
  ...
  const navigate = useCallback((route: string, params?: any) => {
    if (platformInfo.isReactNative) {
      ...
      if (typeof window !== 'undefined') {
        const url = params ? "template string" : route;
  ...
  const reset = useCallback((routes: string[]) => {
    if (platformInfo.isReactNative) {
      try {
        const { useNavigation } = require('@react-navigation/native');
        const navigation = useNavigation();
...
export function usePlatformPermissions() {
  ...
  const checkPermission = useCallback(async (permission: string): Promise<PlatformPermission> => {
    if (platformInfo.isReactNative) {
  ...
  const requestPermission = useCallback(async (permission: string): Promise<PlatformPermission> => {
    if (platformInfo.isReactNative) {
...
export function usePlatformNotifications() {
  ...
  const showNotification = useCallback(async (notification: Omit<PlatformNotification, 'id' | 'timestamp'>) => {
  ...
  const scheduleNotification = useCallback(async (
    notification: Omit<PlatformNotification, 'id' | 'timestamp'>, 
    delay: number
  ) => {
    if (platformInfo.isReactNative) {
...
export function usePlatformFileSystem() {
  ...
  const readFile = useCallback(async (path: string): Promise<string> => {
    if (platformInfo.isReactNative) {
      try {
        ...
        throw new Error("template string");
      }
  ...
  const listFiles = useCallback(async (directory: string): Promise<string[]> => {
    if (platformInfo.isReactNative) {
      try {
        const RNFS = require('react-native-fs');
  ...
  const pickFile = useCallback(async (): Promise<{ uri: string; name: string; type: string } | null> => {
    if (platformInfo.isReactNative) {
      try {
