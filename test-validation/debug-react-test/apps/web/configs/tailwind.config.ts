{{!-- Template: tailwind.config.ts.hbs --}}
{{!-- Category: frontend --}}
{{!-- Type: config --}}
{{!-- Framework: next --}}
{{!-- Migrated from: /Volumes/Development/Xaheen Enterprise/xaheen/apps/cli/templates/frontend/react/next/tailwind.config.ts.hbs --}}
{{#if typescript}}import type { Config } from 'tailwindcss';{{/if}}

/**
 * Tailwind CSS Configuration for Xala UI System
...
const config{{#if typescript}}: Config{{/if}} = {
  // Content paths
  content: [
    ...
    './node_modules/@xaheen-ai/design-system/**/*.{js,ts,jsx,tsx}',
  ],
  ...
  theme: {
    ...
    spacing: {
      ...
      9: '36px',   // 36px
      10: '40px',  // 40px
      11: '44px',  // 44px - Professional button height (WCAG compliant)
  ...
  plugins: [
    ...
    // Custom plugin for Xala UI System utilities
    function({ addUtilities, theme }) {
      const newUtilities = {
        ...
        '.focus-ring-offset': {
          '&:focus': {
            outline: 'none',
            boxShadow: theme('boxShadow.focus-ring-offset'),
          },
          '&:focus-visible': {
            outline: 'none',
            boxShadow: theme('boxShadow.focus-ring-offset'),
