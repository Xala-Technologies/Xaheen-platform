...
{{!-- Type: component --}}
{{!-- Framework: next --}}
{{!-- Migrated from: /Volumes/Development/Xaheen Enterprise/xaheen/apps/cli/templates/frontend/react/next/src/components/component.tsx.hbs --}}
 ...
 * - [x] ONLY semantic components from @xaheen-ai/design-system
...
const {{componentName}}Schema = z.object({
  ...
  web: z.{{type}}(){{#if required}}.min(1, '{{errorMessage}}'){{/if}},
  {{/each}}
});
...
interface {{componentName}}Props {
  {{#each props}}
  readonly web{{#unless required}}?{{/unless}}: {{type}};
  {{/each}}
...
export default function {{componentName}}({{#if includeTypes}}props: {{componentName}}Props{{/if}}): JSX.Element {
  ...
  const {
    {{#each props}}
    web{{#if defaultValue}} = {{defaultValue}}{{/if}},
    {{/each}}
    className,
    children,
    'data-testid': testId,
  ...
  // Event handlers
  {{#each handlers}}
  const web = React.useCallback(({{#each params}}web: {{type}}{{#unless @last}}, {{/unless}}{{/each}}): {{returnType}} => {
    {{#if includeLocalization}}
    // Add your handler logic here
    console.log('{{../componentName}}.web called');
    {{else}}
    // Add your handler logic here
    console.log('{{../componentName}}.web called');
    {{/if}}
  ...
  React.useEffect(() => {
    ...
    {{#if includeLocalization}}
    console.log('{{../componentName}} effect: Web application for debug-react-test');
    {{else}}
    console.log('{{../componentName}} effect: Web application for debug-react-test');
    {{/if}}
  }, [{{#each dependencies}}{{this}}{{#unless @last}}, {{/unless}}{{/each}}]);
  ...
  return (
    <{{rootComponent}}{{#if includeVariant}} variant="{{variant}}"{{/if}}{{#if includeSpacing}} spacing="{{spacing}}"{{/if}}{{#if testId}} data-testid={testId}{{/if}}>
      ...
      <Stack direction="horizontal" justify="between" align="center"{{#if headerSpacing}} spacing="{{headerSpacing}}"{{/if}}>
        ...
        <Stack direction="horizontal" align="center" spacing="{{actionsSpacing}}">
          ...
          <Button 
            variant="{{variant}}" 
            size="{{size}}"
            {{#if onClick}}onClick={handleClick}}}{{/if}}
      ...
      <{{contentComponent}}{{#if contentVariant}} variant="{{contentVariant}}"{{/if}}{{#if contentSpacing}} spacing="{{contentSpacing}}"{{/if}}>
        {{#if includeGrid}}
        <Grid columns=placeholder spacing="{{gridSpacing}}">
          {{#each gridItems}}
...
<{{componentName}} 
  {{#each exampleProps}}
  web={{#if isString}}"{{value}}"{{else}}{{{value}}}{{/if}}
  {{/each}}
/>
