...
export const ProjectExplorer = ({
							...
							children: [
								{
									...
									analysis: {
										...
										type: "component",
										suggestions: [
											"Consider extracting shared components to ui-system",
										],
									},
								},
								{
	...
	const renderFileTreeNode = useCallback(
		(node: FileTreeNode, depth: number = 0): React.JSX.Element => {
			const isExpanded = expandedPaths.has(node.path);
			const isSelected = selectedPath === node.path;
			const hasChildren = node.children && node.children.length > 0;
			const hasIssues =
				node.analysis?.issues && node.analysis.issues.length > 0;
			const hasSuggestions =
				node.analysis?.suggestions && node.analysis.suggestions.length > 0;
					...
					>
						...
						{node.analysis && (
							<Stack direction="horizontal" gap="xs">
								...
								<Badge
									variant={
										node.analysis.importance === "high"
											? "default"
											: node.analysis.importance === "medium"
												? "secondary"
												: "outline"
									}
									size="xs"
