...
export const AgentStatusMonitor = ({
  ...
  const mockAgents: AgentInfo[] = [
    {
      ...
      currentTasks: activeTasks.filter(t => t.agentId === 'mcp-server').length,
      totalTasks: 1247,
    ...
    },
    {
      ...
      currentTasks: activeTasks.filter(t => t.agentId === 'xaheen-cli').length,
      totalTasks: 892,
    ...
    },
    {
      ...
      status: isConnected ? 'active' : 'offline',
      version: '1.0.0',
      capabilities: ['task-distribution', 'agent-coordination', 'workflow-management'],
      currentTasks: 0,
      totalTasks: 2134,
      averageTaskTime: 0.8,
      lastHeartbeat: new Date(),
      ...
      const interval = setInterval(() => {
        ...
        setAgents(prevAgents => 
          prevAgents.map(agent => ({
            ...agent,
            lastHeartbeat: new Date(),
            cpuUsage: agent.cpuUsage ? agent.cpuUsage + (Math.random() - 0.5) * 5 : undefined,
            memoryUsage: agent.memoryUsage ? agent.memoryUsage + (Math.random() - 0.5) * 10 : undefined,
          }))
        );
      }, 5000);
  ...
  const getStatusBadgeVariant = (status: AgentInfo['status']): 'success' | 'secondary' | 'warning' | 'destructive' | 'outline' => {
    switch (status) {
  ...
  // Render progress bar
  const renderProgressBar = useCallback((progress: number, status: AgentTask['status']): React.JSX.Element => {
      ...
      <Stack direction="vertical" gap="md">
                  ...
                  <Stack direction="vertical" gap="sm">
                    <Stack direction="horizontal" gap="sm" align="center" justify="between">
                      ...
                      <Stack direction="horizontal" gap="xs">
                        ...
                        <Badge variant={getStatusBadgeVariant(agent.status)} size="xs">
                          {agent.status}
                        </Badge>
                      </Stack>
                    </Stack>
        ...
        <Stack direction="vertical" gap="sm">
          ...
          {visibleTasks.length === 0 ? (
            ...
            <Stack direction="vertical" gap="xs" className="max-h-64 overflow-y-auto">
                    ...
                    <Stack direction="horizontal" gap="sm" align="center" justify="between">
                      <Stack direction="vertical" gap="xs" className="flex-1">
                        ...
                        <Stack direction="horizontal" gap="xs">
                          <Badge 
                            variant={
                              task.type === 'generation' ? 'default' :
                              task.type === 'analysis' ? 'secondary' :
                              task.type === 'migration' ? 'warning' : 'success'
                            }
                            size="xs"
                          >
                            {task.type}
