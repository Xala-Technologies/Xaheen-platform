...
export class WebSocketService {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.socket.onclose = (event) => {
          ...
          if (!this.isReconnecting && this.currentReconnectAttempts < this.config.reconnectAttempts) {
  ...
  private attemptReconnect(): void {
    ...
    const delay = this.config.reconnectDelay * Math.pow(2, this.currentReconnectAttempts - 1);
    
    console.log(`Attempting to reconnect (${this.currentReconnectAttempts}/${this.config.reconnectAttempts}) in ${delay}ms`);
