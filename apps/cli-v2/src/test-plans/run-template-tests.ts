...
class TemplateTestRunner {
  ...
  private async runTestSuite(suite: { name: string; file: string; description: string }): Promise<void> {
    ...
    try {
      ...
      // Parse test results
      const testResult = this.parseTestOutput(stdout, stderr, suite.name, duration);
      this.report.results.push(testResult);

      if (testResult.status === 'passed') {
        this.report.passed++;
        consola.success(`✅ ${suite.name} - ${testResult.status} (${Math.round(duration)}ms)`);
      } else {
        this.report.failed++;
        consola.error(`❌ ${suite.name} - ${testResult.status} (${Math.round(duration)}ms)`);
  ...
  private async calculateTemplateCoverage(): Promise<void> {
    ...
    try {
      ...
      consola.info(`📊 Template Coverage: ${testedTemplates.size}/${totalTemplates} (${this.report.coverage.percentage}%)`);
  ...
  private generateReport(): void {
    ...
    for (const result of this.report.results) {
      ...
      consola.info(`${status} ${result.name} (${Math.round(result.duration)}ms)`);
  ...
  private async saveReportToFile(): Promise<void> {
    try {
      ...
      const reportFile = path.join(reportPath, `template-test-report-${timestamp}.json`);
  ...
  async runHealthCheck(): Promise<void> {
    ...
    const checks = [
      { name: 'Template Directory Exists', check: () => this.checkTemplateDirectory() },
      { name: 'Template Loader Available', check: () => this.checkTemplateLoader() },
      { name: 'Service Injector Available', check: () => this.checkServiceInjector() },
      { name: 'Template Registry Available', check: () => this.checkTemplateRegistry() },
  ...
  private async checkTemplateLoader(): Promise<boolean> {
    try {
      const { TemplateLoader } = await import('../services/templates/template-loader.js');
  ...
  private async checkTemplateRegistry(): Promise<boolean> {
    try {
      const { getServiceTemplates } = await import('../services/templates/template-registry.js');
