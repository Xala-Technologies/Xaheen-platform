...
import type { ProjectContext, ServiceConfiguration, ServiceTemplate } from '../types/index.js';
...
describe('Template Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    testProjectPath = path.join(process.cwd(), 'test-output', `integration-${Date.now()}`);
  ...
  describe('Next.js + Xala Component Integration', () => {
    ...
    it('should render complete Next.js project with Xala components', async () => {
      ...
      for (const templatePath of templates) {
        const result = await templateLoader.renderTemplate(templatePath, nextJsXalaContext);
      ...
      const packageJson = JSON.parse(results.get('frontend/next/configs/package.json.hbs')!);
      expect(packageJson.name).toBe('next-xala-app');
      expect(packageJson.dependencies).toHaveProperty('next');
      expect(packageJson.dependencies).toHaveProperty('react');

      // Check Next.js config includes i18n for Norwegian support
      const nextConfig = results.get('frontend/next/configs/next.config.ts.hbs')!;
      ...
      const xalaNorwegian = results.get('components/files/xala-display-norwegian.hbs')!;
  ...
  describe('Full-Stack Application Scenarios', () => {
    it('should handle Norwegian E-commerce Platform scenario', async () => {
      ...
      for (const templatePath of fullStackTemplates) {
        const result = await templateLoader.renderTemplate(templatePath, ecommerceContext);
    ...
    it('should handle Enterprise SaaS Application scenario', async () => {
      ...
      for (const templatePath of enterpriseTemplates) {
        const result = await templateLoader.renderTemplate(templatePath, enterpriseContext);
  ...
  describe('Template Combination Tests', () => {
    it('should render compatible template combinations', async () => {
      ...
      const combinations = [
        // Next.js + tRPC
        ['frontend/next/configs/package.json.hbs', 'api/trpc/files/trpc.ts.hbs'],
        
        // Next.js + Prisma
        ['frontend/next/configs/package.json.hbs', 'database/prisma/files/schema.prisma.hbs'],
        
        // Next.js + Auth
        ['frontend/next/components/layout.tsx.hbs', 'auth/components/sign-in.tsx.hbs'],
        
        // Norwegian components
        ['components/files/xala-display-norwegian.hbs', 'components/files/xala-form-norwegian.hbs']
      ];
      ...
      for (const [template1, template2] of combinations) {
        ...
        if (template1.includes('package.json') && template2.includes('prisma')) {
