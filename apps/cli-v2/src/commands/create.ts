...
export const createCommand = new Command('create')
  ...
  .option('-p, --preset <preset>', 'Use a preset bundle (e.g., saas-starter, saas-enterprise)')
  .option('-f, --framework <framework>', 'Frontend framework (next, nuxt, remix, etc.)')
  .option('-b, --backend <backend>', 'Backend framework (hono, express, fastify, etc.)')
  .option('-d, --database <database>', 'Database (postgresql, mysql, mongodb, etc.)')
  ...
  .action(async (name, options) => {
    try {
      ...
      const injector = new ServiceInjector(registry);
      ...
      resolution.resolvedServices.forEach(service => {
        consola.info(`  - ${chalk.green(service.serviceType)}: ${service.provider} v${service.version}`);
      });
...
async function getProjectConfig(name?: string, options?: any): Promise<ProjectConfig> {
  ...
  if (isCancel(projectName)) return projectName;
  ...
  if (isCancel(framework)) return framework;
  ...
  if (isCancel(backend)) return backend;

  // Get database if backend selected
  let database = 'none';
  if (backend !== 'none') {
    database = options?.database || await select({
...
async function selectBundle(resolver: BundleResolver): Promise<ServiceBundle | null> {
  const bundleType = await select({
    message: 'Select project type',
    options: [
      { value: 'saas-starter', label: 'SaaS Starter - Essential SaaS features' },
      { value: 'saas-professional', label: 'SaaS Professional - Full-featured SaaS' },
      { value: 'saas-enterprise', label: 'SaaS Enterprise - Enterprise-grade SaaS' },
