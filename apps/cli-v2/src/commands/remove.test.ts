...
describe('Remove Command', () => {
  ...
  it('should be properly configured', () => {
    ...
    expect(removeCommand.description()).toContain('Remove services from existing project');
  });
  ...
  describe('command options', () => {
    ...
    it('should support cleanup operations', () => {
      const cleanupOption = removeCommand.options.find(opt => opt.long === '--cleanup');
      expect(cleanupOption).toBeDefined();
      expect(cleanupOption?.description).toContain('orphaned');
    });

    it('should support dry-run mode', () => {
      const dryRunOption = removeCommand.options.find(opt => opt.long === '--dry-run');
      expect(dryRunOption).toBeDefined();
      expect(dryRunOption?.description).toContain('Preview changes');
    });

    it('should support keeping configuration', () => {
      const keepConfigOption = removeCommand.options.find(opt => opt.long === '--keep-config');
