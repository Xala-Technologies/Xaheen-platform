...
export class ServiceRemover {
  ...
  async analyzeDependencies(
    ...
    await this.checkConfigurationDependencies(servicesToRemove, projectPath, warnings);
  ...
  async createRemovalPlan(
    ...
    for (const serviceType of servicesToRemove) {
      await this.analyzeServiceForRemoval(serviceType, projectPath, projectInfo, plan, options);
    }
    ...
    try {
      ...
      for (const modification of plan.filesToModify) {
        try {
          ...
          result.errors.push(`Failed to modify file ${modification.path}: ${error}`);
        }
      }
      ...
      if (plan.dependenciesToRemove.length > 0) {
        try {
          ...
          consola.debug(`Removed ${plan.dependenciesToRemove.length} dependencies`);
        } catch (error) {
      ...
      for (const configUpdate of plan.configUpdates) {
        try {
          ...
          result.errors.push(`Failed to update config ${configUpdate.file}: ${error}`);
        }
      }
  ...
  private async findDependentServices(
    ...
    for (const service of remainingServices) {
      ...
      for (const template of templates) {
        const hasDependency = template.dependencies.some(dep => 
          dep.serviceType === serviceToRemove
        );
        ...
        try {
          ...
          for (const service of servicesToRemove) {
            if (content.includes(service)) {
              warnings.push(`Configuration file ${configFile} may reference ${service} service`);
            }
          }
        } catch (error) {
  ...
  private async checkCodeDependencies(
    ...
    try {
      ...
      if (result.stdout) {
        ...
        if (files.length > 0) {
          warnings.push(`Found potential references in ${files.length} file(s) - manual review recommended`);
        }
      }
    } catch (error) {
    ...
    for (const injectionPoint of template.injectionPoints) {
      ...
      switch (injectionPoint.type) {
        ...
        case 'ast-modify':
          ...
          const existingModification = plan.filesToModify.find(m => m.path === targetPath);
          if (existingModification) {
            existingModification.changes.push(`Remove ${serviceType} modifications`);
          } else {
          ...
          if (targetPath === 'package.json') {
            ...
            try {
              ...
              if (templateData.dependencies) {
                plan.dependenciesToRemove.push(...Object.keys(templateData.dependencies));
              }
              if (templateData.devDependencies) {
                plan.dependenciesToRemove.push(...Object.keys(templateData.devDependencies));
              }
            } catch (error) {
              consola.debug(`Could not parse template for ${serviceType}:`, error);
            }
          } else {
            // Other JSON files
  ...
  private async addCleanupOperations(
    ...
    const unusedDeps = await this.findUnusedDependencies(projectPath, plan.dependenciesToRemove);
  ...
  private generatePostRemovalSteps(plan: RemovalPlan): string[] {
    ...
    steps.push('Test the application to ensure removed services are not causing issues');
