...
export class ProjectScaffolder {
  ...
  async scaffoldProject(params: {
    ...
    const { name, path: projectPath, config, bundle, resolution, options = {} } = params;
    ...
    try {
      ...
      if (!options.skipInstall && !options.dryRun) {
        await this.installDependencies(projectPath, options.packageManager || 'bun');
      }
      ...
      return {
        projectName: name,
        projectPath,
        services: resolution.resolvedServices.map(s => `${s.serviceType}:${s.provider}`),
        postInstallSteps,
        success: true
      };
...
`
  ...
  private async initializeGit(projectPath: string): Promise<void> {
    try {
      ...
      await execa('git', ['commit', '-m', 'Initial commit from Xaheen CLI'], { cwd: projectPath });
