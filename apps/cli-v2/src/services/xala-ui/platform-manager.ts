...
export abstract class Platform {
	...
	abstract generateComponent(spec: XalaComponentSpec, context: TemplateContext): Promise<GenerationResult>;
	abstract validateComponent(filePath: string): Promise<{ valid: boolean; issues: string[] }>;
	...
	protected registerHelpers(): void {
		...
		this.handlebars.registerHelper('localize', function(key: string, context: any) {
...
export class ReactPlatform extends Platform {
	constructor() {
		super({
			...
			dependencies: {
				ui: ['@xala-technologies/ui-system', 'clsx', 'class-variance-authority'],
	...
	async generateComponent(spec: XalaComponentSpec, context: TemplateContext): Promise<GenerationResult> {
		...
		try {
			...
			const componentDir = path.join(context.project.name, this.config.outputDir, componentName);

			// Generate main component file
			const componentPath = path.join(componentDir, `${componentName}${this.config.fileExtensions.component}`);
			...
			if (!context.features.cssInJs) {
				const stylePath = path.join(componentDir, `${componentName}${this.config.fileExtensions.style}`);
			...
			if (spec.withTests) {
				const testPath = path.join(componentDir, `${componentName}${this.config.fileExtensions.test}`);
			...
			if (spec.withStories) {
				const storyPath = path.join(componentDir, `${componentName}${this.config.fileExtensions.story}`);
	...
	async validateComponent(filePath: string): Promise<{ valid: boolean; issues: string[] }> {
		const issues: string[] = [];
		...
		try {
			...
			const rawHtmlPattern = /<(div|span|p|h[1-6]|button|input|form|ul|ol|li|table|tr|td|th|thead|tbody|tfoot)\s/g;
			const rawHtmlMatches = content.match(rawHtmlPattern);
			
			if (rawHtmlMatches) {
				issues.push(`Found raw HTML elements: ${rawHtmlMatches.join(', ')}. Use semantic components instead.`);
			}
			...
			if (textMatches) {
				...
				if (filteredMatches.length > 0) {
					issues.push(`Found hardcoded text that should be localized: ${filteredMatches.slice(0, 3).join(', ')}`);
				}
			}
			...
			if (inlineStylePattern.test(content)) {
				issues.push('Found inline styles. Use design tokens and semantic components instead.');
			}

			// Check for proper imports
			if (!content.includes('@xala-technologies/ui-system')) {
				issues.push('Missing import from @xala-technologies/ui-system. Use semantic components.');
			}
...
`;
		await fs.writeFile(path.join(projectPath, 'src/styles/modules.css'), cssConfig);
	}
}
	...
	async validateComponent(filePath: string): Promise<{ valid: boolean; issues: string[] }> {
		...
		try {
			...
			if (!content.includes('$t(') && !content.includes('useI18n')) {
				issues.push('Component should use localization with $t() function or useI18n.');
			}
...
export class AngularPlatform extends Platform {
	...
	async generateComponent(spec: XalaComponentSpec, context: TemplateContext): Promise<GenerationResult> {
		...
		try {
			...
			result.success = false;
			result.errors.push(`Angular component generation failed: ${error.message}`);
		}
	...
	async validateComponent(filePath: string): Promise<{ valid: boolean; issues: string[] }> {
		...
		try {
			...
			if (templateMatch) {
				...
				if (rawHtmlMatches) {
					issues.push(`Found raw HTML elements in template: ${rawHtmlMatches.join(', ')}`);
				}
			}
