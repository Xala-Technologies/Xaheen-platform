...
export class ProjectValidator {
  ...
  private async validateServices(projectPath: string, projectInfo: ProjectInfo): Promise<void> {
  ...
  private async validateDependencies(projectPath: string, projectInfo: ProjectInfo): Promise<void> {
    ...
    for (const dep of Object.keys(deps)) {
      if (!usedDeps.has(dep) && !this.isFrameworkDependency(dep, projectInfo.framework)) {
        this.addIssue({
          id: `deps-unused-${dep}`,
          category: 'dependencies',
          severity: 'warning',
    ...
    try {
      const result = await execa(projectInfo.packageManager || 'npm', ['outdated', '--json'], {
        cwd: projectPath,
        stdio: 'pipe',
        reject: false
      });
  ...
  private async validateEnvironment(projectPath: string, projectInfo: ProjectInfo): Promise<void> {
  ...
  private async validateLinting(projectPath: string, projectInfo: ProjectInfo): Promise<void> {
    ...
    try {
      ...
      } else if (projectInfo.dependencies?.eslint || projectInfo.devDependencies?.eslint) {
  ...
  private async validateTypes(projectPath: string, projectInfo: ProjectInfo): Promise<void> {
  ...
  private async calculateMetrics(projectPath: string, projectInfo: ProjectInfo) {
    const metrics: any = {};

    // Dependencies metrics
    if (projectInfo.dependencies || projectInfo.devDependencies) {
      const total = Object.keys({ ...projectInfo.dependencies, ...projectInfo.devDependencies }).length;
  ...
  private detectServiceTypeFromFile(file: string): string | null {
    ...
    const serviceTypes = ['auth', 'database', 'prisma', 'stripe', 'email', 'analytics', 'monitoring', 'cache'];
