...
import type { ServiceConfiguration, ServiceTemplate, ProjectContext } from '../../types/index.js';
...
describe('ServiceInjector', () => {
  ...
  describe('injectService', () => {
    ...
    it('should inject service with external template', async () => {
      ...
      await fs.writeFile(templateFullPath, 'export const {{name}} = "{{value}}";');
    ...
    it('should handle file append injection', async () => {
      ...
      expect(content).toBe('console.log("existing");\n\nconsole.log("appended");');
    });
    ...
    it('should skip existing files unless force option is used', async () => {
      ...
      expect(result1.warnings).toContain('File already exists: src/existing.ts');
  ...
  describe('injectServices', () => {
    it('should inject multiple services', async () => {
      ...
      const file1 = await fs.readFile(path.join(testProjectPath, 'src/lib/test1.ts'), 'utf-8');
      const file2 = await fs.readFile(path.join(testProjectPath, 'src/lib/test2.ts'), 'utf-8');
...
{{variable}}`;
      ...
      const inlineContent = await fs.readFile(path.join(testProjectPath, 'src/inline.ts'), 'utf-8');
      const externalContent = await fs.readFile(path.join(testProjectPath, 'src/external.ts'), 'utf-8');
