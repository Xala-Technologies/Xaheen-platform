...
export class ServiceInjector implements IServiceInjector {
  ...
  async injectService(
    ...
    consola.debug(`Injecting service: ${service.serviceType}:${service.provider}`);
        ...
        try {
          ...
          switch (injectionPoint.type) {
            case 'file-create':
              ...
              const content = await this.renderTemplate(injectionPoint.template, {
                ...projectContext,
                service,
            ...
            case 'json-merge':
              ...
              const mergeContent = await this.renderTemplate(injectionPoint.template, {
                ...projectContext,
                service,
                config: service.configuration
              });
            ...
            case 'file-append':
              ...
              const appendContent = await this.renderTemplate(injectionPoint.template, {
                ...projectContext,
                service,
                config: service.configuration
              });
            ...
            default:
              warnings.push(`Unsupported injection type: ${injectionPoint.type}`);
          }
        } catch (error) {
          errors.push(`Failed to process injection point ${injectionPoint.target}: ${error}`);
        }
      }
