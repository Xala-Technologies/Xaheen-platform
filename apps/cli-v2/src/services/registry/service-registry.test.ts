...
describe('ServiceRegistry', () => {
  ...
  describe('getTemplate', () => {
    ...
    it('should return null for non-existent template', async () => {
      const template = await serviceRegistry.getTemplate('auth', 'non-existent');
      expect(template).toBeNull();
    });

    it('should return null for non-existent service type', async () => {
      const template = await serviceRegistry.getTemplate('non-existent' as any, 'provider');
      expect(template).toBeNull();
    });
  });
  ...
  describe('registerTemplate', () => {
    ...
    it('should register valid template', async () => {
      ...
      await expect(serviceRegistry.registerTemplate(template)).resolves.not.toThrow();

      const retrieved = await serviceRegistry.getTemplate('cache', 'test-provider');
    ...
    it('should reject invalid template', async () => {
      ...
      await expect(serviceRegistry.registerTemplate(invalidTemplate))
  ...
  describe('built-in templates validation', () => {
    ...
    it('should have valid database template structure', async () => {
      const template = await serviceRegistry.getTemplate('database', 'postgresql');
      expect(template).toBeDefined();
    ...
    it('should have valid analytics template structure', async () => {
      const template = await serviceRegistry.getTemplate('analytics', 'posthog');
      expect(template).toBeDefined();
      expect(template?.injectionPoints).toHaveLength(2);
      expect(template?.envVariables).toHaveLength(2);
      expect(template?.frameworks).toContain('next');
    ...
    it('should have valid monitoring template structure', async () => {
      const template = await serviceRegistry.getTemplate('monitoring', 'sentry');
      expect(template).toBeDefined();
  ...
  describe('template injection points', () => {
    ...
    it('should have correct injection point types', async () => {
      const authTemplate = await serviceRegistry.getTemplate('auth', 'better-auth');
    ...
    it('should have valid target paths', async () => {
      const dbTemplate = await serviceRegistry.getTemplate('database', 'postgresql');
