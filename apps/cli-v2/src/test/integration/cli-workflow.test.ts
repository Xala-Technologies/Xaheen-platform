...
describe('CLI Workflow Integration', () => {
  ...
  describe('Complete Project Lifecycle', () => {
    it('should handle full project lifecycle: create -> add -> validate -> remove', async () => {
      ...
      const packageJson = await fs.readJson(path.join(mockProject.path, 'package.json'));
      packageJson.dependencies['better-auth'] = '^1.0.0';
      await fs.writeJson(path.join(mockProject.path, 'package.json'), packageJson, { spaces: 2 });
