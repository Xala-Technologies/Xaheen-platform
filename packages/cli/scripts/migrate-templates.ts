...
class TemplateMigrator {
	...
	async migrate() {
		...
		consola.success(
			`Migration completed! ${this.mappings.length} templates processed.`,
		);
	}
	...
	private createMapping(
		...
		const { targetCategory, type, framework, provider } = this.analyzeFilePath(
			category,
			filePath,
		);
	...
	private async copyTemplates() {
		...
		for (const mapping of this.mappings) {
			try {
				...
				const transformedContent = await this.transformTemplate(
					content,
					mapping,
				);
				...
				if (this.config.verbose) {
					consola.success(
						`Copied: ${mapping.sourcePath} -> ${mapping.targetPath}`,
					);
				}
		...
		consola.info(
			`Copied ${copied} templates, skipped ${skipped} existing files`,
		);
	}

	private async transformTemplate(
		content: string,
	...
	private async generateRegistryEntries() {
		...
		const registryPath = path.join(
			this.config.targetDir,
			"template-registry-migrations.json",
		);
