...
export default class PackageDomain {
  ...
  private initializePackageTemplates(): Map<string, PackageTemplate> {
    ...
    templates.set('ui-components', {
      ...
      dependencies: ['react', '@xala-technologies/ui-system', 'class-variance-authority']
    });
  ...
  async create(command: CLICommand): Promise<void> {
    ...
    logger.info(`📦 Creating ${packageTemplate.name}: ${chalk.green(packageName)}`);
    ...
    try {
      ...
      await this.generatePackageStructure(packagePath, packageName, packageTemplate);
      
      logger.success(`✅ Package created successfully at: ${chalk.cyan(packagePath)}`);
      logger.info(`   Type: ${packageTemplate.type}`);
      logger.info(`   Exports: ${packageTemplate.exports.join(', ')}`);
      
      if (packageTemplate.dependencies && packageTemplate.dependencies.length > 0) {
      ...
      if (isMonorepo) {
        ...
        logger.info(`   ${chalk.cyan(`"workspaces": ["packages/${packageName}"]`)}`);
      }
    } catch (error) {
        ...
        if (stat.isDirectory()) {
          try {
            ...
            if (packageJson.main || packageJson.exports) {
              logger.info(`    Exports: ${chalk.yellow(packageJson.main || 'ESM exports')}`);
            }
            logger.info('');
          } catch (error) {
            logger.info(`  • ${chalk.green(pkg)} ${chalk.gray('(no package.json)')}`);
          }
        }
      }
    } catch (error) {
  ...
  async add(command: CLICommand): Promise<void> {
    ...
    if (!isMonorepo) {
      logger.warn('Not in a monorepo structure. Use "xaheen package create" instead.');
      return;
    }
  ...
  private async generatePackageStructure(
    ...
    const packageJson = {
      ...
      peerDependencies: template.type === 'ui' || template.type === 'hooks' ? {
        'react': '>=18.0.0',
        'react-dom': '>=18.0.0'
      } : undefined
    };
  ...
  private generateIndexFile(packageName: string, template: PackageTemplate): string {
    const exports = template.exports.map(exp => `export * from './${exp}';`).join('\n');
...
export default defineConfig({
  ...
  private generateReadme(packageName: string, template: PackageTemplate): string {
...
Part of the monorepo managed by [Xaheen Unified CLI](https://github.com/xala-technologies/xaheen)`;
  ...
  private async createTemplateFiles(srcPath: string, template: PackageTemplate): Promise<void> {
...
export type Status = 'idle' | 'loading' | 'success' | 'error';`;
  ...
  private getDependenciesForTemplate(template: PackageTemplate): Record<string, string> {
