...
export default class ThemeDomain {
  ...
  public async create(command: CLICommand): Promise<void> {
    ...
    if (!themeName) {
      throw new CLIError('Theme name is required', 'MISSING_THEME_NAME', 'theme', 'create');
    }
    ...
    try {
      ...
      throw new CLIError(`Failed to create theme: ${error}`, 'THEME_CREATE_FAILED', 'theme', 'create');
    }
  }
  ...
  public async list(command: CLICommand): Promise<void> {
    ...
    try {
      ...
      const themes = ['default', 'healthcare', 'finance', 'e-commerce', 'dashboard'];
      ...
      throw new CLIError(`Failed to list themes: ${error}`, 'THEME_LIST_FAILED', 'theme', 'list');
    }
  }
}