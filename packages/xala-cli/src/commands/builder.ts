...
import { CommandMetadata, registerCommand } from "./index.js";
...
async function builderCommand(
	...
	try {
		console.log(
			chalk.cyan.bold("🚀 Welcome to Xaheen Interactive Tech Builder!\n"),
		);
		...
		const generationResult = await projectGenerator.generateProject(
			stack,
			targetPath,
			{
 ...
 */
async function generatePresetCommand(
	presetId: string,
	options: PresetGeneratorOptions,
): Promise<void> {
	try {
		console.log(
			chalk.cyan.bold(`📦 Generating project from preset: ${presetId}\n`),
		);
 ...
 */
async function generateBundleCommand(
	bundleId: string,
	options: BundleGeneratorOptions,
): Promise<void> {
	try {
		console.log(
			chalk.cyan.bold(`📦 Generating project from bundle: ${bundleId}\n`),
		);
...
async function listPresetsCommand(): Promise<void> {
	try {
		...
		for (const [projectType, presets] of Object.entries(presetsByType)) {
			const projectTypeInfo = config.projectTypes.find(
			...
			for (const preset of presets) {
				...
				console.log(
					`    ${chalk.cyan(`Command: xala builder preset ${preset.id}`)}`,
				);
			}
		}
...
async function listBundlesCommand(): Promise<void> {
	try {
		...
		for (const [category, categoryBundles] of Object.entries(
			bundlesByCategory,
		)) {
			...
			for (const bundle of categoryBundles) {
				...
				console.log(
					`    ${chalk.cyan(`Command: xala builder bundle ${bundle.id}`)}`,
				);
			}
		}
...
async function validateConfigCommand(): Promise<void> {
	try {
		...
		if (errors.length === 0) {
			...
			console.log(
				`   Tech Options: ${Object.keys(config.techOptions).length} categories`,
			);
		} else {
...
function showSuccessSummary(
	...
	console.log(
		`${chalk.blue("Files Generated:")} ${result.generatedFiles.length}`,
	);
...
function showNextSteps(stack: StackConfiguration, targetPath: string): void {
	...
	console.log(
		`${stack.install ? "2" : "3"}. ${chalk.white(`${stack.packageManager} run dev`)}`,
	);

	console.log(chalk.gray("\n💡 Additional commands:"));
	console.log(
		`   ${chalk.gray("•")} ${chalk.white(`${stack.packageManager} run build`)} - Build for production`,
	);
	console.log(
		`   ${chalk.gray("•")} ${chalk.white(`${stack.packageManager} run lint`)} - Run linting`,
	);

	if (stack.orm === "prisma") {
		console.log(
			`   ${chalk.gray("•")} ${chalk.white(`${stack.packageManager} run db:push`)} - Push database schema`,
		);
	} else if (stack.orm === "drizzle") {
		console.log(
			`   ${chalk.gray("•")} ${chalk.white(`${stack.packageManager} run db:generate`)} - Generate migrations`,
		);
	}
}
...
async function getBundleConfiguration(bundleId: string) {
	const bundles = {
		"saas-starter": {
			...
			description:
				"Complete SaaS application with authentication, billing, and admin dashboard",
			category: "starter",
			features: [
				"Multi-tenancy",
				"Stripe Integration",
...
export function register(
	registerCommandFn: (metadata: CommandMetadata) => void,
): void {
	// Main interactive builder command
	registerCommandFn({
		name: "builder",
