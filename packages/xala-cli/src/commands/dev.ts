	...
	action: async (options: DevOptions = {}) => {
		try {
			...
			try {
				...
				console.log(chalk.bold("\n⚡ Features:"));
				console.log(
					`  ${serverConfig.hotReload ? "✅" : "❌"} Hot Module Replacement`,
				);
				...
				console.log(chalk.bold("\n🛠️  Development Tools:"));
				console.log(
					`  Component Preview: ${chalk.cyan(server.localUrl + "/preview")}`,
				);
				console.log(
					`  Theme Switcher:    ${chalk.cyan(server.localUrl + "/themes")}`,
				);
				console.log(
					`  Design Tokens:     ${chalk.cyan(server.localUrl + "/tokens")}`,
				);
				console.log(
					`  Documentation:     ${chalk.cyan(server.localUrl + "/docs")}`,
				);
