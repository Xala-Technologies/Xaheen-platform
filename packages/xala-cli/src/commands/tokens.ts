...
async function handleGenerateTokens(options: TokenOptions): Promise<void> {
	...
	try {
		...
		console.log(
			`  Import tokens: ${chalk.cyan('import { tokens } from "./src/tokens"')}`,
		);
		console.log(
			`  Use in components: ${chalk.cyan("tokens.colors.primary.500")}`,
		);
	} catch (error) {
...
async function handleValidateTokens(options: TokenOptions): Promise<void> {
	...
	try {
		...
		if (validation.isValid) {
			...
			console.log(
				chalk.green(
					`✅ ${validation.tokenCount} tokens validated successfully`,
				),
			);
		} else {
...
async function handleSyncTokens(options: TokenOptions): Promise<void> {
	...
	if (!platform) {
		throw new ValidationError(
			"Platform is required. Example: xala tokens sync react",
		);
	}
	...
	try {
		...
		console.log(
			`  Updated token files in: ${chalk.cyan(options.output || "./src/tokens")}`,
		);
...
async function handleTransformTokens(options: TokenOptions): Promise<void> {
	...
	if (!targetFormat) {
		throw new ValidationError(
			"Target format is required. Example: xala tokens transform css",
		);
	}
	...
	try {
		...
		results.forEach((result) => {
			console.log(
				`  ${chalk.cyan(result.sourceFormat)} → ${chalk.cyan(result.targetFormat)}`,
			);
...
async function showTokensHelp(): Promise<void> {
	console.log(chalk.bold("\n🎨 Token Commands:"));
	console.log(
		`  ${chalk.cyan("generate")}        Generate tokens for target platforms`,
	);
	console.log(`  ${chalk.cyan("validate")}        Validate token definitions`);
	console.log(
		`  ${chalk.cyan("sync")} <platform>  Sync tokens for specific platform`,
	);
	console.log(
		`  ${chalk.cyan("transform")} <format> Transform tokens to different format`,
	);
	console.log(
		`  ${chalk.cyan("watch")}           Watch and auto-regenerate tokens`,
	);
