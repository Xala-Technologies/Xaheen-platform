	...
	action: async (options: AnalyzeOptions = {}) => {
		try {
			...
			try {
				...
				if (
					analysisResults.reportPaths &&
					analysisResults.reportPaths.length > 0
				) {
					console.log(chalk.bold("\n📁 Reports Generated:"));
				...
				if (
					...
					analysisResults.recommendations.forEach((rec) => {
						console.log(
							`  ${getRecommendationIcon(rec.priority)} ${rec.message}`,
						);
					});
				} else {
...
function displayPerformanceResults(performance: any): void {
	...
	console.log(
		`  Bundle Size: ${getScoreColor(performance.bundleScore)}${performance.bundleSize}${chalk.reset()}`,
	);
	console.log(
		`  Load Time: ${getScoreColor(performance.loadScore)}${performance.loadTime}${chalk.reset()}`,
	);
	console.log(
		`  First Paint: ${getScoreColor(performance.paintScore)}${performance.firstPaint}${chalk.reset()}`,
	);
	console.log(
		`  Interactive: ${getScoreColor(performance.interactiveScore)}${performance.timeToInteractive}${chalk.reset()}`,
	);
	console.log(
		`  Overall Score: ${getScoreColor(performance.overallScore)}${performance.overallScore}/100${chalk.reset()}`,
	);
...
function displayAccessibilityResults(accessibility: any): void {
	...
	console.log(
		`  WCAG Compliance: ${getComplianceColor(accessibility.wcagLevel)}${accessibility.wcagLevel}${chalk.reset()}`,
	);
	console.log(
		`  Color Contrast: ${getScoreColor(accessibility.contrastScore)}${accessibility.contrastScore}/100${chalk.reset()}`,
	);
	console.log(
		`  Keyboard Navigation: ${getScoreColor(accessibility.keyboardScore)}${accessibility.keyboardScore}/100${chalk.reset()}`,
	);
	console.log(
		`  Screen Reader: ${getScoreColor(accessibility.screenReaderScore)}${accessibility.screenReaderScore}/100${chalk.reset()}`,
	);
	console.log(
		`  Overall Score: ${getScoreColor(accessibility.overallScore)}${accessibility.overallScore}/100${chalk.reset()}`,
	);
...
function displayComplianceResults(compliance: any): void {
	...
	console.log(
		`  Component Usage: ${getScoreColor(compliance.componentScore)}${compliance.componentScore}/100${chalk.reset()}`,
	);
	console.log(
		`  Design Tokens: ${getScoreColor(compliance.tokenScore)}${compliance.tokenScore}/100${chalk.reset()}`,
	);
	console.log(
		`  Localization: ${getScoreColor(compliance.i18nScore)}${compliance.i18nScore}/100${chalk.reset()}`,
	);
	console.log(
		`  TypeScript: ${getScoreColor(compliance.typeScore)}${compliance.typeScore}/100${chalk.reset()}`,
	);
	console.log(
		`  Code Quality: ${getScoreColor(compliance.qualityScore)}${compliance.qualityScore}/100${chalk.reset()}`,
	);
	console.log(
		`  Overall Score: ${getScoreColor(compliance.overallScore)}${compliance.overallScore}/100${chalk.reset()}`,
	);
