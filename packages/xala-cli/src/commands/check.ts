...
export const check = new Command("check")
	.description(
		"Quick validation of components and pages against UI system standards",
	)
...
function checkV5Architecture(content: string): CheckResult[] {
	...
	if (content.includes("className") && !content.includes("cva")) {
		checks.push({
			...
			suggestion:
				"Add class-variance-authority for consistent styling variants",
		});
...
function checkStyling(content: string): CheckResult[] {
	...
	if (foundSmallSizes.length > 0) {
		checks.push({
			...
			suggestion:
				"Use professional sizing (min h-12 for buttons, h-14 for inputs)",
		});
	} else {
		checks.push({
			name: "Professional Sizing",
...
function calculateScore(checks: CheckResult[]): number {
	...
	for (const check of checks) {
		const weight =
			check.severity === "error" ? 3 : check.severity === "warning" ? 2 : 1;
		maxPoints += weight;
		if (check.passed) {
			totalPoints += weight;
		}
...
function displayCheckResults(result: ComponentCheck, options: any) {
	...
	console.log(
		`${statusIcon} ${statusColor.bold(result.status.toUpperCase())} - Score: ${statusColor(result.score + "%")}`,
	);
	...
	if (result.status === "excellent") {
		...
		console.log(
			`   🔧 Run: ${chalk.yellow("xala migrate convert --interactive")}`,
		);
	} else if (result.status === "needs-work") {
		console.log(
			"   ⚠️  This component needs some work to meet UI system standards.",
		);
		...
		console.log(
			`   🔄 Consider: ${chalk.yellow("xala migrate analyze")} first`,
		);
		console.log(
			`   🆕 Or generate new: ${chalk.yellow("xala generate component NewComponent")}`,
		);
	}
}
