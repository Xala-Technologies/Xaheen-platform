	...
	options: [
		{
			...
			description:
				"Target platform (react|nextjs|vue|angular|svelte|react-native|electron)",
			defaultValue: "react",
		},
		{
			flags: "--output <path>",
		...
		try {
			if (!type || !name) {
				throw new ValidationError(
					"Both type and name are required. Example: xala create component UserCard",
				);
			}
			...
			try {
				...
				console.log(chalk.bold("\n📝 Next Steps:"));
				console.log(
					`  1. Import: ${chalk.cyan(`import { ${name} } from './${results.importPath}'`)}`,
				);
				if (options.story) {
...
async function collectCreationInfo(
	type: string,
	name: string,
