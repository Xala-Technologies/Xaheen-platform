...
export const scaffoldCommand = new Command()
  ...
  .option('--tier2', 'Enable Tier 2: TypeScript Code Manipulation (Nx DevKit)', true)
  ...
  .option('-s, --services <services>', 'Comma-separated list of services to run')
  .action(async (name: string | undefined, options: ScaffoldOptions) => {
    try {
      ...
      logger.error(`Scaffolding failed: ${error instanceof Error ? error.message : String(error)}`);
      process.exit(1);
    }
  });
...
scaffoldCommand
  ...
  .action(async (name: string, projectOptions: any) => {
    ...
    const context = getDefaultScaffoldingContext(name, projectPath, projectOptions.framework);
...
scaffoldCommand
  ...
  .action(async (name: string, componentOptions: any) => {
    ...
    const spinner = ora(`Generating ${componentOptions.type} component: ${name}`).start();
    
    try {
      const result = await scaffolder.generateWithServices(context, { ...options, services: serviceOptions });
...
scaffoldCommand
  ...
  .option('--add-interfaces <interfaces>', 'Add TypeScript interfaces (JSON format)')
  ...
  .action(async (transformOptions: any) => {
    ...
    const options: HybridScaffoldingOptions = {
      tier2: {
        imports: transformOptions.addImports ? JSON.parse(transformOptions.addImports) as ImportStatement[] : [],
        interfaces: transformOptions.addInterfaces ? JSON.parse(transformOptions.addInterfaces) as TypeScriptInterface[] : [],
        routes: transformOptions.addRoutes ? JSON.parse(transformOptions.addRoutes) as RouteConfiguration[] : []
      },
      orchestration: {
        strategy: 'sequential'
      }
...
scaffoldCommand
  ...
  .action(async (name: string | undefined, previewOptions: any) => {
    ...
    const context = getDefaultScaffoldingContext(projectName, projectPath, previewOptions.framework);
    context.dryRun = true;
    context.features = previewOptions.features ? previewOptions.features.split(',') : [];
    ...
    try {
      ...
      if (changes.length === 0) {
        ...
        changes.forEach(change => {
          ...
          const color = operation === '+' ? 'green' : operation === '-' ? 'red' : 'yellow';
...
async function runInteractiveMode(options: ScaffoldOptions): Promise<void> {
  ...
  const projectPath = answers.action === 'project' ? resolve(answers.name) : process.cwd();
  ...
  try {
    ...
    logger.error(`Interactive mode failed: ${error instanceof Error ? error.message : String(error)}`);
    throw error;
  }
}

async function runDirectMode(name: string, options: ScaffoldOptions): Promise<void> {
  const projectPath = process.cwd();
  const context: ScaffoldingContext = {
    projectPath,
    projectName: name,
...
async function generateInteractiveProject(scaffolder: any, context: ScaffoldingContext) {
  const options = getDefaultHybridOptions();
  return await scaffolder.generateProject(context, options);
}

async function generateInteractiveComponent(scaffolder: any, context: ScaffoldingContext) {
  const serviceOptions = {
    'component-generator': {
  ...
  return await scaffolder.generateWithServices(context, { services: serviceOptions });
}

async function generateInteractivePage(scaffolder: any, context: ScaffoldingContext) {
  const serviceOptions = {
    'page-generator': {
      name: context.projectName,
...
async function runInteractiveTransform(scaffolder: any, context: ScaffoldingContext) {
  const options: HybridScaffoldingOptions = {
    tier2: {
      transformations: [],
      imports: [],
