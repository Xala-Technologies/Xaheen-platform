...
import {
	...
	TemplateValidator,
} from "../services/template-validator.js";
...
export const validateCommand = new Command("validate")
	...
	.option(
		"-s, --severity <level>",
		"Filter by severity level (error|warning|all)",
		"all",
	)
...
function displayConsoleReport(
	...
	complianceKeys.forEach((key) => {
		...
		const percentage =
			totalTemplates > 0
				? Math.round((compliantCount / totalTemplates) * 100)
				: 0;
		const status = percentage === 100 ? "✅" : percentage >= 80 ? "⚠️" : "❌";
		const label = key.charAt(0).toUpperCase() + key.slice(1);

		console.log(
			`${status} ${label}: ${compliantCount}/${totalTemplates} (${percentage}%)`,
		);
	});
	...
	if (invalidResults.length > 0) {
		...
		invalidResults.forEach((result) => {
			...
			result.violations
				...
				.forEach((violation) => {
					const location = violation.line
						? chalk.dim(` (Line ${violation.line})`)
						: "";
					console.log(
						`  ${chalk.red("❌")} ${chalk.bold(violation.rule)}${location}: ${violation.message}`,
					);
			...
			if (result.violations.length > 5) {
				console.log(
					`  ${chalk.dim(`... and ${result.violations.length - 5} more errors`)}`,
				);
			}
		});
	}
		...
		warningResults.slice(0, 3).forEach((result) => {
			...
			result.warnings.slice(0, 3).forEach((warning) => {
				const location = warning.line
					? chalk.dim(` (Line ${warning.line})`)
					: "";
				console.log(
					`  ${chalk.yellow("⚠️")} ${chalk.bold(warning.rule)}${location}: ${warning.message}`,
				);
...
async function generateMarkdownReport(
	...
	const markdownReport = validator
