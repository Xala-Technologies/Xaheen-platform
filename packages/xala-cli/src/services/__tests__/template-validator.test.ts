...
Card.displayName = 'Card';
      ...
      const result = await validator.validateContent(validTemplate, 'test-card.tsx');
...
export const BadCard = () => {
  return (
    <div className="p-4 border rounded">
      ...
      const result = await validator.validateContent(invalidTemplate, 'bad-card.tsx');

      expect(result.isValid).toBe(false);
      expect(result.violations.some(v => v.rule === 'no-raw-html')).toBe(true);
...
export const AccessibilityTest = () => {
  return (
    <Container>
      ...
      const result = await validator.validateContent(template, 'accessibility-test.tsx');

      expect(result.violations.some(v => v.rule === 'aria-attributes')).toBe(true);
      expect(result.violations.some(v => v.message.includes('missing accessibility label'))).toBe(true);
    });
...
export const HardcodedStyles = () => {
  return (
    <div style={{ color: '#ff0000', padding: '16px', margin: '8px' }}>
      ...
      const result = await validator.validateContent(template, 'hardcoded-styles.tsx');

      expect(result.violations.some(v => v.rule === 'design-token-usage')).toBe(true);
      expect(result.violations.some(v => v.rule === 'no-hardcoded-values')).toBe(true);
    });
...
export const TypeScriptIssues = (props: any) => {
  ...
  return <div>{result}</div>;
      ...
      const result = await validator.validateContent(template, 'typescript-issues.tsx');

      expect(result.violations.some(v => v.rule === 'typescript-strict')).toBe(true);
...
export const ResponsiveComponent = () => {
  return (
    <div className="grid grid-cols-4 w-full h-screen">
      ...
      const result = await validator.validateContent(template, 'responsive-test.tsx');

      expect(result.warnings.some(w => w.rule === 'responsive-classes')).toBe(true);
      expect(result.warnings.some(w => w.rule === 'breakpoint-usage')).toBe(true);
    });
...
export const NorwegianForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit} data-security-level="high">
      ...
      const result = await validator.validateContent(template, 'norwegian-form.tsx');

      // Should have warnings about localization
      expect(result.warnings.some(w => w.rule === 'localization-norwegian')).toBe(true);
    });
  });
...
describe('Individual Validation Rules', () => {
  describe('NoRawHTMLRule', () => {
    ...
    it('should pass for semantic components', async () => {
      const content = '<Container><Typography><Button>Click</Button></Typography></Container>';
      const result = await rule.validate(content, 'test.tsx');
  ...
  describe('DesignTokenUsageRule', () => {
    ...
    it('should detect hardcoded colors', async () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.violations.some(v => v.includes('Hardcoded color value'))).toBe(true);
    });
  ...
  describe('AriaAttributesRule', () => {
    ...
    it('should detect missing ARIA labels', async () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.violations[0]).toContain('Button missing accessibility label');
    });

    it('should pass with ARIA labels', async () => {
      const content = '<Button ariaLabel="Submit form" onClick={handleClick}>Submit</Button>';
      const result = await rule.validate(content, 'test.tsx');
