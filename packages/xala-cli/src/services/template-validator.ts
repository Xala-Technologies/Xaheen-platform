...
export class TemplateValidator {
				...
				if (!result.passed) {
					...
					result.violations.forEach((violation, index) => {
						...
						violations.push({
							...
							column: match?.column,
							suggestion:
								result.suggestions[index] || "No suggestion available",
						});
					});
					...
					result.warnings.forEach((warning, index) => {
						...
						warnings.push({
							...
							line: match?.line,
							suggestion:
								result.suggestions[index + result.violations.length] ||
	...
	async validateDirectory(
		...
		pattern: string = "**/*.{hbs,tsx,ts,vue,svelte}",
	): Promise<ReadonlyArray<TemplateValidationResult>> {
	...
	generateReport(results: ReadonlyArray<TemplateValidationResult>): string {
		...
		complianceKeys.forEach((key) => {
			...
			lines.push(
				`- **${this.capitalizeFirst(key)}**: ${status} ${compliantCount}/${results.length} (${percentage}%)`,
			);
		});
		...
		const perfectResults = results.filter(
			(r) => r.isValid && r.warnings.length === 0,
		);
	...
	private calculateScore(
		...
		return Math.max(
			0,
			Math.round(((maxScore - penaltyScore) / maxScore) * 100),
		);
	}
