...
export class LivePreviewService implements LivePreview {
	...
	async generatePreview(stack: StackConfiguration): Promise<PreviewContext> {
		try {
			...
			const projectStructure = await projectGenerator.getProjectStructurePreview(stack);
	 ...
	 */
	async validateConfiguration(stack: StackConfiguration): Promise<readonly ValidationError[]> {
	...
	estimateGenerationTime(stack: StackConfiguration): number {
		...
		if (stack.webFrontend && stack.webFrontend !== 'none') {
			const frontends = Array.isArray(stack.webFrontend) ? stack.webFrontend : [stack.webFrontend];
		...
		// Additional services
		const services = [
			stack.analytics, stack.monitoring, stack.notifications,
			stack.payments, stack.search, stack.caching
	...
	calculateComplexity(stack: StackConfiguration): 'simple' | 'moderate' | 'complex' {
		...
		// SAAS features
		const saasFeatures = [
			stack.saasAdmin, stack.subscriptions, stack.licensing,
			stack.rbac, stack.multiTenancy
		...
		const additionalServices = [
			stack.analytics, stack.monitoring, stack.notifications,
			stack.payments, stack.messaging, stack.search, stack.caching,
	 ...
	 * Generate dependencies preview
	 */
	private async generateDependenciesPreview(stack: StackConfiguration): Promise<Record<string, string>> {
	 ...
	 * Generate scripts preview
	 */
	private async generateScriptsPreview(stack: StackConfiguration): Promise<Record<string, string>> {
	 ...
	 * Generate config files preview
	 */
	private async generateConfigFilesPreview(stack: StackConfiguration): Promise<readonly string[]> {
	 ...
	 */
	private async performAdvancedValidation(stack: StackConfiguration): Promise<ValidationError[]> {
		const errors: ValidationError[] = [];
	 ...
	 */
	private async validateBestPractices(stack: StackConfiguration): Promise<ValidationError[]> {
		const errors: ValidationError[] = [];
		...
		if (stack.webDeploy && !['vercel', 'netlify', 'cloudflare'].includes(stack.webDeploy)) {
			errors.push(new ValidationError(
