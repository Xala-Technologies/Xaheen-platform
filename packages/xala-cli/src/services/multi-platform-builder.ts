...
export class MultiPlatformBuilder {
	...
	): Promise<ReadonlyArray<BuildResult>> {
	...
	private async buildSinglePlatform(
		platform: string,
		options: BuildOptions,
	): Promise<BuildResult> {
	...
	private async executePlatformBuild(
		platform: string,
		options: BuildOptions,
	): Promise<{
		outputPath: string;
		...
		switch (platform) {
			...
			default:
				throw new ValidationError(
					`Build not implemented for platform: ${platform}`,
				);
		}
	}
