...
export class BreakpointUsageRule implements ValidationRule {
	...
	validate(content: string, filePath: string): ValidationResult {
		...
		const hasResponsiveClasses = responsivePrefixes.some((prefix) =>
		...
		if (hasLayoutClasses && !hasResponsiveClasses && content.length > 500) {
			warnings.push(
				"Template uses layout classes but lacks responsive variants",
			);
			suggestions.push(
				"Add responsive prefixes (sm:, md:, lg:) to layout classes for better mobile experience",
			);
		}
		...
		if (breakpointMatches && breakpointMatches.length > 0) {
			const uniqueBreakpoints = [
				...new Set(breakpointMatches.map((m) => m.split(":")[0])),
			];

			// Warn if only using large breakpoints without mobile-first approach
			if (
				uniqueBreakpoints.includes("lg") &&
				!uniqueBreakpoints.includes("sm")
			) {
				warnings.push("Using large breakpoints without mobile-first approach");
				suggestions.push(
					"Consider mobile-first design with sm: and md: breakpoints",
				);
			}
		}
	...
	validate(content: string, filePath: string): ValidationResult {
		...
		conflictingPatterns.forEach(({ pattern, message }) => {
			if (pattern.test(content.replace(/\s+/g, " "))) {
				...
				suggestions.push(
					"Use responsive prefixes to avoid class conflicts: hidden sm:block",
				);
			}
		});
		...
		if (
			...
			suggestions.push(
				"Consider mobile navigation patterns with hidden/block responsive classes",
			);
		}
	...
	validate(content: string, filePath: string): ValidationResult {
		...
		if (gridMatches && gridMatches.length > 0) {
			...
			if (!hasResponsiveGrid) {
				...
				suggestions.push(
					"Add responsive grid column variations: grid-cols-1 md:grid-cols-2 lg:grid-cols-3",
				);
			}
		}
	...
	validate(content: string, filePath: string): ValidationResult {
		...
		lines.forEach((line, lineIndex) => {
			...
			while ((match = anyTypeRegex.exec(line)) !== null) {
				...
				suggestions.push(
					'Replace "any" with specific types or use "unknown" for better type safety',
				);
				matches.push({
					line: lineIndex + 1,
		...
		lines.forEach((line, lineIndex) => {
			...
			while ((match = nonNullRegex.exec(line)) !== null) {
				...
				if (nonNullCount > 3) {
					...
					suggestions.push(
						"Use proper null checking instead of non-null assertion operator",
					);
					matches.push({
						line: lineIndex + 1,
						column: match.index + 1,
		...
		if (propsMatches && propsMatches.length > 0) {
			propsMatches.forEach((propsName) => {
				...
				if (!interfacePattern.test(content) && !typePattern.test(content)) {
					violations.push(
						`${propsName} used without interface or type definition`,
					);
					suggestions.push(
						`Define interface ${propsName} with readonly properties`,
					);
				}
			});
		}
		...
		anyPatterns.forEach((pattern) => {
			lines.forEach((line, lineIndex) => {
				...
				while ((match = pattern.exec(line)) !== null) {
					...
					suggestions.push(
						'Use specific types, unknown, or generic constraints instead of "any"',
					);
					matches.push({
						line: lineIndex + 1,
						column: match.index + 1,
...
export class DataClassificationRule implements ValidationRule {
	...
	readonly description =
		"Validates Norwegian NSM data classification compliance";
	readonly severity = "error" as const;
	...
	validate(content: string, filePath: string): ValidationResult {
		...
		const hasClassification = classificationLevels.some(
			(level) =>
		...
		if (handlesSensitiveData && !hasClassification) {
			violations.push(
				"Template handles sensitive data without NSM classification",
			);
			suggestions.push(
				"Add data classification comment: @classification KONFIDENSIELT or HEMMELIG",
			);
		}
		...
		if (
			...
			if (!hasSecurityHeaders) {
				...
				suggestions.push(
					"Add proper authentication and security headers for NSM compliance",
				);
			}
		}
...
export class SecurityLabelsRule implements ValidationRule {
	...
	readonly description =
		"Ensures proper security labeling for Norwegian compliance";
	readonly severity = "warning" as const;
	readonly category = "norwegian" as const;
	...
	validate(content: string, filePath: string): ValidationResult {
		...
		if (hasSecurityComponents) {
			...
			const hasSecurityAttributes = securityAttributes.some((attr) =>
				content.includes(attr),
			);

			if (!hasSecurityAttributes) {
				warnings.push(
					"Security-related components should have security attributes",
				);
				suggestions.push(
					"Add data-security-level or data-classification attributes",
				);
			}
		}
...
export class LocalizationNorwegianRule implements ValidationRule {
	...
	validate(content: string, filePath: string): ValidationResult {
		...
		const hasLocalization =
			content.includes("useLocalization") ||
			content.includes("t(") ||
		...
		if (
			...
			if (!hasNorwegianFormatting) {
				...
				suggestions.push(
					"Use Norwegian locale (nb-NO) for date and time formatting",
				);
			}
		}
		...
		if (content.includes("ariaLabel") && hasNorwegianText) {
			...
			suggestions.push(
				"Use localization functions for ARIA labels and accessibility text",
			);
		}
...
export class ComplianceDocumentationRule implements ValidationRule {
	...
	validate(content: string, filePath: string): ValidationResult {
		...
		if (!hasFileHeader) {
			...
			suggestions.push(
				"Add JSDoc header with @fileoverview and @compliance information",
			);
		}
		...
		if (!hasComplianceInfo && content.length > 1000) {
			...
			suggestions.push(
				"Add compliance annotations (@compliance WCAG AAA, NSM)",
			);
		}
