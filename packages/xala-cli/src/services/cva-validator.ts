...
export class CVAValidator {
	private readonly rules: ReadonlyArray<CVAValidationRule> = [
		...
		},
		{
			...
			severity: "error",
			check: (content: string) =>
				content.includes("VariantProps<typeof") &&
				content.includes("Variants>"),
			message:
				"Component interface must extend VariantProps<typeof componentVariants>",
		},
		{
			name: "cva-variants",
			...
			severity: "error",
			check: (content: string) =>
				/const\s+\w+Variants\s*=\s*cva\(/.test(content),
		...
		},
		{
			...
			message:
				"Component should use semantic Tailwind classes (bg-primary, text-card-foreground, etc.)",
		},
		{
			name: "no-usetokens",
			...
			message:
				"Component should not use useTokens hook - use CVA variants instead",
		},
		{
			name: "cn-utility",
			...
			severity: "error",
			check: (content: string) =>
				content.includes("cn(") && content.includes("import { cn }"),
		...
		},
		{
			...
			check: (content: string) => {
				const hasVariants =
					content.includes("variants:") && content.includes("{");
				return hasVariants;
			},
		...
		},
		{
			...
			message:
				"Component should include accessibility attributes (aria-*, role, data-testid)",
		},
	];

	validate(content: string, componentName?: string): CVAValidationResult {
		logger.info(
			`Validating CVA compliance${componentName ? ` for ${componentName}` : ""}`,
		);
