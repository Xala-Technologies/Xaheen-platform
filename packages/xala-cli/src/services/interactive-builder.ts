...
export class InteractiveBuilder {
	...
	private async handleQuickPreset(): Promise<StackConfiguration> {
		...
		const allPresets = await configLoader
			.loadConfiguration()
			.then((config) => config.quickPresets);
		...
		const combinedPresets = [
			...
			...popularPresets.filter(
				(p) => !presets.find((existing) => existing.id === p.id),
			),
		];
		...
		if (combinedPresets.length === 0) {
			console.log(
				chalk.yellow(
					"No presets available for this project type. Switching to custom build...",
				),
			);
			return await this.handleCustomBuild();
		}
	...
	private async handleCustomBuild(): Promise<StackConfiguration> {
		...
		const relevantCategories = await configLoader.getRelevantCategories(
			projectType.id,
		);
		const allCategories = await configLoader
			.loadConfiguration()
			.then((config) => config.techCategories.map((tc) => tc.id));
	...
	private async selectProjectType(): Promise<ProjectType> {
		...
		if (!projectType) {
			throw new BuilderError(
				`Project type ${projectTypeId} not found`,
	 ...
	 * Select quick preset
	 */
	private async selectQuickPreset(
	...
	private async processCategorySelection(category: string): Promise<void> {
		...
		if (availableOptions.length === 0) {
			console.log(
				chalk.yellow(
					`⚠️  No compatible options available for ${category}, skipping...`,
				),
			);
			return;
		}
		...
		if (
			currentSelection &&
			!(await this.askToChangeSelection(category, currentSelection))
		) {
			console.log(
				chalk.blue(
					`⏭️  Keeping current selection for ${category}: ${currentSelection}`,
				),
			);
			return;
		}
		...
		const categoryPrompt = await this.createCategoryPrompt(
			category,
			availableOptions,
		);
		...
		if (!result.skipped) {
			...
			console.log(
				chalk.green(
					`✅ ${this.getCategoryDisplayName(category)}: ${this.formatSelection(result.selection)}`,
				),
			);
		}
	 ...
	 */
	private async createCategoryPrompt(
		category: string,
	 ...
	 */
	private async showCategoryPrompt(
		...
		const skipped =
			selection === "__skip__" ||
			(Array.isArray(selection) && selection.includes("__skip__"));
	 ...
	 */
	private async customizePresetStack(
		presetStack: StackConfiguration,
		...
		const categoriesToCustomize = await this.selectCategoriesToCustomize(
	 ...
	 */
	private async selectCategoriesToCustomize(
	 ...
	 */
	private async askToChangeSelection(
		category: string,
		currentSelection: string | readonly string[],
	): Promise<boolean> {
	 ...
	 */
	private async interactiveErrorFix(
	 ...
	 */
	private async selectErrorFix(
		error: ValidationError,
		suggestions: readonly TechOption[],
	): Promise<string | null> {
	...
	private async showStackSummary(stack: StackConfiguration): Promise<void> {
		...
		if (stack.addons && stack.addons.length > 0) {
			console.log(
				`${chalk.blue("Add-ons".padEnd(15))}: ${chalk.white(stack.addons.join(", "))}`,
			);
		}
	}
