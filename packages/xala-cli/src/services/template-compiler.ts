...
import { ComplianceValidator, ValidationResult } from './compliance-validator.js';
...
export class TemplateCompiler {
  ...
  async compileTemplate(
    ...
    options: CompilationOptions = { validateBefore: true, validateAfter: true, strict: false, autofix: false }
    ...
    try {
      ...
      if (options.validateBefore) {
        ...
        const beforeResult = await this.templateValidator.validateTemplate(templatePath);
        ...
        if (!beforeResult.isValid && options.strict) {
          ...
          return this.createFailureResult(templatePath, errors, warnings, validationResults, Date.now() - startTime, 0, Date.now() - validationStart);
        }
        
        validationTime += Date.now() - validationStart;
        logger.info(`✅ Pre-compilation validation completed (Score: ${beforeResult.score}%)`);
      }

      // Template compilation
      const compilationStart = Date.now();
      const compiledContent = await this.performCompilation(templatePath, context, options);
      ...
      if (options.validateAfter) {
        ...
        try {
          const afterResult = await this.templateValidator.validateTemplate(tempPath);
          
          // Run compliance validation on compiled output
          const complianceResult = await this.complianceValidator.validateCode(
            compiledContent,
            this.detectPlatform(options.format || this.detectFormatFromPath(outputPath))
          );
          ...
          if (options.autofix && !afterResult.isValid) {
            const fixedContent = await this.applyAutoFixes(compiledContent, afterResult);
            if (fixedContent !== compiledContent) {
              ...
              const revalidatedResult = await this.templateValidator.validateTemplate(tempPath);
              validationResults = { ...validationResults, after: revalidatedResult };
            }
          }
        ...
        if (validationResults.after) {
          logger.info(`✅ Post-compilation validation completed (Score: ${validationResults.after.score}%)`);
        }
      }
      ...
      const hasPostValidationErrors = validationResults.after !== undefined && 
        !validationResults.after.isValid && 
        validationResults.after.violations.some(v => v.severity === 'error');

      if (hasPostValidationErrors && options.strict && validationResults.after) {
      ...
      if (success) {
        logger.info(`✅ Template compilation completed successfully in ${totalTime}ms`);
      } else {
        logger.warn(`⚠️ Template compilation completed with errors in ${totalTime}ms`);
      }
      ...
      return this.createFailureResult(templatePath, errors, warnings, validationResults, totalTime, compilationTime, validationTime);
    }
  }
  ...
  async compileBatch(
    ...
    for (let i = 0; i < templatePaths.length; i++) {
      ...
      try {
        const result = await this.compileTemplate(templatePath, context, options);
        results.push(result);
      } catch (error) {
    ...
    logger.info(`✅ Batch compilation completed: ${successCount} success, ${failureCount} failures`);
  ...
  private async performCompilation(
    ...
    logger.debug(`Compiling template with context keys: ${Object.keys(context).join(', ')}`);
  ...
  private determineOutputPath(templatePath: string, options: CompilationOptions): string {
    const baseName = templatePath.replace(/\.hbs$/, '');
    const extension = this.getExtensionForFormat(options.format || this.detectFormatFromPath(templatePath));
  ...
  getValidationStats(results: ReadonlyArray<CompilationResult>): {
    ...
    const averageScore = totalTemplates > 0 
      ? Math.round(validationResults.reduce((sum, v) => sum + v.score, 0) / totalTemplates)
      : 0;
