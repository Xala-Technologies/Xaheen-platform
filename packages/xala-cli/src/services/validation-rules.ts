...
export class NoRawHTMLRule implements ValidationRule {
  ...
  readonly description = 'Prevents usage of raw HTML elements in favor of semantic components';
...
export class SemanticComponentUsageRule implements ValidationRule {
  ...
  readonly description = 'Ensures templates use semantic components from design system';
  readonly severity = 'error' as const;
  readonly category = 'semantic' as const;
  ...
  validate(content: string, filePath: string): ValidationResult {
    ...
    if (!hasRequiredImports) {
      ...
      suggestions.push('Import semantic components from @xala-technologies/ui-system');
    }

    // Check for semantic component usage
    const semanticComponents = [
      'Card', 'Stack', 'Container', 'Typography', 'Button',
      'Input', 'Select', 'Checkbox', 'Radio', 'Switch',
      'Modal', 'Dialog', 'Popover', 'Tooltip', 'Badge',
      'Avatar', 'Progress', 'Skeleton', 'Spinner'
    ];

    const usedComponents = semanticComponents.filter(comp => 
...
export class ComponentStructureRule implements ValidationRule {
  ...
  readonly description = 'Validates proper React component structure with CVA pattern';
  ...
  validate(content: string, filePath: string): ValidationResult {
    ...
    if (!content.includes('cva(') && content.includes('variants')) {
      ...
      suggestions.push('Use cva() function from class-variance-authority for variant management');
    }

    // Check for forwardRef usage
    if (content.includes('interface') && content.includes('Props') && !content.includes('forwardRef')) {
      warnings.push('Component should use React.forwardRef for better ref handling');
...
export class DesignTokenUsageRule implements ValidationRule {
  ...
  readonly description = 'Ensures proper design token usage instead of hardcoded values';
  readonly severity = 'error' as const;
  ...
  validate(content: string, filePath: string): ValidationResult {
    ...
    const semanticClasses = [
      ...
      'text-card-foreground', 'text-muted-foreground', 'border-border', 'border-input'
    ];

    const usesSemanticClasses = semanticClasses.some(cls => content.includes(cls));
    ...
    lines.forEach((line, lineIndex) => {
      ...
      while ((match = colorRegex.exec(line)) !== null) {
        ...
        suggestions.push('Use semantic Tailwind classes like bg-primary, text-foreground');
        matches.push({
          line: lineIndex + 1,
          column: match.index + 1,
          match: match[0]
...
export class NoHardcodedValuesRule implements ValidationRule {
  ...
  readonly description = 'Prevents hardcoded CSS values in favor of design tokens';
  readonly severity = 'error' as const;
  readonly category = 'tokens' as const;
  ...
  validate(content: string, filePath: string): ValidationResult {
    ...
    lines.forEach((line, lineIndex) => {
      ...
      while ((match = inlineStyleRegex.exec(line)) !== null) {
        ...
        suggestions.push('Use Tailwind CSS classes or CVA variants instead of inline styles');
        matches.push({
          line: lineIndex + 1,
          column: match.index + 1,
          match: match[0]
    ...
    lines.forEach((line, lineIndex) => {
      ...
      while ((match = arbitraryValueRegex.exec(line)) !== null) {
        ...
        suggestions.push('Use predefined Tailwind classes or add custom values to theme configuration');
        matches.push({
          line: lineIndex + 1,
          column: match.index + 1,
          match: match[0]
...
export class TokenReferenceRule implements ValidationRule {
  ...
  validate(content: string, filePath: string): ValidationResult {
    ...
    if (content.includes('var(--') && !content.includes('css')) {
      ...
      suggestions.push('Use Tailwind semantic classes instead of CSS custom properties');
    }

    // Check for consistent token naming
    const tokenPatterns = ['token.', 'tokens.', 'theme.', 'design.'];
    const hasTokenReferences = tokenPatterns.some(pattern => content.includes(pattern));

    if (hasTokenReferences && !content.includes('useTokens')) {
      warnings.push('Token references found without useTokens hook');
      suggestions.push('Use useTokens hook from @xala-technologies/ui-system for consistent token access');
    }
...
export class AriaAttributesRule implements ValidationRule {
  ...
  validate(content: string, filePath: string): ValidationResult {
    ...
    const interactiveElements = ['Button', 'Input', 'Select', 'Checkbox', 'Radio', 'Switch'];
    ...
    interactiveElements.forEach(element => {
      ...
      lines.forEach((line, lineIndex) => {
        ...
        while ((match = elementRegex.exec(line)) !== null) {
          ...
          if (!elementContent.includes('ariaLabel') && 
            ...
            suggestions.push(`Add ariaLabel prop to ${element}: ariaLabel="Descriptive label"`);
            matches.push({
              line: lineIndex + 1,
              column: match.index + 1,
              match: match[0]
...
export class SemanticHTMLRule implements ValidationRule {
  ...
  validate(content: string, filePath: string): ValidationResult {
    ...
    if (headings && headings.length > 1) {
      ...
      for (let i = 1; i < headingLevels.length; i++) {
        if (headingLevels[i] - headingLevels[i-1] > 1) {
          ...
          suggestions.push('Use sequential heading levels (h1, h2, h3...) for proper document structure');
          break;
        }
      }
    }
    ...
    const landmarkElements = ['main', 'nav', 'header', 'footer', 'aside', 'section'];
    const hasLandmarks = landmarkElements.some(element => content.includes(`<${element}`));
    
    if (!hasLandmarks && content.length > 1000) {
      warnings.push('Large template missing landmark elements');
      suggestions.push('Use semantic HTML5 landmarks (main, nav, header, footer) for better structure');
    }
...
export class KeyboardNavigationRule implements ValidationRule {
  ...
  validate(content: string, filePath: string): ValidationResult {
    ...
    const interactiveElements = content.match(/<(Button|Input|Select|Checkbox|Radio)[^>]*>/g);
    
    if (interactiveElements && interactiveElements.length > 0) {
      const hasFocusManagement = content.includes('tabIndex') || 
      ...
      if (!hasFocusManagement) {
        ...
        suggestions.push('Consider adding tabIndex, autoFocus, or keyboard event handlers for better navigation');
      }
    }
    ...
    if (content.length > 2000 && !content.includes('skip-to-content')) {
      ...
      suggestions.push('Add skip links for keyboard users to bypass repetitive content');
    }
...
export class ColorContrastRule implements ValidationRule {
  ...
  validate(content: string, filePath: string): ValidationResult {
    ...
    const colorClasses = content.match(/text-(red|green|yellow|blue|purple|pink|indigo)-\d+/g);
    
    if (colorClasses && colorClasses.length > 0) {
      const hasTextAlternatives = content.includes('ariaLabel') || 
                                  content.includes('title=') ||
                                  content.includes('sr-only');
      
      if (!hasTextAlternatives) {
        warnings.push('Color used for information without text alternatives');
        suggestions.push('Provide text alternatives for color-coded information (icons, labels, or screen reader text)');
      }
    }

    // Check for semantic color usage
    const hasSemanticColors = content.includes('text-destructive') ||
    ...
    if (!hasSemanticColors && colorClasses) {
      ...
      suggestions.push('Use semantic color classes like text-destructive, bg-success for better accessibility');
    }
...
export class ScreenReaderRule implements ValidationRule {
  ...
  validate(content: string, filePath: string): ValidationResult {
    ...
    const hasComplexContent = content.includes('table') || 
    ...
    if (hasComplexContent && !content.includes('sr-only')) {
      ...
      suggestions.push('Add sr-only classes with descriptive text for complex visual content');
    }

    // Check for proper live regions
    const hasDynamicContent = content.includes('useState') ||
                             content.includes('loading') ||
                             content.includes('error');

    if (hasDynamicContent && !content.includes('aria-live')) {
      warnings.push('Dynamic content may need live regions');
      suggestions.push('Add aria-live attributes to announce dynamic content changes');
    }
