...
export class YeomanIntegration {
  ...
  constructor(virtualFs: VirtualFileSystem, generatorsPath?: string) {
    ...
    this.generatorsPath = generatorsPath || join(__dirname, '../../../generators');
  }
  ...
  async generateProject(
    ...
    try {
      ...
      // Run generator
      const result = await this.runGenerator(generatorNamespace, generatorOptions);
  ...
  private createGeneratorClass(config: YeomanGeneratorConfig): typeof Generator {
    const self = this;
    ...
    return class extends Generator {
      ...
      async install() {
        if (!this.options.skipInstall && config.dependencies && config.dependencies.length > 0) {
      ...
      async end() {
        ...
        if (this.options.gitInit) {
          ...
          this.spawnCommandSync('git', ['commit', '-m', 'Initial commit from Xala CLI']);
        }
      }
    };
  }
  ...
  private async runGenerator(
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        generator.write = function(this: any, filepath: string, contents: string) {
