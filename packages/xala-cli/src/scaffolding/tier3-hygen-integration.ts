...
export class HygenIntegration {
	...
	async listGenerators(): Promise<readonly string[]> {
		try {
			const entries = await fs.readdir(this.generatorsPath, {
				withFileTypes: true,
			});
			return entries
	...
	async generate(
		...
		try {
			...
			const files = await this.generateFromTemplates(
				config,
				generationContext,
				dryRun,
			);
	...
	async generateComponent(
		...
		const hygenArgs = ["generate", "component", type, "--name", name];
...
${content}`;
	...
	async installTeamPatterns(
		patternsConfig: Record<string, HygenGeneratorConfig>,
	): Promise<void> {
...
`;
	...
	private async createGeneratorIndex(
		generatorPath: string,
		config: HygenGeneratorConfig,
	): Promise<void> {
		const indexContent = `/**
...
`;
	...
	private async createTeamPatternsIndex(
		patterns: Record<string, HygenGeneratorConfig>,
	): Promise<void> {
		const indexContent = {
			version: "1.0.0",
