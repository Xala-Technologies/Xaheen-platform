...
export const defaultValidationConfig: ValidationConfig = {
	...
	exclusions: {
		...
		directories: ["node_modules", ".git", "dist", "build", ".next", ".nuxt"],
		rules: [
...
export async function loadValidationConfig(
	...
	if (configPath) {
		try {
			...
			console.warn(
				`Failed to load validation config from ${configPath}:`,
				error,
			);
		}
	}
...
export function mergeValidationConfigs(
	...
	return {
		rules: {
			...
			designTokens: {
				...base.rules.designTokens,
				...override.rules?.designTokens,
			},
			accessibility: {
				...base.rules.accessibility,
		...
		exclusions: {
			...
			directories: [
				...base.exclusions.directories,
				...(override.exclusions?.directories || []),
			],
