...
export class ProjectGenerator {
	...
	private async generateConfigFiles(
		...
		if (["react", "vue", "angular"].includes(config.platform)) {
			const tsConfig = this.templateEngine.render(
				"configs/tsconfig.json.hbs",
				config,
			);
		...
		// ESLint config
		const eslintConfig = this.templateEngine.render(
			"configs/eslint.config.js.hbs",
			config,
		);
		writeFileSync(join(options.outputDir, "eslint.config.js"), eslintConfig);

		// Prettier config
		const prettierConfig = this.templateEngine.render(
			"configs/.prettierrc.hbs",
			config,
		);
		writeFileSync(join(options.outputDir, ".prettierrc"), prettierConfig);

		// Design system config
		const xalaConfig = this.templateEngine.render(
			"configs/xala.config.js.hbs",
			config,
		);
