	...
	describe("Template Processing Performance", () => {
		it("should load and process template under 100ms", async () => {
			const { metrics } = await benchmark.measure(
				"Template Loading",
				async () => {
					const { stdout } = await execAsync("xala templates validate button", {
						cwd: testDir,
					});
					return stdout;
				},
		...
		it("should process all templates under 2000ms", async () => {
			const { metrics } = await benchmark.measure(
				...
				async () => {
					const { stdout } = await execAsync("xala templates validate --all", {
						cwd: testDir,
					});
					return stdout;
				},
	...
	describe("AI Integration Performance", () => {
		...
		it("should chunk complex prompts efficiently", async () => {
			const { metrics } = await benchmark.measure(
				...
				async () => {
					const complexPrompt =
						"Generate a complete e-commerce platform with user management, product catalog, shopping cart, checkout, payment processing, order tracking, inventory management, and admin dashboard";
					const { stdout } = await execAsync(
	...
	describe("Build Performance", () => {
		...
		it("should analyze project under 1000ms", async () => {
			const { metrics } = await benchmark.measure(
				"Project Analysis",
				async () => {
					const { stdout } = await execAsync("xala analyze --quick", {
		...
		it("should validate TypeScript under 2000ms", async () => {
			const { metrics } = await benchmark.measure(
				...
				async () => {
					const { stdout } = await execAsync("xala validate --typescript", {
						cwd: path.join(testDir, "build-perf-test"),
					});
					return stdout;
				},
		...
		it("should check accessibility under 1500ms", async () => {
			const { metrics } = await benchmark.measure(
				"Accessibility Check",
				async () => {
					const { stdout } = await execAsync("xala validate --accessibility", {
						cwd: path.join(testDir, "build-perf-test"),
					});
					return stdout;
				},
			...
			const { metrics } = await benchmark.measure(
				...
				async () => {
					...
					for (const dir of dirs) {
						await fs.mkdir(path.join(testDir, "structure-test", dir), {
							recursive: true,
						});
					}
					return dirs;
			...
			const { metrics } = await benchmark.measure(
				...
				async () => {
					const promises = projects.map((proj) =>
						execAsync("xala build --quick", { cwd: path.join(testDir, proj) }),
					);
