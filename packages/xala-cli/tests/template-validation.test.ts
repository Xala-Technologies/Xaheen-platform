...
class TemplateValidator {
	private forbiddenPatterns = [
		{
			pattern: /useState\s*\(/,
			message: "useState hook found - components must be pure",
		},
		{
			pattern: /useEffect\s*\(/,
			message: "useEffect hook found - components must be pure",
		},
		{
			pattern: /useCallback\s*\(/,
			message: "useCallback hook found - components must be pure",
		},
		{
			pattern: /useMemo\s*\(/,
			message: "useMemo hook found - components must be pure",
		},
		{
			pattern: /useTokens\s*\(/,
			message: "useTokens hook found - use CSS custom properties",
		},
		{
			pattern: /useContext\s*\(/,
			message: "useContext found - pass props from parent",
		},
		{
			pattern: /createContext\s*\(/,
			message: "createContext found - providers should be separate",
		},
		{
			pattern: /style\s*=\s*\{\{/,
			message: "Inline styles found - use Tailwind/CVA classes",
		},
		{ pattern: /: any[\s,;\)]/, message: 'TypeScript "any" type found' },
		{
			pattern: /className\s*=\s*["']p-2\s/,
			message: "Small padding (p-2) - use professional sizing",
		},
		{
			pattern: /className\s*=\s*["']text-xs/,
			message: "Extra small text - maintain readability",
		},
	];
		...
		it("should generate compliance report for all templates", async () => {
			...
			const report = {
				...
				compliance: {
					...
					pureComponent: allTemplates.filter((t) => t.compliance.pureComponent)
						.length,
					cvaPattern: allTemplates.filter((t) => t.compliance.cvaPattern)
						.length,
					forwardRef: allTemplates.filter((t) => t.compliance.forwardRef)
						.length,
					typeScript: allTemplates.filter((t) => t.compliance.typeScript)
						.length,
					accessibility: allTemplates.filter((t) => t.compliance.accessibility)
						.length,
				},
			...
			console.log(
				`Valid: ${report.valid} (${Math.round((report.valid / report.total) * 100)}%)`,
			);
			console.log(`Invalid: ${report.invalid}`);
			console.log("\nCompliance Metrics:");
			console.log(`- No Hooks: ${report.compliance.noHooks}/${report.total}`);
			console.log(
				`- Pure Components: ${report.compliance.pureComponent}/${report.total}`,
			);
			console.log(
				`- CVA Pattern: ${report.compliance.cvaPattern}/${report.total}`,
			);
			console.log(
				`- ForwardRef: ${report.compliance.forwardRef}/${report.total}`,
			);
			console.log(
				`- TypeScript: ${report.compliance.typeScript}/${report.total}`,
			);
			console.log(
				`- Accessibility: ${report.compliance.accessibility}/${report.total}`,
			);
			console.log("─".repeat(50));
