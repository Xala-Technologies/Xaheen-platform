...
describe.skip("CLI Integration Tests (Requires built CLI)", () => {
	...
	beforeEach(async () => {
		...
		for (const file of files) {
			await fs.rm(path.join(testProjectDir, file), {
				recursive: true,
				force: true,
			});
		}
	...
	describe("Component Generation", () => {
		...
		it("should generate a button component from template", async () => {
			const { stdout } = await execAsync(
				"xala generate component Button --template button",
			);
	...
	describe("Error Handling", () => {
		it("should handle missing project gracefully", async () => {
			const { stderr } = await execAsync("xala generate component Test", {
				cwd: testProjectDir,
			}).catch((e) => e);
	...
	describe("Development Server", () => {
		it("should start development server", async () => {
			...
			const devProcess = exec("xala dev --port 3456", {
