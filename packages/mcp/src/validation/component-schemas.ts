 ...
 */
export const ComponentStabilitySchema = z.enum(
...
export const PrimitiveTypeSchema = z.enum([
	...
	"null",
	"undefined",
]) satisfies z.ZodSchema<PrimitiveType>;

/**
 * Complex types
 */
export const ComplexTypeSchema = z.enum([
	"function",
	"node",
	"element",
	"component",
	...
	"date",
	"file",
]) satisfies z.ZodSchema<ComplexType>;
 ...
 * Number constraints
 */
export const NumberConstraintsSchema = z
	.object({
...
export const PrimitiveTypeDefinitionSchema = z.object({
	...
	constraints: z
		.union([StringConstraintsSchema, NumberConstraintsSchema])
		.optional(),
});
 ...
 */
export const UnionTypeDefinitionSchema: z.ZodLazy<z.ZodType<any>> = z.lazy(() =>
	z.object({
		union: z
			.array(
				z.union([
 ...
 */
export const ArrayTypeDefinitionSchema: z.ZodLazy<z.ZodType<any>> = z.lazy(() =>
	z.object({
		array: z.union([
			PrimitiveTypeDefinitionSchema,
			ComplexTypeDefinitionSchema,
 ...
 */
export const ObjectTypeDefinitionSchema: z.ZodLazy<z.ZodType<any>> = z.lazy(
	() =>
		z.object({
...
export const CrossValidationSchema = z.object({
	props: z
		...
		.min(2, "Cross validation requires at least 2 props"),
...
export const SupportedPlatformSchema = z.enum([
	...
	"solid",
	"web-components",
]) satisfies z.ZodSchema<SupportedPlatform>;
 ...
 */
export const PlatformConfigsSchema = z.record(
	SupportedPlatformSchema,
	PlatformPropConfigSchema,
);
 ...
 */
export const PropDefinitionSchema: z.ZodLazy<z.ZodType<any>> = z.lazy(() =>
	z.object({
...
export const ChildrenCompositionSchema = z.object({
	supported: z.boolean(),
	types: z
		.array(z.enum(["string", "number", "element", "node", "function"]))
		.optional(),
	constraints: z
		.object({
...
export const PropsSpecificationSchema = z.object({
	schema: z.record(
		z
			.string()
			.regex(/^[a-zA-Z][a-zA-Z0-9]*$/, "Property name must be camelCase"),
...
export const PlatformSupportSchema = z.object({
	supported: z
		...
		.min(1, "At least one platform must be supported"),
	primary: SupportedPlatformSchema.optional(),
	implementations: z
		.record(SupportedPlatformSchema, PlatformImplementationSchema)
		.optional(),
});
 ...
 */
export const ComponentSpecificationSchema = z
	.object({
	...
	.refine(
		(data) => {
			...
			if (missingProps.length > 0) {
				throw new Error(
					`Required props not found in schema: ${missingProps.join(", ")}`,
				);
			}
	...
	.refine(
		(data) => {
			...
			const schemaKeys = Object.keys(data.props.schema);
			const missingProps = optionalProps.filter(
				(prop) => !schemaKeys.includes(prop),
			);

			if (missingProps.length > 0) {
				throw new Error(
					`Optional props not found in schema: ${missingProps.join(", ")}`,
				);
			}
	...
	.refine(
		(data) => {
			...
			if (
				data.platforms.primary &&
				!data.platforms.supported.includes(data.platforms.primary)
			) {
				throw new Error(
					`Primary platform '${data.platforms.primary}' must be in supported platforms list`,
				);
			}
 ...
 */
export const validateComponentSpecification = (
	data: unknown,
): {
	success: boolean;
	data?: any;
 ...
 */
export const validateNorwegianCompliance = (
	data: unknown,
): {
	success: boolean;
	data?: any;
 ...
 */
export const validatePropDefinition = (
	propName: string,
	data: unknown,
): {
	success: boolean;
 ...
 */
export type InferredComponentSpecification = z.infer<
	typeof ComponentSpecificationSchema
>;
export type InferredPropDefinition = z.infer<typeof PropDefinitionSchema>;
export type InferredNorwegianCompliance = z.infer<
	typeof NorwegianComplianceSchema
>;
