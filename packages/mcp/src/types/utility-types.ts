 ...
 */
export type NSMAccessLevel<T extends NSMClassification> = T extends "OPEN"
	? "public"
	: T extends "RESTRICTED"
		? "internal"
		: T extends "CONFIDENTIAL"
			? "authorized"
			: T extends "SECRET"
				? "classified"
				: never;
 ...
 */
export interface NorwegianComplianceMetadata<
	T extends NSMClassification = "OPEN",
> {
	readonly nsmClassification: T;
	readonly accessLevel: NSMAccessLevel<T>;
	readonly gdprCompliant: boolean;
	readonly auditRequired: T extends "RESTRICTED" | "CONFIDENTIAL" | "SECRET"
		? true
		: boolean;
	readonly encryptionRequired: T extends "CONFIDENTIAL" | "SECRET"
		? true
		: boolean;
...
export type ExtractPropTypes<T extends ComponentSpecification> = {
	readonly [K in keyof T["props"]["schema"]]: InferPropType<
		T["props"]["schema"][K]
	>;
};
...
export type ExtractRequiredProps<T extends ComponentSpecification> = {
	readonly [K in T["props"]["groups"]["required"][number]]: InferPropType<
		T["props"]["schema"][K]
	>;
};
...
export type ExtractOptionalProps<T extends ComponentSpecification> = {
	readonly [K in T["props"]["groups"]["optional"][number]]?: InferPropType<
		T["props"]["schema"][K]
	>;
};
...
export type InferPrimitiveType<T extends PrimitiveTypeDefinition> =
	T["primitive"] extends "string"
		? string
		: T["primitive"] extends "number"
			? number
			: T["primitive"] extends "boolean"
				? boolean
				: T["primitive"] extends "null"
					? null
					: T["primitive"] extends "undefined"
						? undefined
						: never;
...
export type InferComplexType<T extends ComplexTypeDefinition> =
	T["complex"] extends "function"
		? Function
		: T["complex"] extends "node"
			? React.ReactNode
			: T["complex"] extends "element"
				? React.ReactElement
				: T["complex"] extends "component"
					? React.ComponentType<any>
					: T["complex"] extends "ref"
						? React.RefObject<HTMLElement>
						: T["complex"] extends "date"
							? Date
							: T["complex"] extends "file"
								? File
								: unknown;
...
export type InferCustomType<T extends CustomTypeDefinition> =
	T["custom"] extends "color"
		? string
		: T["custom"] extends "size"
			? "xs" | "sm" | "md" | "lg" | "xl"
			: T["custom"] extends "variant"
				? "primary" | "secondary" | "success" | "warning" | "error"
				: T["custom"] extends "email"
					? string
					: T["custom"] extends "url"
						? string
						: string;
 ...
 */
export type PlatformProps<T, P extends SupportedPlatform> = P extends "react"
	? T & ReactSpecificProps
	: P extends "vue"
		? T & VueSpecificProps
		: P extends "angular"
			? T & AngularSpecificProps
			: P extends "svelte"
				? T & SvelteSpecificProps
				: P extends "solid"
					? T & SolidSpecificProps
					: T;
...
export const validateNorwegianCompliance = <T>(
	...
	if (currentLevel < requiredLevel) {
		return {
			success: false,
			errors: [
				`NSM classification '${nsmClassification}' is below required level '${requiredNSMLevel}'`,
			],
		};
	}
 ...
 */
export const isComponentSpecification = (
	data: unknown,
): data is ComponentSpecification => {
	return (
		typeof data === "object" &&
		data !== null &&
		"metadata" in data &&
 ...
 */
export const isComponentCategory = (
	value: string,
): value is ComponentCategory => {
 ...
 */
export const isComponentStability = (
	value: string,
): value is ComponentStability => {
	const stabilities: ComponentStability[] = [
		"experimental",
		"beta",
		"stable",
		"deprecated",
 ...
 */
export const isNSMClassification = (
	value: string,
): value is NSMClassification => {
...
export const createComponentFactory = <T extends ComponentSpecification>(
	...
	return (
		...
		const validatedProps = props as PlatformProps<
			ComponentProps<T>,
