...
export class CliStyleTools {
	 ...
	 */
	async listTemplates(
		args: { platform?: string; category?: string; filter?: string } = {},
	) {
	...
	async getTemplateInfo(args: { name: string }) {
		...
		if (!template) {
			throw new Error(
				`Template "${name}" not found. Use list_templates to see available templates.`,
			);
		}
	...
	async systemDoctor() {
		...
		try {
			...
			const testConfig = {
				...
				responsive: {
					breakpoints: ["mobile", "desktop"] as Array<
		...
		if (usedMem > 500) {
			output +=
				"- ⚠️ Consider clearing template cache if memory usage is high\n";
		}
		if (cacheStats.size === 0) {
			output +=
				"- 💡 Template cache is empty - it will populate as you use components\n";
		}

		output +=
			"- ✅ System appears healthy and ready for component generation\n";
...
export const cliStyleTools: Tool[] = [
	{
		name: "list_templates",
		description:
			"List available component templates with filtering options (CLI-style)",
		inputSchema: {
			type: "object",
			properties: {
				platform: {
	...
	},
	{
		...
		description:
			"Get detailed information about a specific template (CLI-style)",
		inputSchema: {
			type: "object",
			properties: {
				name: {
