...
export class CliIntegrationManager {
	...
	private buildPageCommands(
		...
		commands.push(
			`${this.xalaCliPath} generate page ${pageName} --type ${pageType}`,
		);
		...
		for (const line of lines) {
			...
			if (
				...
				const match = line.match(
					/(?:Created:|Generated:|✅)\s*(.+\.(tsx?|jsx?|vue|html|css|json|md))/,
				);
				if (match && match[1]) {
...
export const PREDEFINED_WORKFLOWS = {
	"full-stack-setup": {
		...
		description:
			"Complete setup for full-stack application with authentication, database, and deployment",
		steps: [
			"create-project",
			"setup-database",
			"add-authentication",
