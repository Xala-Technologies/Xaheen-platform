...
import { cacheUtils, templateCache } from "../utils/template-cache.js";
...
export class ComponentRetrievalTools {
	...
	constructor() {
		...
		this.specificationPath = path.resolve(
			__dirname,
	...
	async getComponentSource(args: {
		...
		try {
			...
			if (!exists) {
				throw new Error(
					`Component template not found: ${name} for ${platform}`,
				);
			}

			// Read main component source with caching
			const source = await cacheUtils.cacheTemplate(templatePath, () =>
				fs.readFile(templatePath, "utf-8"),
			);

			// Get component metadata with caching
			const metadata = await cacheUtils.cacheMetadata(name, platform, () =>
			...
			throw new Error(
				`Failed to retrieve component source: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	async getComponentDemo(args: {
		...
		try {
			...
			throw new Error(
				`Failed to retrieve component demo: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	async browseComponentLibrary(
		...
		try {
			...
			const platforms = [
				...new Set(allComponents.flatMap((comp) => comp.platforms)),
			];
			...
			throw new Error(
				`Failed to browse component library: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	async getComponentMetadata(
		...
		try {
			...
			metadata.dependencies = [
				...new Set([
					...metadata.dependencies,
					...additionalMetadata.dependencies,
				]),
			];
			metadata.features = [
				...new Set([...metadata.features, ...additionalMetadata.features]),
			];
			...
			throw new Error(
				`Failed to get component metadata: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	private async generateBasicDemo(
		name: string,
		platform: string,
	): Promise<string> {
...
import ${componentName} from './${componentName}.vue';
	...
	private async analyzeTemplate(
		templatePath: string,
	): Promise<{ dependencies: string[]; features: string[] }> {
...
export const componentRetrievalTools: Tool[] = [
	{
		...
		description:
			"Retrieve component source code from the template library with metadata and related files",
		inputSchema: {
			type: "object",
			properties: {
				name: {
	...
	},
	{
		...
		description:
			"Get component demo/example code with usage patterns and props documentation",
		inputSchema: {
			type: "object",
			properties: {
				name: {
	...
	},
	{
		...
		description:
			"Browse the component library with filtering and search capabilities",
		inputSchema: {
			type: "object",
			properties: {
				platform: {
	...
	},
	{
		...
		description:
			"Get detailed component metadata including dependencies, features, and configuration options",
		inputSchema: {
			type: "object",
			properties: {
				name: {
