	...
	return [
		{
			...
			description:
				"Generate a component from its JSON specification with full type safety and compliance",
			inputSchema: {
				type: "object",
				properties: {
					componentName: {
						...
						enum: Object.keys(
							specifications.getSpecificationByName("Button")
								? { Button: true }
								: {},
						),
					},
		...
		},
		{
			...
			description:
				"List all available component specifications with their metadata",
			inputSchema: {
				type: "object",
				properties: {
					category: {
		...
		},
		{
			...
			description:
				"Get detailed specification for a component including all props, variants, and compliance info",
			inputSchema: {
				type: "object",
				properties: {
					componentName: {
		...
		},
		{
			...
			description:
				"Generate a complete form using form component specifications",
			inputSchema: {
				type: "object",
				properties: {
					formName: {
		...
		},
		{
			...
			description:
				"Generate layouts using Container, Grid, and Stack specifications",
			inputSchema: {
				type: "object",
				properties: {
					layoutName: {
		...
		},
		{
			...
			description:
				"Generate navigation using WebNavbar, Sidebar, Tabs, or Breadcrumb specifications",
			inputSchema: {
				type: "object",
				properties: {
					navigationType: {
		...
		},
		{
			...
			description:
				"Check if a component meets specification compliance requirements",
			inputSchema: {
				type: "object",
				properties: {
					componentName: {
		...
		},
		{
			...
			description:
				"Generate comprehensive tests based on component specification",
			inputSchema: {
				type: "object",
				properties: {
					componentName: {
		...
		},
		{
			...
			description:
				"Migrate existing component to use specification-based architecture",
			inputSchema: {
				type: "object",
				properties: {
					componentPath: {
		...
		},
		{
			...
			description:
				"Generate a complete component bundle from multiple specifications",
			inputSchema: {
				type: "object",
				properties: {
					bundleName: {
		...
		// ===== NEW ADVANCED TOOLS =====
		{
			...
			description:
				"Analyze existing project structure and generate migration strategy",
			inputSchema: {
				type: "object",
				properties: {
					projectPath: {
		...
		},
		{
			...
			description:
				"Set up complete development environment with testing, linting, and pre-commit hooks",
			inputSchema: {
				type: "object",
				properties: {
					projectPath: {
...
export const specificationToolHandlers = {
	generate_from_spec: async (args: any) => {
		...
		if (!spec) {
			throw new Error(
				`No specification found for component: ${args.componentName}`,
			);
		}
	...
	list_specifications: async (args: any) => {
		...
		if (args.category && args.category !== "all") {
			filtered = filtered.filter(
				(spec) => spec.metadata.category === args.category,
			);
		}
		...
		return {
			...
			specifications: filtered.map((spec) => ({
				...
				nsmClassification:
					spec.compliance?.norwegian?.nsmClassification || "OPEN",
	...
	check_spec_compliance: async (args: any) => {
		...
		if (args.complianceType === "all" || args.complianceType === "norwegian") {
			compliance.results.norwegian = {
				nsmClassification:
					spec.compliance?.norwegian?.nsmClassification || "Not specified",
	...
	generate_types_from_spec: async (args: any) => {
		...
		const specs =
			componentNames.length > 0
				? componentNames
						.map((name: string) => specifications.getSpecificationByName(name))
						.filter(Boolean)
...
${Object.entries(spec.props?.schema || {})
...
${Object.entries(spec.props?.schema || {})
	.map(([propName, propDef]: [string, any]) => {
		const zodType =
			propDef.type?.primitive === "string"
				? "z.string()"
				: propDef.type?.primitive === "number"
					? "z.number()"
					: propDef.type?.primitive === "boolean"
						? "z.boolean()"
						: "z.any()";
...
${Object.entries(spec.variants?.simple || {})
	.map(([variantName, variant]: [string, any]) => {
...
`;
	...
	generate_test_from_spec: async (args: any) => {
		...
		const testGenerator = new (
			await import("../generators/TestGenerator.js")
		).TestGenerator();
	...
	analyze_spec_coverage: async (args: any) => {
		...
		const coverage = {
			...
			categories: {
				"action-feedback": allSpecs.filter(
					(s) => s.metadata.category === "action-feedback",
				).length,
				"data-display": allSpecs.filter(
					(s) => s.metadata.category === "data-display",
				).length,
				form: allSpecs.filter((s) => s.metadata.category === "form").length,
				layout: allSpecs.filter((s) => s.metadata.category === "layout").length,
				navigation: allSpecs.filter((s) => s.metadata.category === "navigation")
					.length,
	...
	generate_spec_bundle: async (args: any) => {
		const componentSpecs = args.components
			.map((name: string) => specifications.getSpecificationByName(name))
			.filter(Boolean);
	...
	list_workflows: async (args: any) => {
		return {
			success: true,
			workflows: Object.entries(PREDEFINED_WORKFLOWS).map(
				([key, workflow]) => ({
					name: key,
