...
export class GeneratorTools {
  ...
  async generateForm(args: {
    ...
    fields: Array<{
      name: string;
      type: 'input' | 'textarea' | 'select' | 'checkbox' | 'radio' | 'switch' | 'slider' | 'date' | 'time';
      label: string;
      placeholder?: string;
      required?: boolean;
      disabled?: boolean;
    ...
    const config: FormConfig = {
      ...
      submission: {
        ...
        successMessage: args.submission?.successMessage ?? 'Form submitted successfully',
  ...
  async generatePageTemplate(args: {
    name: string;
    platform: SupportedPlatform;
    template: 'dashboard' | 'landing' | 'auth' | 'profile' | 'settings' | 'analytics' | 'user-management' | 'content-management' | 'e-commerce' | 'blog';
    layout?: {
    ...
    theme?: 'enterprise' | 'finance' | 'healthcare' | 'education' | 'ecommerce' | 'productivity';
  ...
  async generateMultiPlatform(args: {
    ...
    category: 'components' | 'data-components' | 'theme-components' | 'layouts' | 'providers' | 'patterns' | 'tools';
  ...
  async generateTests(args: {
    ...
    platform: SupportedPlatform;
    testTypes?: Array<'unit' | 'integration' | 'e2e' | 'visual' | 'accessibility'>;
    framework?: 'jest' | 'vitest' | 'cypress' | 'playwright' | 'testing-library';
  }) {
    return {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        columns: {
          ...
          items: {
            ...
            properties: {
              ...
              type: {
                type: 'string',
                enum: ['text', 'number', 'date', 'boolean', 'badge', 'avatar', 'actions'],
              },
        ...
        fields: {
          ...
          items: {
            ...
            properties: {
              ...
              type: {
                type: 'string',
                enum: ['input', 'textarea', 'select', 'checkbox', 'radio', 'switch', 'slider', 'date', 'time'],
              },
              label: { type: 'string' },
        ...
        navigation: {
          type: 'object',
          properties: {
            type: { type: 'string', enum: ['horizontal', 'vertical', 'drawer', 'tabs', 'breadcrumb'] },
            items: {
              type: 'array',
              items: {
                type: 'object',
        ...
        styling: {
          type: 'object',
          properties: {
            variant: { type: 'string', enum: ['default', 'outline', 'ghost', 'destructive', 'secondary'] },
        ...
        runtime: {
          type: 'string',
          enum: ['node', 'python', 'go', 'java', 'dotnet', 'php', 'rust', 'alpine', 'distroless'],
          description: 'Container runtime (for docker type)',
        },
        imageName: {
          type: 'string',
          description: 'Container image name (for docker/kubernetes/helm types)',
        },
        imageTag: {
          type: 'string',
        ...
        options: {
          type: 'object',
          properties: {
            includeTests: { type: 'boolean', description: 'Generate test files' },
            includeStories: { type: 'boolean', description: 'Generate Storybook stories' },
            includeDocs: { type: 'boolean', description: 'Generate documentation' },
            includeLocalization: { type: 'boolean', description: 'Generate localization files' },
    ...
    switch (args.type) {
      case 'component':
        ...
        return await tools.componentGenerator.generateComponent({
          ...
          styling: args.styling || { variant: 'default', spacing: 'comfortable' },
          accessibility: {
            level: 'AAA',
            screenReader: true,
            keyboardNavigation: true,
