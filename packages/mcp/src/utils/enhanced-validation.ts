...
const errorMessages = {
	INVALID_COMPONENT_NAME:
		'Component name must be a valid PascalCase identifier (e.g., "UserProfile", "DataTable")',
	INVALID_PLATFORM:
		"Platform must be one of: react, nextjs, vue, angular, svelte, electron, react-native",
	INVALID_CATEGORY:
		"Category must be one of: components, data-components, theme-components, layouts, providers, patterns, tools",
	MISSING_REQUIRED_FIELD: (field: string) =>
		`Required field "${field}" is missing`,
	INVALID_PRESET_TYPE:
		"Preset type must be one of: basic-form, data-table, navigation-menu, dashboard-layout, modal-dialog",
	INVALID_THEME:
		"Theme must be one of: enterprise, finance, healthcare, education, ecommerce, productivity",
	COMPONENT_NAME_TOO_SHORT: "Component name must be at least 2 characters long",
	COMPONENT_NAME_TOO_LONG:
		"Component name must be less than 50 characters long",
	INVALID_ACCESSIBILITY_LEVEL:
		'Accessibility level must be either "AA" or "AAA"',
	INVALID_VARIANT:
		"Variant must be one of: default, outline, ghost, destructive, secondary",
};
...
export class EnhancedValidator {
		...
		try {
			...
			if (!config.name) {
				errors.push({
					...
					code: "MISSING_NAME",
					suggestions: [
						'Provide a component name like "UserProfile" or "DataTable"',
					],
				});
			} else {
				try {
					...
					if (e instanceof z.ZodError) {
						errors.push({
							field: "name",
							message:
								e.errors[0]?.message || errorMessages.INVALID_COMPONENT_NAME,
							code: "INVALID_NAME",
							suggestions: [
			...
			if (!config.platform) {
				errors.push({
					...
					suggestions: [
						"Choose from: react, nextjs, vue, angular, svelte, electron, react-native",
					],
				});
			} else {
				try {
					...
					errors.push({
						...
						suggestions: [
							"Available platforms: react, nextjs, vue, angular, svelte, electron, react-native",
						],
					});
				}
			}
			...
			if (!config.category) {
				errors.push({
					...
					suggestions: [
						"Choose from: components, data-components, layouts, navigation, form, etc.",
					],
				});
			} else {
				try {
					...
					errors.push({
						...
						suggestions: [
							"Available categories: components, data-components, theme-components, layouts, providers, patterns, tools",
						],
					});
				}
			}
			...
			if (config.platform === "react-native" && config.category === "layouts") {
				warnings.push({
					field: "category",
					message:
						"Layout components may have limited functionality on React Native",
					suggestion:
						'Consider using "components" category for better React Native support',
				});
			}
			...
			if (config.accessibility?.level === "AA" && config.theme === "finance") {
				warnings.push({
					...
					message:
						"Financial applications typically require WCAG AAA compliance",
					suggestion:
						'Consider setting accessibility.level to "AAA" for financial themes',
				});
			}

			// Check for platform-specific feature conflicts
			if (config.platformConfig?.features) {
				this.validatePlatformFeatures(
					config.platform,
					config.platformConfig.features,
					warnings,
				);
			}
	...
	static validateQuickGenerate(args: any): ValidationResult {
		...
		if (!args.type) {
			errors.push({
				...
				code: "MISSING_TYPE",
				suggestions: [
					"Choose from: basic-form, data-table, navigation-menu, dashboard-layout, modal-dialog",
				],
			});
		} else {
			try {
				...
				errors.push({
					...
					code: "INVALID_TYPE",
					suggestions: [
						"Available types: basic-form, data-table, navigation-menu, dashboard-layout, modal-dialog",
					],
				});
			}
		}
		...
		if (args.type === "dashboard-layout" && args.platform === "react-native") {
			warnings.push({
				...
				suggestion:
					"Consider using navigation-menu or basic-form for React Native",
			});
		}
	 ...
	 */
	private static validatePlatformFeatures(
		platform: string,
		features: any,
		...
		if (platform === "nextjs") {
			if (features.appRouter && features.pagesRouter) {
				warnings.push({
					...
					suggestion:
						"Choose either App Router (recommended) or Pages Router, not both",
				});
			}
		}
	}
	...
	static getValidationSuggestions(config: any): string[] {
		...
		if (config.name && typeof config.name === "string") {
			if (config.name.includes("-") || config.name.includes("_")) {
				suggestions.push(
					'Convert component name to PascalCase (e.g., "user-profile" → "UserProfile")',
				);
			}
		...
		if (
			...
			!config.platformConfig?.features?.appRouter
		) {
		...
		if (
			config.platform === "vue" &&
			!config.platformConfig?.features?.compositionApi
		) {
