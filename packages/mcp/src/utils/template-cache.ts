...
export class TemplateCache {
	...
	private cleanup(): void {
		...
		if (toDelete.length > 0) {
			console.log(
				`TemplateCache: Cleaned up ${toDelete.length} expired entries`,
			);
		}
	}
	...
	async warmUp(
		...
		console.log(
			`TemplateCache: Warming up cache with ${templatePaths.length} templates...`,
		);

		const promises = templatePaths.map(
			async ({ key, path: filePath, loader }) => {
				try {
		...
		console.log(
			`TemplateCache: Warm up completed. Cache size: ${this.cache.size}`,
		);
	}
