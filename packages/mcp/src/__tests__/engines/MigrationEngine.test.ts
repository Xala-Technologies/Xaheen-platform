...
describe("MigrationEngine", () => {
	...
	describe("executePhase", () => {
		...
		it("should handle phase with unsatisfied dependencies", async () => {
			...
			expect(result.errors).toContain(
				"Phase dependencies not satisfied: missing-phase",
			);
		});
	...
	describe("executeTransformation", () => {
      ...
      `,
			...
			const result = await engine.executeTransformation(transformation, [
				"Button",
			]);

			expect(result.success).toBe(true);
			expect(result.transformedFiles).toContain(
				`${mockProject.root}/src/components/Button.tsx`,
			);
			expect(result.errors).toHaveLength(0);

			// Check that transformation was applied
			const transformedContent = mockFs.files.get(
				`${mockProject.root}/src/components/Button.tsx`,
			);
      ...
      `,
		...
		it("should skip files that do not exist", async () => {
			...
			expect(result.skippedFiles).toContain(
				`${mockProject.root}/src/components/NonExistentComponent.tsx`,
			);
		});
	...
	describe("transformFile", () => {
		it("should transform file with replace operation", async () => {
			...
			mockFs.files.set(
				filePath,
				"This contains oldText that should be replaced.",
			);
	...
	describe("rollback", () => {
		it("should rollback changes from backup", async () => {
			...
			expect(result.success).toBe(true);
			expect(result.restoredFiles).toContain(
				`${mockProject.root}/src/components/Button.tsx`,
			);
			expect(result.restoredFiles).toContain(
				`${mockProject.root}/src/components/Input.tsx`,
			...
			expect(
				mockFs.files.get(`${mockProject.root}/src/components/Button.tsx`),
			).toBe("original button content");
			expect(
				mockFs.files.get(`${mockProject.root}/src/components/Input.tsx`),
			).toBe("original input content");
		});
	...
	describe("generateComponentMapping", () => {
		...
		it("should return empty array for unknown frameworks", () => {
			const mappings = engine.generateComponentMapping(
				"UnknownFramework",
				"AnotherUnknown",
			);
