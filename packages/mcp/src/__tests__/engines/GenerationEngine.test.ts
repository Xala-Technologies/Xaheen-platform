...
describe("GenerationEngine", () => {
	...
	describe("generateFromDescription", () => {
		it("should generate component from natural language description", async () => {
			const description =
				"Create a primary button component with loading state and click handler";
      ...
      `;
		...
		it("should handle modal component generation", async () => {
			const description =
				"Create a modal dialog with backdrop blur, close button, and escape key support";
	...
	describe("generateComponent", () => {
		...
		it("should generate complex components with multiple files", async () => {
			...
			const mainFile = result.files.find((f) =>
				f.path.includes("DataTable.tsx"),
			);
			expect(mainFile).toBeDefined();

			// Should generate hooks
			const sortHook = result.files.find((f) =>
				f.path.includes("useTableSort"),
			);
			...
			const headerComponent = result.files.find((f) =>
				f.path.includes("TableHeader"),
			);
		...
		it("should handle different styling approaches", async () => {
			...
			const cssFile = modulesResult.files.find((f) =>
	...
	describe("generatePage", () => {
		...
		it("should handle different page layouts", async () => {
			...
			const pageFile = result.files.find((f) =>
				f.path.includes("DashboardPage"),
			);
	...
	describe("generateProject", () => {
		it("should generate complete project structure", async () => {
			const projectSpec = {
				...
				features: ["typescript", "eslint", "prettier", "husky", "lint-staged"],
				initialComponents: [
			...
			const tailwindFile = result.files.find((f) =>
		...
		it("should handle different project types", async () => {
			...
			const nextConfigFile = result.files.find(
				(f) => f.path === "next.config.js",
			);
	...
	describe("Requirements Parsing", () => {
		it("should extract component names from descriptions", () => {
			...
			expect(
				(engine as any).extractComponentName("Create a Button component"),
			).toBe("Button");
			expect(
				(engine as any).extractComponentName("Build a DataTable with sorting"),
			).toBe("DataTable");
			expect(
				(engine as any).extractComponentName("Generate a modal dialog"),
			).toBe("Modal");
		});

		it("should extract component types from descriptions", () => {
			expect(
				(engine as any).extractComponentType("Create a button component"),
			).toBe("button");
			expect(
				(engine as any).extractComponentType("Build a form with validation"),
			).toBe("form");
			expect(
				(engine as any).extractComponentType("Generate a modal dialog"),
			).toBe("modal");
			expect((engine as any).extractComponentType("Make a data table")).toBe(
				"table",
			);
			expect(
				(engine as any).extractComponentType("Create a navigation menu"),
			).toBe("navigation");
		});
	...
	describe("Integration", () => {
		it("should generate components compatible with project structure", async () => {
			const result = await engine.generateFromDescription(
				"Create a theme-aware button component",
			);
			...
			if (
				generationContext.projectStructure.framework.features?.includes(
					"tailwind",
				)
			) {
