	...
	describe("Component Specification Management", () => {
		...
		it("should reject invalid component specifications", () => {
			...
			expect(result.errors).toContain(
				"Prop 'children' is missing required 'type' field",
			);
		});
		...
		it("should filter specifications by platform support", () => {
			...
			const filterByPlatform = (
				...
				return specifications.filter(
					(spec) => spec.platform && (spec.platform as any)[platform] === true,
				);
			};
			...
			const generateTSInterface = (spec: typeof sampleButtonSpec) => {
				...
				const props = Object.entries(spec.props)
					.map(([propName, propConfig]) => {
						...
						const comment = propConfig.description
		...
		it("should generate component templates from specifications", () => {
			const generateComponentTemplate = (spec: typeof sampleButtonSpec) => {
				...
				const defaultProps = Object.entries(spec.props)
					...
					.map(
						([name, config]) => `${name} = ${JSON.stringify(config.default)}`,
					)
					.join(", ");
...
${stories}`;
	...
	describe("Specification Validation", () => {
		it("should validate prop types", () => {
			const validatePropTypes = (props: Record<string, any>) => {
				...
				for (const [propName, propConfig] of Object.entries(props)) {
					...
					if (type.includes("|")) {
						...
						const validUnion = unionTypes.every(
							(t: string) =>
					...
					if (propConfig.required && propConfig.default !== undefined) {
						errors.push(
							`Prop '${propName}' cannot be required and have a default value`,
						);
					}
				}
			...
			expect(invalidResult.errors).toContain(
				"Invalid type for prop 'invalidProp': InvalidType",
			);
			expect(invalidResult.errors).toContain(
				"Prop 'conflictProp' cannot be required and have a default value",
			);
		});
		...
		it("should validate accessibility requirements", () => {
			const validateAccessibility = (spec: typeof sampleButtonSpec) => {
				...
				if (spec.category === "overlay" && !accessibility.focusManagement) {
					errors.push(
						"Overlay components require focus management specification",
					);
				}
		...
		it("should validate design token usage", () => {
			const validateDesignTokens = (spec: typeof sampleButtonSpec) => {
				...
				if (!spec.designTokens) {
					warnings.push(
						"No design tokens specified - consider using design system tokens",
					);
					...
					for (const [key, value] of Object.entries(obj)) {
						...
						if (typeof value === "string") {
							...
							if (
								...
								warnings.push(
									`Hardcoded color value at ${currentPath}: ${value}`,
								);
							}
							...
							if (/^\d+px$/.test(value)) {
								warnings.push(
									`Hardcoded pixel value at ${currentPath}: ${value}`,
								);
							}
				...
				if (
					...
					Object.keys(designTokens.spacing).length === 0
				) {
			...
			expect(hardcodedResult.warnings).toContain(
				"Hardcoded color value at colors.primary: #007acc",
			);
			expect(hardcodedResult.warnings).toContain(
				"Hardcoded pixel value at spacing.padding: 16px",
			);
		});
	});
	...
	describe("Specification Utilities", () => {
		...
		it("should merge specification variants", () => {
			...
			const mergeSpecs = (
				base: typeof baseSpec,
				variant: typeof variantSpec,
			) => {
				return {
					...base,
					...variant,
	...
	describe("Error Handling", () => {
		it("should handle malformed specification files", () => {
			...
			expect(() => loadSpec("/specs/invalid.json")).toThrow(
		...
		it("should handle missing specification files", () => {
			...
			expect(() => loadSpec("/specs/missing.json")).toThrow(
	...
	describe("Performance", () => {
		it("should handle large specification registries efficiently", () => {
			...
			mockFs.files.set(
				"/specs/large-registry.json",
				JSON.stringify(largeRegistry),
			);
			require("fs").readFileSync.mockImplementation(mockFs.readFileSync);

			const startTime = Date.now();
			const registry = JSON.parse(
				require("fs").readFileSync("/specs/large-registry.json", "utf8"),
			);
