...
export class ReportingEngine {
	...
	async generateArchitectureReport(analysisResult: any): Promise<string> {
		...
		const godComponents = components.filter(
	...
	async generateExecutiveSummary(analysisResult: any): Promise<string> {
		...
		const highComplexityCount = analysisResult.components.filter(
			(c: any) => c.complexity === "high",
		).length;
	...
	private generateHtmlReport(analysisResult: any): string {
		...
		const componentCount = analysisResult.components.length;
		const depCount = analysisResult.dependencies.length;
		const prodDeps = analysisResult.dependencies.filter(
			(d: any) => !d.isDev,
		).length;
		const devDeps = analysisResult.dependencies.filter(
			(d: any) => d.isDev,
		).length;
	...
	private processTemplate(template: string, data: any): string {
		...
		processed = processed.replace(
			/\{\{components\.length\}\}/g,
			String(data.components?.length || 0),
		);
		processed = processed.replace(
			/\{\{averageComplexity\}\}/g,
