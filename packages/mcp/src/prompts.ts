...
import { MCPArguments, MCPMessage, MCPPromptMap } from "./types/index.js";
...
export const promptHandlers = {
	"build-shadcn-page": (
		args: MCPArguments<(typeof prompts)["build-shadcn-page"]>,
	): { messages: MCPMessage[] } => {
		const {
			pageType,
			features = "",
		...
		const text =
			`Create a complete ${pageType} page using shadcn/ui v4 components and blocks.\n\n` +
			`REQUIREMENTS:\n` +
			`- Page Type: ${pageType}\n` +
	...
	"create-dashboard": (
		args: MCPArguments<(typeof prompts)["create-dashboard"]>,
	): { messages: MCPMessage[] } => {
		const { dashboardType, widgets = "", navigation = "sidebar" } = args;
		const framework = getFramework();

		const text =
			`Create a comprehensive ${dashboardType} dashboard using shadcn/ui v4 blocks and components.\n\n` +
			`REQUIREMENTS:\n` +
	...
	"create-auth-flow": (
		args: MCPArguments<(typeof prompts)["create-auth-flow"]>,
	): { messages: MCPMessage[] } => {
		const { authType, providers = "", features = "" } = args;

		const text =
			`Generate ${authType} authentication pages using shadcn/ui v4 login blocks.\n\n` +
			`REQUIREMENTS:\n` +
			`- Auth Type: ${authType}\n` +
	...
	"optimize-shadcn-component": (
		args: MCPArguments<(typeof prompts)["optimize-shadcn-component"]>,
	): { messages: MCPMessage[] } => {
		const { component, optimization = "", useCase = "" } = args;

		const text =
			`Optimize or enhance the ${component} shadcn/ui component with best practices.\n\n` +
			`REQUIREMENTS:\n` +
			`- Component: ${component}\n` +
	...
	"create-data-table": (
		args: MCPArguments<(typeof prompts)["create-data-table"]>,
	): { messages: MCPMessage[] } => {
		const { dataType, features = "", actions = "" } = args;

		const text =
			`Create advanced data table for ${dataType} using shadcn/ui components.\n\n` +
			`REQUIREMENTS:\n` +
			`- Data Type: ${dataType}\n` +
...
function getPageTypeSpecificInstructions(pageType: string): string {
	const instructions: Record<string, string> = {
		dashboard:
			"Include metrics cards, charts, and data visualization components.",
		login: "Focus on form validation, security, and user experience.",
		calendar:
			"Implement date navigation, event display, and interaction features.",
	...
	return (
		instructions[pageType] ||
		"Implement the requested functionality with best practices."
	);
}
...
function getOptimizationInstructions(optimization: string): string {
	const instructions: Record<string, string> = {
		performance:
			"Focus on rendering optimization, memoization, and bundle size reduction.",
		accessibility:
			"Enhance ARIA labels, keyboard navigation, and screen reader support.",
		responsive:
			"Improve mobile and tablet layouts with better breakpoint handling.",
		animations: "Add smooth transitions and micro-interactions for better UX.",
	};
	return (
		instructions[optimization] ||
		"Apply general best practices and code quality improvements."
	);
}
