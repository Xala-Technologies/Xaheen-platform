...
class TypeGenerator {
	...
	private async processSpecificationFile(
		filePath: string,
	): Promise<GeneratedTypeFile | null> {
		try {
 ...
 */`);
		...
		if (spec.compliance.norwegian.nsmClassification !== "OPEN") {
			...
			lines.push(
				`  readonly _compliance?: ${spec.metadata.name}ComplianceMetadata;`,
			);
		}
	 ...
	 */
	private generatePropTypeDefinition(
		propName: string,
	...
	private generatePropJSDoc(propDef: PropDefinition, indent: number): string {
		...
		if (propDef.defaultValue !== undefined) {
			lines.push(
				`${spaces} * @default ${JSON.stringify(propDef.defaultValue)}`,
			);
		}

		if (propDef.examples && propDef.examples.length > 0) {
			lines.push(
				`${spaces} * @example ${JSON.stringify(propDef.examples[0].value)}`,
			);
		}

		if (propDef.deprecated) {
			lines.push(
				`${spaces} * @deprecated since v${propDef.deprecated.since} - ${propDef.deprecated.reason}`,
			);
			lines.push(
				`${spaces} * @see Use ${propDef.deprecated.alternative} instead`,
			);
		}
	 ...
	 */
	private generatePrimitiveType(typeDef: {
		primitive: string;
		constraints?: any;
	}): string {
		...
		if (baseType === "string" && typeDef.constraints?.enum) {
			const values = typeDef.constraints.enum
				.map((v: string) => `'${v}'`)
				.join(" | ");
			return values;
		}
	 ...
	 */
	private generateComplexType(typeDef: {
		complex: string;
		signature?: any;
	 ...
	 */
	private generateCustomType(typeDef: {
		custom: string;
		values?: any[];
		pattern?: string;
	}): string {
	 ...
	 */
	private generateUnionType(typeDef: {
		union: any[];
		discriminant?: string;
	}): string {
		const types = typeDef.union.map((type) =>
			this.generateTypeScriptType(type),
		);
		return types.join(" | ");
	}
	 ...
	 */
	private generateArrayType(typeDef: {
		array: any;
		constraints?: any;
	}): string {
	 ...
	 */
	private generateObjectType(typeDef: {
	...
	private generateVariantTypes(spec: ComponentSpecification): string {
		...
		if (spec.variants.simple) {
			for (const [variantName, variant] of Object.entries(
				spec.variants.simple,
			)) {
				const values = Object.keys(variant.values)
					.map((v) => `'${v}'`)
					.join(" | ");
				lines.push(
					`export type ${spec.metadata.name}${variantName.charAt(0).toUpperCase() + variantName.slice(1)} = ${values};`,
				);
			}
		}
		...
		if (spec.variants.compound) {
			...
			spec.variants.compound.forEach((variant) => {
				Object.keys(variant.conditions).forEach((prop) =>
	...
	private generateEventHandlerTypes(spec: ComponentSpecification): string {
		...
		for (const [propName, propDef] of Object.entries(spec.props.schema)) {
			if (
				...
				propDef.type &&
				typeof propDef.type === "object" &&
				"complex" in propDef.type &&
 ...
 */`);
		...
		switch (platform) {
			case "react":
				lines.push(
					`export interface ${spec.metadata.name}ReactProps extends ${spec.metadata.name}Props {`,
				);
			...
			case "vue":
				lines.push(
					`export interface ${spec.metadata.name}VueProps extends ${spec.metadata.name}Props {`,
				);
			...
			case "angular":
				lines.push(
					`export interface ${spec.metadata.name}AngularProps extends ${spec.metadata.name}Props {`,
				);
	...
	private generateComponentUtilityTypes(spec: ComponentSpecification): string {
		...
		if (requiredProps.length > 0) {
			...
			lines.push(
				`export type ${spec.metadata.name}RequiredProps = ${requiredUnion};`,
			);
		}
		...
		if (optionalProps.length > 0) {
			...
			lines.push(
				`export type ${spec.metadata.name}OptionalProps = ${optionalUnion};`,
			);
		}
		...
		lines.push(
			`export type Partial${spec.metadata.name}Props = Partial<${spec.metadata.name}Props>;`,
		);
 ...
 */`);
		...
		lines.push("");
		lines.push("/**");
		lines.push(" * Component registry for type-safe component lookup");
		lines.push(" */");
