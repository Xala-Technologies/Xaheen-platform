		...
		if (headings.length === 0) {
			...
			for (const heading of headings) {
				if (heading.depth > lastDepth + 1) {
					warnings.push(
						`Heading hierarchy skip: H${lastDepth} to H${heading.depth}`,
					);
				}
...
async function validateCategoryIndexes(specifications, result) {
	...
	const categories = [
		...new Set(
			specifications.map((spec) => spec.metadata.category || "uncategorized"),
		),
	];
	...
	for (const category of categories) {
		...
		const categorySpecs = specifications.filter(
			(spec) => (spec.metadata.category || "uncategorized") === category,
		);
		...
		if (await fs.pathExists(indexPath)) {
			...
			for (const spec of categorySpecs) {
				if (!content.includes(spec.metadata.name)) {
					result.addWarning(
						indexPath,
						`Component ${spec.metadata.name} not listed in category index`,
					);
				}
			}
		}
...
async function validateCrossReferences(specifications, result) {
	...
	for (const docFile of docFiles) {
		...
		try {
			...
			result.addError(
				filePath,
				`Error reading file for cross-reference validation: ${error.message}`,
			);
		}
	}
}
...
function generateReport(result, options = {}) {
	...
	const coveragePercent =
		result.coverage.totalComponents === 0
			? 0
			: Math.round(
					(result.coverage.components / result.coverage.totalComponents) * 100,
				);
	...
	console.log(
		`Coverage: ${coveragePercent}% (${result.coverage.components}/${result.coverage.totalComponents} components)`,
	);
	...
	if (result.errors.length > 0) {
		...
		result.errors.forEach((error, index) => {
			console.log(
				`${index + 1}. ${path.relative(DOCS_DIR, error.file)}: ${error.message}`,
			);
		});
	}
	...
	if (result.warningMessages.length > 0 && options.showWarnings) {
		...
		result.warningMessages.forEach((warning, index) => {
			console.log(
				`${index + 1}. ${path.relative(DOCS_DIR, warning.file)}: ${warning.message}`,
			);
		});
	}
	...
	console.log("\n📈 Coverage Details:");
	console.log(
		`- Component Overviews: ${result.coverage.documentation.overview}`,
	);
...
async function validateDocs(options = {}) {
	...
	try {
		...
		if (specifications.length === 0) {
			console.warn(
				"⚠️  No specifications found. Cannot validate documentation.",
			);
			return result;
		}
