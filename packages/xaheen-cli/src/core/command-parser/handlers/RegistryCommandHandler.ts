...
export class RegistryCommandHandler implements ICommandHandler {
  public readonly domain = 'registry';
  ...
  public getSupportedActions(): string[] {
    return ['add', 'list', 'info', 'search', 'build', 'serve'];
  }
  ...
  public async execute(command: CLICommand): Promise<void> {
    const { action, arguments: args, options } = command;

    switch (action) {
      case 'add':
        const components = Array.isArray(args.components) ? args.components : [args.components].filter(Boolean);
      ...
      case 'list':
        ...
        break;
      case 'info':
        await this.handleInfo(args.component, options);
        break;
      case 'search':
        await this.handleSearch(args.query, options);
        break;
      case 'build':
      ...
      default:
        throw new CLIError(
          `Unsupported registry action: ${action}`,
          'UNSUPPORTED_ACTION',
          this.domain,
          action
        );
  ...
  public async handleList(options: any): Promise<void> {
    ...
    try {
      ...
      // Display items
      console.log(chalk.blue('📦 Available Registry Items:'));
  ...
  public async handleInfo(component: string, options: any): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.blue(`📦 ${item.title || item.name}`));
      ...
      if (item.nsm) {
        console.log(chalk.yellow('Norwegian Compliance:'));
      ...
      if (item.registryDependencies && item.registryDependencies.length > 0) {
        console.log(chalk.yellow('Registry Dependencies:'));
      ...
      console.log(chalk.gray(`To install: xaheen registry add ${component}`));
  ...
  public async handleBuild(options: any): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.gray('Registry files generated in dist/registry and public/r'));
  ...
  public async handleServe(options: any): Promise<void> {
    ...
    try {
      ...
      app.listen(port, host, () => {
        console.log(chalk.green(`✨ Registry server running at http://${host}:${port}`));
