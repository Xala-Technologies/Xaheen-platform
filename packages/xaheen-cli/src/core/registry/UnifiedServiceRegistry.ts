import type { ServiceTemplate, ComponentTemplate, TemplateFile, AppTemplate, MonorepoTarget, PlatformType, MonorepoStructure } from '../../types/index.js';
...
export class UnifiedServiceRegistry {
  ...
  constructor(config: RegistryConfig = {}) {
    this.config = {
      xaheenTemplatesPath: config.xaheenTemplatesPath || '../xaheen-cli/src/templates',
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.success(`Registry initialized with ${this.serviceTemplates.size} service templates, ${this.componentTemplates.size} component templates, and ${this.appTemplates.size} app templates`);
    } catch (error) {
  ...
  private async loadXaheenServiceTemplates(): Promise<void> {
    const templatesPath = this.resolveTemplatePath(this.config.xaheenTemplatesPath!);
  ...
  private async loadServiceCategory(basePath: string, category: string): Promise<void> {
    ...
    try {
      ...
      const serviceTemplate: ServiceTemplate = {
        ...
        description: templateDef.description || `${this.formatName(category)} service template`,
      ...
      logger.warn(`Failed to load service template from ${categoryPath}:`, error);
    }
  }

  private async loadXalaComponentTemplates(): Promise<void> {
    const templatesPath = this.resolveTemplatePath(this.config.xalaTemplatesPath!);
  ...
  private async loadComponentPlatform(basePath: string, platform: string): Promise<void> {
  ...
  private async loadComponentTemplate(basePath: string, platform: string, componentFile: string): Promise<void> {
    ...
    try {
      ...
      const componentTemplate: ComponentTemplate = {
        ...
        description: metadata.description || `${this.formatName(componentName)} component for ${platform}`,
        platform,
      ...
      logger.debug(`Loaded component template: ${componentTemplate.name} (${platform})`);
      
    } catch (error) {
      logger.warn(`Failed to load component template from ${componentPath}:`, error);
    }
  }

  private async loadCustomTemplates(): Promise<void> {
    const templatesPath = this.resolveTemplatePath(this.config.customTemplatesPath!);
  ...
  private async loadMonorepoAppTemplates(): Promise<void> {
    ...
    const appTemplates: AppTemplate[] = [
      // Web Applications
      {
        ...
        framework: 'nextjs',
        targetPath: 'apps',
        dependencies: ['next', 'react', 'react-dom', '@xala-technologies/ui-system'],
      ...
      },
      {
        ...
        framework: 'react',
        targetPath: 'apps',
        dependencies: ['vite', 'react', 'react-dom', '@xala-technologies/ui-system'],
      ...
      // Desktop Applications
      {
        ...
        framework: 'electron',
        targetPath: 'apps',
        dependencies: ['electron', 'react', 'react-dom', '@xala-technologies/ui-system'],
      ...
      },
      {
        ...
        framework: 'tauri',
        targetPath: 'apps',
        dependencies: ['@tauri-apps/api', 'react', 'react-dom', '@xala-technologies/ui-system'],
      ...
      },
      {
        ...
        framework: 'expo',
        targetPath: 'apps',
        dependencies: ['expo', 'react-native', 'react', '@xala-technologies/ui-system'],
    ...
    appTemplates.forEach(template => {
      ...
      logger.debug(`Loaded app template: ${template.name} (${template.platform})`);
    });
  }
    ...
    for (const line of lines) {
      ...
      if (inMetadata) {
        if (trimmed.startsWith('@name')) {
          ...
          const propMatch = trimmed.match(/@prop\s+\{(\w+)\}\s+(\w+)\s*-?\s*(.*)/);
          if (propMatch) {
            metadata.props.push({
              name: propMatch[2],
              type: propMatch[1],
...
export default function Home() {
  ...
  private getTauriAppFiles(): TemplateFile[] { return []; }
  private getReactNativeAppFiles(): TemplateFile[] { return []; }
  private getExpoAppFiles(): TemplateFile[] { return []; }
  private getNestJSAppFiles(): TemplateFile[] { return []; }
  private getFastifyAppFiles(): TemplateFile[] { return []; }
  ...
  public getServiceTemplatesByCategory(category: string): ServiceTemplate[] {
    return this.getAllServiceTemplates().filter(template => template.category === category);
  }

  public getComponentTemplatesByPlatform(platform: string): ComponentTemplate[] {
    return this.getAllComponentTemplates().filter(template => template.platform === platform);
  }

  public getAppTemplatesByPlatform(platform: PlatformType): AppTemplate[] {
    return this.getAllAppTemplates().filter(template => template.platform === platform);
  }

  public getAvailablePlatforms(): PlatformType[] {
    const platforms = new Set<PlatformType>();
    this.getAllAppTemplates().forEach(template => platforms.add(template.platform));
  ...
  public searchTemplates(query: string): { 
    ...
    const services = this.getAllServiceTemplates().filter(template => 
      template.name.toLowerCase().includes(lowerQuery) ||
      template.description.toLowerCase().includes(lowerQuery) ||
      template.category.toLowerCase().includes(lowerQuery)
    );
    
    const components = this.getAllComponentTemplates().filter(template =>
      template.name.toLowerCase().includes(lowerQuery) ||
      template.description.toLowerCase().includes(lowerQuery) ||
      template.category.toLowerCase().includes(lowerQuery)
    );

    const apps = this.getAllAppTemplates().filter(template =>
      template.name.toLowerCase().includes(lowerQuery) ||
      template.description.toLowerCase().includes(lowerQuery) ||
  ...
  public getRegistryStats(): { 
    ...
    const platforms = [...new Set(this.getAllAppTemplates().map(t => t.platform))];
    const categories = [...new Set([
      ...this.getAllServiceTemplates().map(t => t.category),
      ...this.getAllComponentTemplates().map(t => t.category)
    ])];
    const frameworks = [...new Set(this.getAllAppTemplates().map(t => t.framework))];
