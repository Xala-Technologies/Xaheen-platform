...
import type { XaheenConfig, LegacyXaheenConfig, XalaConfig, MigrationResult } from '../../types/index.js';
...
export class ConfigManager {
  ...
  public async loadConfig(): Promise<XaheenConfig> {
    ...
    if (migrationResult.success) {
      logger.info(`Successfully migrated from ${migrationResult.source} to CLI config`);
   ...
   */
  public async updateConfig(updates: Partial<XaheenConfig>): Promise<XaheenConfig> {
   ...
   */
  public async updateDesignConfig(designConfig: Partial<XaheenConfig['design']>): Promise<void> {
    const currentConfig = await this.loadConfig();
   ...
   */
  public async updateAIConfig(aiConfig: Partial<XaheenConfig['ai']>): Promise<void> {
    const currentConfig = await this.loadConfig();
  ...
  private async migrateFromXaheen(): Promise<MigrationResult> {
    try {
      ...
      const xaheenConfig: LegacyXaheenConfig = await fs.readJson(this.configPaths.xaheen);
  ...
  private mergeConfigs(base: XaheenConfig, updates: Partial<XaheenConfig>): XaheenConfig {
    return {
      ...base,
      ...updates,
