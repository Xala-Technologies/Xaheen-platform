...
describe("Project Creation Integration", () => {
  ...
  beforeEach(async () => {
    ...
    const result = tmp.dirSync({ prefix: "xaheen-integration-test-", unsafeCleanup: true });
    testDir = result.name;
  ...
  return (
    <main>
      ...
      await testUtils.assert.assertFileExists(path.join(projectPath, "package.json"));
      await testUtils.assert.assertFileExists(path.join(projectPath, "src", "app", "layout.tsx"));
      await testUtils.assert.assertFileExists(path.join(projectPath, "src", "app", "page.tsx"));
      
      // Verify package.json content
      await testUtils.assert.assertValidPackageJson(path.join(projectPath, "package.json"));
...
`
      ...
      await testUtils.assert.assertDirectoryExists(projectPath);
      await testUtils.assert.assertFileExists(path.join(projectPath, "package.json"));
      await testUtils.assert.assertFileExists(path.join(projectPath, "tsconfig.json"));
      await testUtils.assert.assertFileExists(path.join(projectPath, "src", "index.ts"));
  ...
  describe("Service Integration", () => {
    it("should add authentication service to existing project", async () => {
      ...
      // Create base project
      await testUtils.fixtures.createFixtureProject("simple-project", projectPath);
...
`
      ...
      await testUtils.assert.assertFileExists(path.join(projectPath, "src", "services", "auth.ts"));
      await testUtils.assert.assertFileExists(path.join(projectPath, "src", "middleware", "auth.ts"));
      ...
      const updatedPackageJson = await fs.readJson(path.join(projectPath, "package.json"));
...
`
      ...
      await testUtils.assert.assertFileExists(path.join(projectPath, "prisma", "schema.prisma"));
      await testUtils.assert.assertFileExists(path.join(projectPath, "src", "services", "database.ts"));
      await testUtils.assert.assertFileExists(path.join(projectPath, ".env.example"));
...
`
      ...
      await fs.ensureDir(path.join(projectPath, "src", "components", "__tests__"));
      await fs.writeFile(
        path.join(projectPath, "src", "components", "__tests__", `${componentName}.test.tsx`),
...
`
      ...
      await fs.writeFile(
        path.join(projectPath, "src", "stories", `${componentName}.stories.tsx`),
...
`
  ...
  describe("Idempotency", () => {
    ...
    it("should handle adding the same service multiple times", async () => {
      ...
      const authServicePath = path.join(projectPath, "src", "services", "auth.ts");
      await fs.ensureDir(path.join(projectPath, "src", "services"));
  ...
  describe("Error Handling", () => {
    ...
    it("should handle disk space issues gracefully", async () => {
      ...
      try {
        await fs.ensureDir(projectPath);
        await fs.writeFile(path.join(projectPath, "large-file.txt"), largeContent);
        
        // If successful, verify the file was created
        await testUtils.assert.assertFileExists(path.join(projectPath, "large-file.txt"));
        const content = await fs.readFile(path.join(projectPath, "large-file.txt"), "utf8");
