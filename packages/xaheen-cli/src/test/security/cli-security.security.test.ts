...
describe("CLI Security Tests", () => {
  ...
  beforeEach(async () => {
    ...
    const result = tmp.dirSync({ prefix: "xaheen-security-test-", unsafeCleanup: true });
    testDir = result.name;
...
`);
      ...
      console.log("Found real secrets in example file (security violation):", foundRealSecrets);
    });
  });
  ...
  describe("Dependency Security", () => {
    ...
    it("should validate package sources", async () => {
      ...
      if (suspiciousDependencies.length > 0) {
        console.log("Suspicious dependency sources found:", suspiciousDependencies);
      }
  ...
  describe("Command Injection Prevention", () => {
    it("should prevent command injection in system calls", async () => {
      ...
      for (const maliciousCommand of maliciousCommands) {
        const result = await execa("node", [
          cliPath,
          "make:component", maliciousCommand
        ], {
          cwd: projectPath,
          env: { XAHEEN_NO_BANNER: "true" },
          timeout: 10000,
    ...
    it("should sanitize template variables", async () => {
      ...
      if (result.exitCode === 0) {
        ...
        const componentPath = path.join(projectPath, "src", "components", "TestComponent.tsx");
  ...
  describe("Compliance Testing", () => {
    it("should generate GDPR-compliant code structures", async () => {
      ...
      if (await fs.pathExists(projectPath)) {
        ...
        console.log(`GDPR compliance files found: ${gdprFilesFound}/${gdprFiles.length}`);
      }
    });
    ...
    it("should check Norwegian NSM compliance features", async () => {
      ...
      if (await fs.pathExists(projectPath)) {
        ...
        if (await fs.pathExists(packageJsonPath)) {
          ...
          console.log(`Security packages found: ${securityPackagesFound}/${securityPackages.length}`);
    ...
    it("should validate accessibility compliance", async () => {
      ...
      await execa("node", [
        cliPath,
        "make:component", "AccessibleButton",
        "--accessibility", "AAA"
      ], {
        cwd: projectPath,
        env: { XAHEEN_NO_BANNER: "true" },
      ...
      const componentPath = path.join(projectPath, "src", "components", "AccessibleButton.tsx");
      ...
      if (await fs.pathExists(componentPath)) {
        ...
        console.log(`Accessibility features found: ${accessibilityFeaturesFound}/${accessibilityFeatures.length}`);
  ...
  describe("Security Audit Integration", () => {
    it("should run npm audit equivalent checks", async () => {
      ...
      const auditResult = await execa("node", [
        cliPath,
        "security-audit",
        "--tools", "npm-audit",
        "--severity", "high"
    ...
    it("should generate compliance reports", async () => {
      ...
      const reportResult = await execa("node", [
        cliPath,
        "compliance-report",
