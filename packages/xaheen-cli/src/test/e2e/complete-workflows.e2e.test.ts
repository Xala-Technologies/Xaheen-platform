...
describe("Complete CLI Workflows E2E", () => {
  ...
  beforeEach(async () => {
    ...
    const result = tmp.dirSync({ prefix: "xaheen-e2e-test-", unsafeCleanup: true });
    testDir = result.name;
  ...
  describe("New → Generate → Add → AI → Validate Workflow", () => {
    it("should complete full project lifecycle", async () => {
      ...
      expect(generateModelResult.stdout).toContain("Model generated successfully");
      
      // Verify model files were created
      await testUtils.assert.assertFileExists(path.join(projectPath, "src", "models", "User.ts"));
      await testUtils.assert.assertFileExists(path.join(projectPath, "src", "models", "__tests__", "User.test.ts"));
      ...
      const aiGenerateResult = await runner.runCommand([
        "ai", "generate", "Create a responsive user profile card with avatar, name, email, and edit button"
      ], {
        cwd: projectPath,
        timeout: 45000,
  ...
  describe("Error Handling and Recovery", () => {
    ...
    it("should handle interrupted operations", async () => {
      ...
      const createPromise = runner.runCommand([
        "project", "create", projectName,
        "--preset", "saas-enterprise" // Larger preset that takes longer
      ], {
        cwd: testDir,
  ...
  describe("Performance and Scalability", () => {
    it("should handle large project generation efficiently", async () => {
      ...
      const createResult = await runner.runCommand([
        ...
        "--features", "auth,database,payments,notifications,analytics"
      ], {
        cwd: testDir,
      ...
      const packageJson = await fs.readJson(path.join(projectPath, "package.json"));
      expect(Object.keys(packageJson.dependencies || {}).length).toBeGreaterThan(10);
    }, 150000);
    ...
    it("should handle parallel command execution", async () => {
      ...
      const promises = Array.from({ length: 3 }, (_, i) => 
        runner.runCommand([
          "project", "create", `${projectBaseName}-${i}`,
          "--preset", "simple"
        ], {
          cwd: testDir,
          timeout: 60000,
