	...
	describe("Startup Performance", () => {
      ...
      `);
		...
		it("should measure warm start performance", async () => {
			...
			const avgWarmStart =
				warmStartTimes.reduce((a, b) => a + b, 0) / warmStartTimes.length;
      ...
      `);
		...
		it("should compare command parsing overhead", async () => {
			...
			for (const command of commands) {
				...
				for (let i = 0; i < 3; i++) {
					const endMeasure = perf.startMeasurement(
						`command-${commandKey}-${i}`,
					);
	...
	describe("Command Execution Performance", () => {
		...
		it("should benchmark component generation", async () => {
			...
			for (const { name, type } of componentTypes) {
				...
				const result = await execa(
					"node",
					[cliPath, "make:component", name, "--test", "--stories"],
					{
						cwd: projectPath,
						env: { XAHEEN_NO_BANNER: "true" },
						timeout: 20000,
      ...
      `);
		...
		it("should maintain performance under load", async () => {
			...
			for (let i = 0; i < componentCount; i++) {
				const promise = execa(
					"node",
					[cliPath, "make:component", `LoadTestComponent${i}`, "--test"],
					{
						cwd: projectPath,
						env: { XAHEEN_NO_BANNER: "true" },
						timeout: 30000,
