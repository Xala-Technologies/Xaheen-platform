...
export class NSMClassifier {
	private classifications: Map<NSMClassification, NSMClassificationMetadata> =
		new Map();
	...
	getClassification(
		level: NSMClassification,
	): NSMClassificationMetadata | null {
	...
	private hasConfidentialIndicators(
		...
		return (
			context.dataTypes?.some((type) =>
				confidentialKeywords.some((keyword) =>
	...
	private initializeClassifications(): void {
		...
		this.classifications.set("RESTRICTED", {
			level: "RESTRICTED",
			description:
				"Information for authorized personnel within the organization",
			requirements: [
		...
		this.classifications.set("CONFIDENTIAL", {
			level: "CONFIDENTIAL",
			description:
				"Sensitive information requiring special handling and protection",
			requirements: [
