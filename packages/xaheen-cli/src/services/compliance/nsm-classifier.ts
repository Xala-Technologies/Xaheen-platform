...
export class NSMClassifier {
  private classifications: Map<NSMClassification, NSMClassificationMetadata> = new Map();
   ...
   */
  determineClassification(context: Partial<ComplianceContext>): NSMClassification {
   ...
   */
  validateAccess(userClearance: NSMClassification, requiredLevel: NSMClassification): boolean {
  ...
  getSecurityRequirements(classification: NSMClassification): {
    ...
    return {
      templateSecurity: [
        ...
        ...(metadata.dataHandling.encryption ? ['Add data encryption patterns'] : []),
        ...(metadata.uiRequirements.watermarks ? ['Add security watermarks'] : [])
      ],
      runtimeSecurity: [
        'Implement session timeout management',
        'Add user authentication validation',
        'Include authorization pattern checks',
        ...(metadata.dataHandling.accessControl ? ['Add role-based access control'] : []),
        ...(metadata.uiRequirements.sessionTimeout < 60 ? ['Add enhanced session monitoring'] : [])
      ],
      dataHandling: [
        'Implement proper data sanitization',
        'Add input validation patterns',
        'Include output encoding',
        ...(metadata.dataHandling.encryption ? ['Add data encryption at rest and in transit'] : []),
        ...(metadata.dataHandling.anonymization ? ['Add data anonymization patterns'] : [])
      ],
      auditRequirements: [
        'Add comprehensive audit logging',
        'Include user action tracking',
        'Implement change audit trails',
        ...(metadata.auditLevel === 'maximum' ? ['Add real-time security monitoring'] : []),
        ...(metadata.dataHandling.auditTrail ? ['Add detailed data access logging'] : [])
      ]
    };
  }
   ...
   */
  generateComplianceContext(classification: NSMClassification): Record<string, any> {
    const metadata = this.getClassification(classification);
    if (!metadata) {
      throw new Error(`Unknown classification: ${classification}`);
    }
   ...
   */
  private generateNSMHeaders(classification: NSMClassification): Record<string, string> {
   ...
   */
  private getGDPRLawfulBasis(classification: NSMClassification): string {
   ...
   */
  private hasSecretIndicators(context: Partial<ComplianceContext>): boolean {
    const secretKeywords = [
      'national-security', 'military', 'intelligence', 'classified',
      'state-secret', 'defense', 'counterintelligence'
    ];
    
    return context.dataTypes?.some(type => 
      secretKeywords.some(keyword => type.toLowerCase().includes(keyword))
    ) || false;
  }
   ...
   */
  private hasConfidentialIndicators(context: Partial<ComplianceContext>): boolean {
    const confidentialKeywords = [
      'personal-data', 'medical', 'financial', 'biometric',
      'criminal-record', 'government', 'sensitive'
    ];
    
    return context.dataTypes?.some(type => 
      confidentialKeywords.some(keyword => type.toLowerCase().includes(keyword))
    ) || (context.retentionPeriod && context.retentionPeriod > 2555) || false; // > 7 years
  }

  /**
   * Check for RESTRICTED level indicators
   */
  private hasRestrictedIndicators(context: Partial<ComplianceContext>): boolean {
    const restrictedKeywords = [
      'internal', 'employee', 'business', 'proprietary',
      'organizational', 'administrative'
    ];
    
    return context.dataTypes?.some(type => 
      restrictedKeywords.some(keyword => type.toLowerCase().includes(keyword))
    ) || context.internationalTransfer || false;
  }
  ...
  private initializeClassifications(): void {
    ...
    this.classifications.set('CONFIDENTIAL', {
      ...
      uiRequirements: {
        watermarks: true,
        classificationLabels: true,
        restrictedActions: ['export', 'print', 'screenshot', 'copy', 'download'],
    ...
    this.classifications.set('SECRET', {
      ...
      uiRequirements: {
        watermarks: true,
        classificationLabels: true,
        restrictedActions: ['export', 'print', 'screenshot', 'copy', 'download', 'share'],
