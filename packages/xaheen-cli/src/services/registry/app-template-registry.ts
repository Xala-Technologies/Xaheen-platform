...
export class AppTemplateRegistry {
   ...
   */
  private async loadAppTemplate(framework: string): Promise<AppTemplate | null> {
  ...
  private async scanTemplateDirectory(
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const finalPath = isTemplate ? relativeEntryPath.replace(/\.hbs$/, '') : relativeEntryPath;
    ...
    for (const file of template.files) {
      ...
      if (file.isTemplate) {
        ...
        try {
          ...
          let subdir: string;
          if (template.framework === 'electron' || template.framework === 'react-native') {
            subdir = 'platforms';
          } else {
            subdir = 'frontend';
          }
          
          const relativePath = path.join(subdir, template.framework, file.path + '.hbs');
          content = await templateLoader.renderTemplate(relativePath, variables);
        } catch (error) {
          logger.warn(`Failed to render template ${file.path}, using simple replacement:`, error);
   ...
   */
  private simpleTemplateReplace(content: string, variables: Record<string, any>): string {
    let result = content;
   ...
   */
  private getPlatformFromFramework(framework: string): 'web' | 'desktop' | 'mobile' {
    switch (framework) {
      case 'electron':
        return 'desktop';
      case 'react-native':
  ...
  async listAvailableTemplates(): Promise<string[]> {
    try {
      ...
      if (await fs.pathExists(frontendPath)) {
        const frontendEntries = await fs.readdir(frontendPath, { withFileTypes: true });
        templates.push(...frontendEntries
      ...
      if (await fs.pathExists(platformsPath)) {
        const platformEntries = await fs.readdir(platformsPath, { withFileTypes: true });
        templates.push(...platformEntries
          .filter(entry => entry.isDirectory())
          .map(entry => entry.name));
      }
