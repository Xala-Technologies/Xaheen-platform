...
describe("ServiceRemover", () => {
					...
					return Promise.resolve([
						{
							...
							envVariables: [{ name: "AUTH_SECRET", required: true }],
						},
					]);
				}
				return Promise.resolve([]);
	...
	describe("executeRemoval", () => {
		it("should execute removal plan successfully", async () => {
			...
			const result = await remover.executeRemoval(plan, mockProject.path, {
				backup: true,
				cleanup: false,
			});
		...
		it("should generate post-removal steps", async () => {
			...
			const plan = {
				...
				filesToModify: [
					{ path: "src/app/layout.tsx", changes: ["Remove test"] },
				],
				dependenciesToRemove: ["test-package"],
			...
			expect(result.postRemovalSteps).toContain(
				"Run `npm install` to clean up removed dependencies",
			);
			expect(result.postRemovalSteps).toContain(
				"Review modified files for any remaining service references",
			);
			expect(result.postRemovalSteps).toContain(
				"Update .env files to remove unused environment variables",
			);
		});
