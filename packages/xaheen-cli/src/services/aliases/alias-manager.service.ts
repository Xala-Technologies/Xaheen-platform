...
export class AliasManagerService {
	...
	private initializeAliases(): void {
		const aliasDefinitions: CommandAlias[] = [
			...
			// Rails-inspired generator shortcuts (xaheen g)
			{
				alias: "g",
			...
			{
				alias: "g:component",
				originalCommand: "component generate",
				domain: "component",
				action: "generate",
			...
			{
				alias: "g:page",
				originalCommand: "page generate",
				domain: "page",
				action: "generate",
				description: "Generate full page with routing and layout",
				example: "xaheen g:page Dashboard --layout admin",
			},
			{
				alias: "g:layout",
				originalCommand: "make:component",
				domain: "make",
				action: "component",
				description: "Generate layout component",
				example: "xaheen g:layout AdminLayout --type layout",
			},
			{
				alias: "g:model",
				originalCommand: "model generate",
				domain: "model",
				action: "generate",
				description: "Generate data model with TypeScript types",
				example: "xaheen g:model User --fields name:string,email:email",
			},
			{
				alias: "g:migration",
				originalCommand: "make:migration",
				...
				example: "xaheen g:migration create_users_table",
			},
			{
				alias: "g:seed",
				originalCommand: "make:seeder",
				domain: "make",
				action: "seeder",
				description: "Generate database seed file",
				example: "xaheen g:seed UserSeeder",
			},
			{
				alias: "g:test",
				originalCommand: "generate",
				domain: "component",
				action: "generate",
				description: "Generate test files for existing code",
				example: "xaheen g:test UserService --type unit",
			},
			{
				alias: "g:scaffold",
				...
				domain: "make",
				action: "crud",
				description: "Generate complete CRUD scaffold",
				example: "xaheen g:scaffold Post --fields title:string,content:text",
			},
			{
				alias: "scaffold",
				originalCommand: "make:crud",
	...
	private initializeShortcuts(): void {
		const shortcutDefinitions: ShortcutMapping[] = [
			...
			{
				shortcut: "-m",
				fullCommand: "--migration",
				description: "Create migration",
			},
			{
				shortcut: "-c",
				fullCommand: "--controller",
				description: "Create controller",
			},
			{
				shortcut: "-r",
				fullCommand: "--resource",
			...
			{ shortcut: "-s", fullCommand: "--seeder", description: "Create seeder" },
			{
				shortcut: "-a",
				fullCommand: "--all",
				description: "Create all related files",
			},
			...
			},
			{
				shortcut: "-n",
				fullCommand: "--no",
				description: "Auto-decline prompts",
			},

			// AI shortcuts
			{
				shortcut: "--ai",
				fullCommand: "--ai --description",
				description: "AI generation with description prompt",
			},

			// Platform shortcuts
			{
				shortcut: "--react",
				fullCommand: "--platform react",
				description: "Target React platform",
			},
			{
				shortcut: "--vue",
				fullCommand: "--platform vue",
				description: "Target Vue platform",
			},
			{
				shortcut: "--angular",
				fullCommand: "--platform angular",
				description: "Target Angular platform",
			},
			{
				shortcut: "--next",
				fullCommand: "--platform nextjs",
				description: "Target Next.js platform",
			},

			// Quality shortcuts
			{
				shortcut: "--aaa",
				fullCommand: "--accessibility AAA",
				description: "WCAG AAA accessibility",
			},
			{
				shortcut: "--no",
				fullCommand: "--norwegian",
				description: "Norwegian compliance",
			},
			{
				shortcut: "--gdpr",
				fullCommand: "--gdpr",
	...
	public resolveAlias(
		aliasCommand: string,
	): { domain: CLIDomain; action: CLIAction; originalCommand: string } | null {
	...
	public parseAliasedCommand(input: string[]): CLICommand | null {
		...
		const target = expandedArgs.find(
	...
	public registerAlias(alias: CommandAlias): void {
		this.aliases.set(alias.alias, alias);
		logger.debug(
			`Registered new alias: ${alias.alias} -> ${alias.originalCommand}`,
		);
	}
	...
	public getAliasHelp(): string {
		...
		const categories = {
			...
			"Other Commands": aliases.filter(
				(a) => !["make", "project", "mcp"].includes(a.domain),
			),
		};
		...
		help +=
			"  • Use shortcuts for common flags: xaheen new app --react --aaa\n";
