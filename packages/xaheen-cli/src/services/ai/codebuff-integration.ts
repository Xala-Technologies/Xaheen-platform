...
export class CodebuffIntegrationService {
   ...
   */
  async generateCode(prompt: string, options: Partial<CodebuffOptions> = {}): Promise<CodebuffResult> {
    ...
    try {
      ...
      if (config.verbose) {
        console.log(chalk.gray(`📍 Context: ${context.framework} (${context.language})`));
      ...
      if (!patch || patch.trim().length === 0) {
        return {
          ...
          filesAffected: [],
          confidence: 0,
          reasoning: 'No changes generated - prompt may not require code modifications',
   ...
   */
  async generateNorwegianCompliantCode(prompt: string, options: Partial<CodebuffOptions> = {}): Promise<CodebuffResult> {
...
- Implement secure session management with Norwegian standards`;
   ...
   */
  async fixFailingTests(options: Partial<CodebuffOptions> = {}): Promise<CodebuffResult> {
    try {
...
5. Add appropriate documentation and comments`;
  ...
  async ensureIndex(): Promise<boolean> {
    try {
      ...
      console.warn(chalk.yellow('⚠️ Failed to create Codebuff index, continuing without it'));
      return false;
    }
  }
  ...
  private async getProjectContext(): Promise<ProjectContext> {
    try {
      ...
      if (existsSync(packageJsonPath)) {
        ...
        // Detect language
        if (dependencies.includes('typescript') || existsSync(join(this.cwd, 'tsconfig.json'))) {
          language = 'typescript';
        }
      }

      const hasTests = existsSync(join(this.cwd, 'test')) || 
                      existsSync(join(this.cwd, '__tests__')) ||
                      dependencies.some(dep => dep.includes('test') || dep.includes('jest') || dep.includes('vitest'));

      const hasDocumentation = existsSync(join(this.cwd, 'README.md')) ||
   ...
   */
  private enhancePromptWithContext(prompt: string, context: ProjectContext): string {
    return `${prompt}
...
5. ${context.hasDocumentation ? 'Updates relevant documentation' : 'Includes inline documentation'}`;
   ...
   */
  private analyzePatch(patch: string): { filesAffected: string[]; confidence: number; reasoning: string } {
    ...
    const reasoning = `Modified ${filesAffected.length} file(s), added ${addedLines} lines, removed ${removedLines} lines. ${
      confidence > 0.8 ? 'High confidence - focused, small changes.' :
      confidence > 0.6 ? 'Medium confidence - moderate changes.' :
  ...
  private runTests(): string {
    try {
      ...
      return error.stdout?.toString() || error.stderr?.toString() || 'Test execution failed';
    }
  }
}
