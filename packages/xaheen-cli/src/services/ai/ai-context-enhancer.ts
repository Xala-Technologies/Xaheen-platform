...
export class AIContextEnhancer {
	...
	enhanceTemplateContext(
		...
		const semanticHints = this.generateSemanticHints(
			templateName,
			templateType,
			requirements,
		);
	...
	private calculateComplexityMetrics(requirements: any): ComplexityMetrics {
		...
		const accessibilityMultiplier =
			requirements.accessibility === "AAA" ? 1.4 : 1.2;
	 ...
	 */
	private buildGenerationGuidance(
		templateType: string,
		requirements: any,
	): GenerationGuidance {
