...
export class AccessibilityValidator {
     ...
     */
    async validateNorwegianCompliance(context) {
        const violations = [];
        const warnings = [];
     ...
     */
    async validateNSMCompliance(context) {
        const violations = [];
        const warnings = [];
     ...
     */
    generateRecommendations(violations, warnings, context) {
        const recommendations = [];
     ...
     */
    calculateAccessibilityScore(violations, warnings, targetLevel) {
        ...
        // Weighted scoring
        const totalDeductions = (criticalCount * 25) +
            (seriousCount * 15) +
            (moderateCount * 8) +
            (minorCount * 3) +
            (warningCount * 1);
        ...
        this.nsmRules.set('RESTRICTED', [
            {
                ...
                validate: (content) => content.includes('watermark') || content.includes('classification')
            }
        ]);
        ...
        this.nsmRules.set('CONFIDENTIAL', [
            {
                ...
                validate: (content) => content.includes('CONFIDENTIAL') && content.includes('watermark')
            }
        ]);
        ...
        this.nsmRules.set('SECRET', [
            {
                ...
                validate: (content) => content.includes('SECRET') &&
