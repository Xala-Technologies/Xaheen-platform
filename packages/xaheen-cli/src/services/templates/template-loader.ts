...
export class TemplateLoader {
  ...
  private async enhanceContext(context: any): Promise<any> {
    return {
      ...
      i18n: {
        ...
        functions: {
          t: (key: string, options?: any) => `{t('${key}'${options ? `, ${JSON.stringify(options)}` : ''})}`,
          plural: (key: string, count: number) => `{t('${key}', { count: ${count} })}`,
          interpolate: (key: string, variables: Record<string, any>) => 
            `{t('${key}', ${JSON.stringify(variables)})}`,
          formatCurrency: (amount: number, currency = 'NOK') => 
            `{new Intl.NumberFormat('nb-NO', { style: 'currency', currency: '${currency}' }).format(${amount})}`,
          formatDate: (date: string) => 
            `{new Date('${date}').toLocaleDateString('nb-NO')}`,
          formatNumber: (number: number) => 
            `{new Intl.NumberFormat('nb-NO').format(${number})}`
        }
      },
  ...
  async validateAccessibility(
    ...
    try {
      ...
      const validationContext: TemplateAccessibilityContext = {
        templateContent,
  ...
  private registerHelpers(): void {
    ...
    Handlebars.registerHelper('containerSize', (context) => {
      ...
      if (context.type === 'dashboard') return 'xl';
      if (context.type === 'form') return 'md';
      if (context.type === 'landing') return 'full';
      return 'lg';
    });
    
    Handlebars.registerHelper('stackGap', (context) => {
      // Smart gap sizing for Stack components
      if (context.density === 'compact') return 'sm';
      if (context.density === 'spacious') return 'xl';
      return 'lg';
    });
    ...
    Handlebars.registerHelper('buttonVariant', (context) => {
      ...
      if (context.primary) return 'primary';
      if (context.destructive) return 'destructive';
      if (context.subtle) return 'ghost';
      return 'outline';
    });
    ...
    Handlebars.registerHelper('gridColumns', (count, responsive = true) => {
      if (responsive) {
        ...
        if (count <= 4) return '{ base: 1, sm: 2, md: 3, lg: 4 }';
    ...
    Handlebars.registerHelper('accessibilityProps', (component) => {
      const props = [];
      if (component.ariaLabel) props.push(`aria-label="${component.ariaLabel}"`);
      if (component.ariaDescribedBy) props.push(`aria-describedby="${component.ariaDescribedBy}"`);
      if (component.role) props.push(`role="${component.role}"`);
      return props.join(' ');
    });
    ...
    Handlebars.registerHelper('responsiveClasses', (base, responsive) => {
      if (!responsive || typeof responsive !== 'object') return base;
    ...
    Handlebars.registerHelper('mcpPatterns', (context) => {
      if (context.mcp?.patterns?.patterns) {
        return context.mcp.patterns.patterns.map(p => 
          `${p.pattern} - ${p.recommendation}`
        ).join('\n * ');
      }
      return '';
    });
    ...
    Handlebars.registerHelper('mcpAntiPatterns', (context) => {
      if (context.mcp?.patterns?.antiPatterns) {
        return context.mcp.patterns.antiPatterns.map(p => 
          `AVOID: ${p.pattern} - ${p.reason}. USE: ${p.alternative}`
        ).join('\n * ');
      }
      return '';
    });
    ...
    Handlebars.registerHelper('crossPlatformComponent', (componentName, platforms) => {
      if (Array.isArray(platforms)) {
        return platforms.map(platform => 
          `${componentName}.${this.getPlatformSpecificPatterns(platform).fileExtensions[0]}`
        ).join(', ');
      }
   ...
   */
  async getMCPSpecs(componentName: string, platform: string): Promise<any> {
    try {
   ...
   */
  private getPlatformSpecificPatterns(platform: string): any {
    const patterns = {
      react: {
