...
export class TemplateRepositoryService extends EventEmitter {
  ...
  async registerRepository(
    ...
    try {
      ...
      // TODO: Fix logging format to match new API
      ...
      // TODO: Fix logging format to match new API
      console.error('Repository registration failed:', {
        operationId,
  ...
  async cloneRepository(
    ...
    try {
      ...
      // Log operation
      // TODO: Fix logging format to match new API
      console.log("Operation logged:", operationId);      
      // Emit event
    ...
    try {
      ...
      await this.logger.logOperation(
        ...
        "Repository push failed",
        {
          ...
          tags: ["git-operations", "push", "error"],
          structuredData: {
            repositoryName,
            commitMessage,
            error: error instanceof Error ? error.message : String(error)
    ...
    try {
      ...
      await this.logger.logOperation(
        ...
        "Version tagged successfully",
        {
          executionContext: {
            ...
            timestamp: new Date(),
            command: "tag-version",
            arguments: [repositoryName, version],
            workingDirectory: process.cwd(),
          },
          mcpOperation: {
            operationId,
      ...
      await this.logger.logOperation(
        ...
        "Version tagging failed",
        {
          ...
          tags: ["version-management", "tag", "error"],
          structuredData: {
            repositoryName,
            version,
            error: error instanceof Error ? error.message : String(error)
