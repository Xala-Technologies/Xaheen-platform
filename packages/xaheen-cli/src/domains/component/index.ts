...
export default class ComponentDomain {
  ...
  public async generate(command: CLICommand): Promise<void> {
    ...
    if (!description) {
      throw new CLIError('Component description is required', 'MISSING_DESCRIPTION', 'component', 'generate');
    }
    ...
    try {
      ...
      throw new CLIError(`Failed to generate component: ${error}`, 'COMPONENT_GENERATE_FAILED', 'component', 'generate');
    }
  }
  ...
  public async create(command: CLICommand): Promise<void> {
    ...
    if (!componentName) {
      throw new CLIError('Component name is required', 'MISSING_COMPONENT_NAME', 'component', 'create');
    }
    ...
    try {
      ...
      if (templates.length === 0) {
        cliLogger.warn(`No component templates found for platform: ${platform}`);
        return;
      }
      ...
      throw new CLIError(`Failed to create component: ${error}`, 'COMPONENT_CREATE_FAILED', 'component', 'create');
    }
  }
    ...
    try {
      ...
      if (monorepoInfo.isMonorepo) {
        ...
        for (const app of monorepoInfo.apps) {
          cliLogger.step(monorepoInfo.apps.indexOf(app) + 1, monorepoInfo.apps.length, `Building ${app}`);
        }
      }
      ...
      throw new CLIError(`Build failed: ${error}`, 'BUILD_FAILED', 'component', 'build');
    }
  }
}