...
export default class AppDomain {
  ...
  private initializeAppTemplates(): Map<string, AppTemplate> {
    ...
    templates.set('nextjs-app', {
      ...
      platform: 'web',
      framework: 'nextjs',
      features: ['App Router', 'Server Components', 'API Routes', 'TypeScript', 'Tailwind CSS'],
    ...
    templates.set('remix-app', {
      ...
      platform: 'web',
      framework: 'remix',
      features: ['Nested Routes', 'Data Loading', 'Forms', 'TypeScript', 'Tailwind CSS'],
      dependencies: ['@remix-run/react', '@remix-run/node', '@xala-technologies/ui-system']
    });
    ...
    templates.set('vite-react-app', {
      ...
      dependencies: ['react', 'react-dom', 'vite', '@xala-technologies/ui-system']
    });
    ...
    templates.set('expo-app', {
      ...
      dependencies: ['expo', 'react-native', '@xala-technologies/ui-system-mobile']
    });
    ...
    templates.set('react-native-app', {
      ...
      dependencies: ['react-native', 'react', '@xala-technologies/ui-system-mobile']
    });
    ...
    templates.set('electron-app', {
      ...
      platform: 'desktop',
      framework: 'electron',
      features: ['Windows/Mac/Linux', 'Auto Updates', 'System Tray', 'TypeScript'],
      ...
      for (const app of apps) {
        ...
        if (stat.isDirectory()) {
          ...
          try {
            ...
            logger.info(`    ${chalk.gray(packageJson.description || 'No description')}`);
            logger.info(`    Version: ${chalk.cyan(packageJson.version || '0.0.0')}`);
            
            if (packageJson.scripts) {
              const scripts = Object.keys(packageJson.scripts);
              if (scripts.length > 0) {
                logger.info(`    Scripts: ${chalk.yellow(scripts.slice(0, 3).join(', '))}${scripts.length > 3 ? '...' : ''}`);
              }
            }
            logger.info('');
          } catch (error) {
            logger.info(`  • ${chalk.green(app)} ${chalk.gray('(no package.json)')}`);
          }
        }
      }
    } catch (error) {
  ...
  async add(command: CLICommand): Promise<void> {
    ...
    if (!isMonorepo) {
      logger.warn('Not in a monorepo structure. Use "xaheen app create" instead.');
      return;
    }
  ...
  private async generateAppStructure(appPath: string, appName: string, template: AppTemplate): Promise<void> {
    // Create package.json
    const packageJson = {
  ...
  private getDependenciesForTemplate(template: AppTemplate): Record<string, string> {
  ...
  private getDevDependenciesForTemplate(template: AppTemplate): Record<string, string> {
  ...
  private async createFrameworkStructure(appPath: string, appName: string, template: AppTemplate): Promise<void> {
  ...
  private showAvailableTemplates(): void {
    ...
    templatesByPlatform.forEach((templates, platform) => {
      ...
      templates.forEach((template: any) => {
        logger.info(`    • ${chalk.green(template.key)}: ${template.description}`);
      });
    });
  }
}