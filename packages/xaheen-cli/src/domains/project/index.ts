...
export default class ProjectDomain {
  ...
  public async create(command: CLICommand): Promise<void> {
    ...
    if (!projectName) {
      throw new CLIError('Project name is required', 'MISSING_PROJECT_NAME', 'project', 'create');
    }
    ...
    try {
      ...
      if (await fs.pathExists(projectPath)) {
        throw new CLIError(`Directory "${projectName}" already exists`, 'DIRECTORY_EXISTS', 'project', 'create');
      }
      ...
      await this.createProjectStructure(projectPath, projectName, projectConfig);
      ...
      throw new CLIError(`Failed to create project: ${error}`, 'PROJECT_CREATION_FAILED', 'project', 'create');
    }
  }
  ...
  public async validate(command: CLICommand): Promise<void> {
    ...
    try {
      ...
      const structureIssues = await this.validateProjectStructure(config, monorepoInfo);
      ...
      if (structureIssues.length === 0 && serviceIssues.length === 0) {
        ...
        console.log(`  Package Manager: ${chalk.cyan(config.project.packageManager)}`);
        
        if (monorepoInfo.isMonorepo) {
          console.log(`  Monorepo: ${chalk.green('Yes')} (${monorepoInfo.structure})`);
          console.log(`  Apps: ${monorepoInfo.apps.length} (${monorepoInfo.apps.join(', ')})`);
          console.log(`  Packages: ${monorepoInfo.packages.length} (${monorepoInfo.packages.join(', ')})`);
        }
      ...
      throw new CLIError(`Project validation failed: ${error}`, 'VALIDATION_FAILED', 'project', 'validate');
    }
  }
  ...
  private async createProjectStructure(projectPath: string, projectName: string, config: any): Promise<void> {
...
`;
  ...
  private async validateProjectStructure(config: UnifiedConfig, monorepoInfo: any): Promise<string[]> {
    const issues: string[] = [];
  ...
  private showNextSteps(projectName: string, config: any): void {
    ...
    if (config.bundle) {
      ...
      console.log(`  Run ${chalk.cyan('xaheen service list')} to see available services`);
    }
    
    console.log(`\n${chalk.bold('Useful commands:')}`);
    console.log(`  ${chalk.cyan('xaheen add app <name>')} - Add new application`);
    console.log(`  ${chalk.cyan('xaheen service add <service>')} - Add service`);
    console.log(`  ${chalk.cyan('xaheen component generate "<description>"')} - Generate component`);
