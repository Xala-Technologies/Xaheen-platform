...
export default class MCPDomain {
		...
		try {
			...
			if (success) {
				...
				capabilities.forEach((cap) => {
					cliLogger.info(
						`  • ${chalk.cyan(cap.name)}: ${chalk.gray(cap.description)}`,
					);
				});
			} else {
	...
	public async generate(command: CLICommand): Promise<void> {
		try {
			...
			if (!componentName) {
				throw new Error(
					"Component name is required. Usage: xaheen mcp generate <name> --platform <platform>",
				);
			}
			...
			if (allPlatforms) {
				...
				result = await this.mcpClient.generateAllPlatforms(
					componentName,
					platforms,
					command.options,
				);
				...
				cliLogger.info(
					chalk.green(
						`✅ Generated ${componentName} for ${result.platforms.length} platforms:`,
					),
				);
				...
				result = await this.mcpClient.generateComponent(
					componentName,
					platform,
					command.options,
				);
				...
				cliLogger.info(
					chalk.green(`✅ Generated ${componentName} for ${platform}:`),
				);
				result.files.forEach((file: any) => {
	...
	public async info(command: CLICommand): Promise<void> {
		try {
			...
			cliLogger.info(
				`${chalk.bold("Supported:")} ${result.info.supported ? chalk.green("✓") : chalk.red("✗")}`,
			);

			if (result.info.features?.length > 0) {
				cliLogger.info(
					`${chalk.bold("Features:")} ${result.info.features.join(", ")}`,
				);
			}
		} catch (error) {
			throw new CLIError(
	...
	public async context(command: CLICommand): Promise<void> {
		try {
			...
			cliLogger.info(
				`${chalk.bold("Files Analyzed:")} ${context.files.length}`,
			);
			cliLogger.info(
				`${chalk.bold("Dependencies:")} ${context.dependencies.length}`,
			);

			cliLogger.info(chalk.blue("\n🛡️  Compliance:"));
			cliLogger.info(
				`  • Accessibility: ${chalk.cyan(context.compliance.accessibility)}`,
			);
			cliLogger.info(
				`  • Norwegian: ${context.compliance.norwegian ? chalk.green("✓") : chalk.gray("✗")}`,
			);
			cliLogger.info(
				`  • GDPR: ${context.compliance.gdpr ? chalk.green("✓") : chalk.gray("✗")}`,
			);

			// File type breakdown
			const fileTypes = context.files.reduce(
				(acc, file) => {
	...
	public async deploy(command: CLICommand): Promise<void> {
		try {
			...
			const criticalIssues = analysis.issues.filter(
				(issue) => issue.type === "error" || issue.severity >= 8,
			);
