...
describe("Main CLI Entry Point", () => {
	...
	describe("Banner Display", () => {
		it("should display banner by default", async () => {
			...
			const bannerCall = consoleSpy.mock.calls.find(
				(call) => call[0] && call[0].includes("Xaheen CLI"),
			);
	...
	describe("Performance Tracking", () => {
		it("should track command execution time on success", async () => {
			...
			const timeCall = debugCalls.find(
				(call) => call[0] && call[0].includes("Command completed in"),
			);
		...
		it("should track command execution time on error", async () => {
			...
			const timeCall = errorCalls.find(
				(call) => call[0] && call[0].includes("ms)"),
			);
			expect(timeCall).toBeDefined();
		});
	});
