...
const GENERATOR_TYPE_OPTIONS: Array<{
	...
	// Frontend generators
	{
		value: "component",
		label: "Component",
		hint: "React/Vue component with TypeScript",
	},
	...
	{ value: "model", label: "Model", hint: "Database model with Prisma" },
	{
		value: "controller",
		label: "Controller",
		hint: "NestJS controller with CRUD",
	},
	{ value: "service", label: "Service", hint: "Business logic service" },
	{
		value: "middleware",
		label: "Middleware",
		hint: "Request/response middleware",
	},
	...
	// Full-stack generators
	{
		value: "scaffold",
		label: "Scaffold",
		hint: "Complete feature with frontend and backend",
	},
	...
	{ value: "auth", label: "Auth", hint: "Authentication system" },
	{
		value: "feature",
		label: "Feature",
		hint: "Modular feature with all files",
	},
	...
	if (["scaffold", "crud", "model", "api"].includes(type)) {
		...
		if (addFields) {
			const fieldsInput = await text({
				message:
					"Enter fields (comma-separated, e.g., name:string,email:string,age:number):",
 ...
 */
async function runDirectGenerator(
	type: string,
	name: string,
...
export const generateCommand = new Command("generate")
	...
	.option(
		"--help-generators",
		"Show available generators and their descriptions",
	)
	.action(
		async (
			type?: string,
				...
				if (options?.helpGenerators) {
					...
					for (const [category, types] of Object.entries(categories)) {
						...
						for (const genType of types) {
							const option = GENERATOR_TYPE_OPTIONS.find(
								(opt) => opt.value === genType,
							);
							if (option) {
								consola.info(
									`  ${chalk.green(genType.padEnd(12))} - ${option.hint}`,
								);
							}
						}
						consola.info("");
					...
					consola.info(
						`  ${chalk.gray("$")} xaheen generate component MyButton`,
					);
					consola.info(
						`  ${chalk.gray("$")} xaheen g api users --fields "name:string,email:string"`,
					);
					consola.info(
						`  ${chalk.gray("$")} xaheen g scaffold Product --fields "name:string,price:number"`,
					);
					return;
				}
