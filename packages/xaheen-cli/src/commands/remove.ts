...
export const removeCommand = new Command("remove")
	...
	.argument(
		"[services...]",
		"Service types to remove (auth, payments, database, etc.)",
	)
		...
		try {
			...
			if (services.length > 0) {
				...
				const invalidServices = services.filter(
					(s) => !projectInfo.services.includes(s),
				);
				if (invalidServices.length > 0) {
					consola.error(
						`Services not found in project: ${invalidServices.join(", ")}`,
					);
					process.exit(1);
				}
			...
			if (dependencyAnalysis.blockers.length > 0) {
				...
				dependencyAnalysis.blockers.forEach((blocker) => {
					consola.warn(
						`  • ${chalk.yellow(blocker.dependentService)} depends on ${chalk.red(blocker.requiredService)}`,
					);
				});
				...
				if (!options.force) {
					const shouldContinue = await confirm({
						message:
							"Continue with removal anyway? This may break dependent services.",
						initialValue: false,
					});
			...
			const totalChanges =
				removalPlan.filesToRemove.length +
