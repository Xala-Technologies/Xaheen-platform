...
describe("AI Command", () => {
  ...
  beforeEach(async () => {
    ...
    const { AIRefactoringGenerator } = await import("../generators/ai/refactoring.generator.js");
    ...
    (AIRefactoringGenerator as Mock).mockImplementation(() => mockRefactoringGenerator);
  });
  ...
  describe("fixTestsCommand", () => {
    ...
    it("should execute test fixing logic", async () => {
      ...
      expect(mockSpawn).toHaveBeenCalledWith(
        "codebuff",
        expect.arrayContaining([
          expect.stringContaining("fix failing tests"),
        ]),
        expect.any(Object)
      );
    });
  ...
  describe("AI Refactoring Integration", () => {
    ...
    it("should handle refactoring generator errors", async () => {
      mockRefactoringGenerator.generate.mockRejectedValue(new Error("Refactoring failed"));
      
      await expect(
        mockRefactoringGenerator.generate({})
