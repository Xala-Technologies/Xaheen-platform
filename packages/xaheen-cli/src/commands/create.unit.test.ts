...
describe("Create Command", () => {
	...
	describe("Argument Parsing", () => {
		it("should parse project name argument", async () => {
			...
			// Replace the command's action with our mock
			const originalAction = createCommand._actionHandler;
			createCommand._actionHandler = actionFn;

			// Simulate command execution
			await createCommand.parseAsync(["node", "xaheen", "create", ...args], {
				from: "user",
			});

			// Verify the action was called with correct arguments
			expect(actionFn).toHaveBeenCalledWith("test-project", expect.any(Object), expect.any(Object));
			
			// Restore original action
			createCommand._actionHandler = originalAction;
		});
		...
		it("should handle missing project name with interactive prompt", async () => {
			...
			await createCommand.parseAsync(["node", "xaheen", "create"], {
				from: "user",
			});
	...
	describe("Preset Handling", () => {
		it("should use preset when provided", async () => {
			...
			expect(mockBundleResolver.resolveBundle).toHaveBeenCalledWith(
				"saas-starter",
			);
		});
	...
	describe("Project Scaffolding", () => {
		it("should validate project name", async () => {
			...
			expect(mockProjectScaffolder.validateProjectName).toHaveBeenCalledWith(
				"invalid-name",
			);
		});
	...
	describe("Service Selection", () => {
		it("should select services based on preset", async () => {
			...
			const actionFn = vi.fn().mockImplementation(async (name, options) => {
				const resolvedBundle = await mockBundleResolver.resolveBundle(
					options.preset,
				);
	...
	describe("Error Handling", () => {
		it("should handle bundle resolution errors", async () => {
			mockBundleResolver.resolveBundle.mockRejectedValue(
		...
		it("should handle project scaffolding errors", async () => {
			mockProjectScaffolder.scaffold.mockRejectedValue(
				new Error("Scaffolding failed"),
			);
	...
	describe("Interactive Flow", () => {
		it("should complete full interactive project creation", async () => {
			...
			const actionFn = vi.fn().mockImplementation(async (name, options) => {
				...
				if (!options.preset) {
					options.preset = await mockPrompts.select({
						message: "Choose preset:",
					});
				}

				const services = await mockPrompts.multiselect({
					message: "Select services:",
				});
				const confirmed = await mockPrompts.confirm({
