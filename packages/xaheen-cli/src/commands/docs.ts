...
export const docsCommand = new Command("docs")
	...
	.option(
		"-t, --type <type>",
		"Documentation type (api, architecture, deployment, portal, onboarding)",
	)
	...
	.action(async (options: DocsCommandOptions) => {
		try {
			...
			const outputDir =
				options.output || config.outputDir || join(projectPath, "docs");
			...
			if (options.all) {
				...
				outro(
					chalk.green(
						"✅ Comprehensive documentation suite generated successfully!",
					),
				);
				displayPostGenerationInfo(
					outputDir,
					true,
					true,
					options.sync,
			...
			if (options.portal) {
				...
				const result = await generateDocumentationPortal(
					projectInfo,
					config,
					outputDir,
				);
				...
				if (result.success) {
					...
					consola.info(
						chalk.cyan("🌐 Portal URL:"),
						result.portalUrl || "http://localhost:3000",
					);
			...
			if (options.onboarding) {
				...
				const result = await generateOnboardingGuides(
					projectInfo,
					config,
					outputDir,
				);
				...
				if (result.success) {
					...
					consola.info(
						...
						result.guideUrl || `${outputDir}/onboarding`,
					);
					consola.info(
						chalk.cyan("📊 Steps Generated:"),
						result.stepsGenerated,
					);
					consola.info(
						chalk.cyan("⏱️ Estimated Time:"),
						`${result.estimatedCompletionTime} minutes`,
					);
					displayGeneratedFiles(result.files);
				} else {
					outro(chalk.red(`❌ ${result.message}`));
			...
			if (options.type) {
				...
				const result = await generateSpecificDocumentation(
					options.type,
					projectInfo,
					config,
					outputDir,
...
async function analyzeProject(
	...
	try {
		...
		if (dependencies.pg || dependencies.postgresql)
...
async function loadDocumentationConfig(
	configPath?: string,
	projectPath?: string,
): Promise<Partial<DocumentationGeneratorOptions>> {
...
async function handleInteractiveMode(projectInfo: ProjectInfo): Promise<any> {
	const documentationTypes = await multiselect({
		...
		options: [
			{
				value: "portal",
				label: "📚 Documentation Portal (Docusaurus)",
				hint: "Complete documentation website",
			},
			{
				value: "onboarding",
				label: "🎓 Onboarding Guides",
				hint: "Dynamic user onboarding",
			},
			{
				value: "api",
				label: "🔌 API Documentation",
				hint: "OpenAPI and REST docs",
			},
			{
				value: "architecture",
				label: "🏗️ Architecture Docs",
				hint: "System architecture diagrams",
			},
			{
				value: "deployment",
				label: "🚀 Deployment Guides",
				hint: "Deployment instructions",
			},
			{
				value: "troubleshooting",
				label: "🔧 Troubleshooting",
				hint: "Common issues and solutions",
			},
		],
	});
...
async function generateComprehensiveDocumentation(
	...
	const result =
		await DocumentationGeneratorFactory.generateComprehensiveDocumentation(
			options,
		);
		...
		navigation: {
			sidebar: [
				...
				{ type: "doc", label: "Getting Started", id: "getting-started" },
				{
					...
					id: "guides",
					collapsed: false,
					items: [{ type: "generated", label: "Configuration", id: "guides" }],
				},
				{
					type: "category",
					label: "API Reference",
					id: "api",
					collapsed: false,
					items: [{ type: "generated", label: "Endpoints", id: "api" }],
				},
				{
					type: "category",
					label: "Tutorials",
					id: "tutorials",
					collapsed: false,
					items: [{ type: "generated", label: "Examples", id: "tutorials" }],
				},
			],
			navbar: [
			...
			triggers: [
				{
					event: "file-change",
					pattern: "src/**/*.ts",
					actions: [{ type: "update-api-docs" }, { type: "regenerate-docs" }],
				},
			],
			webhooks: [],
...
async function generateSpecificDocumentation(
	...
	switch (type) {
		case "api":
			const apiGenerator =
				DocumentationGeneratorFactory.createAPIReferenceGenerator();
			return await apiGenerator.generate(options);
		case "architecture":
			const archGenerator =
				DocumentationGeneratorFactory.createArchitectureDocsGenerator();
			return await archGenerator.generate(options);
		case "deployment":
			const deployGenerator =
				DocumentationGeneratorFactory.createDeploymentGuideGenerator();
			return await deployGenerator.generate(options);
		case "troubleshooting":
			const troubleGenerator =
				DocumentationGeneratorFactory.createTroubleshootingDocsGenerator();
...
function displayPostGenerationInfo(
	...
	if (hasPortal) {
		...
		consola.info(
			`  ${chalk.blue("•")} Start dev server: cd ${outputDir}/docs-portal && npm start`,
		);
		consola.info(
			`  ${chalk.blue("•")} Build for production: cd ${outputDir}/docs-portal && npm run build`,
		);
	}
	...
	if (hasOnboarding) {
		...
		consola.info(
			`  ${chalk.blue("•")} Interactive guides with progress tracking`,
		);
	...
	consola.info(
		`  ${chalk.blue("•")} Use \`xaheen docs --watch\` for development`,
	);
	consola.info(
		`  ${chalk.blue("•")} Configure automatic deployment with GitHub Actions`,
	);
	consola.info(
		`  ${chalk.blue("•")} Enable search with Algolia for production`,
	);
	consola.info(
		`  ${chalk.blue("•")} Add team collaboration features for larger projects`,
	);
}
