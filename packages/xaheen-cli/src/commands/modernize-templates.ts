...
export const modernizeTemplatesCommand = new Command('modernize')
  .alias('m')
  .description('🚀 Modernize templates with semantic components and Norwegian compliance')
 ...
 */
async function modernizeTemplatesHandler(options: ModernizeCommandOptions): Promise<void> {
  const spinner = ora('🔍 Initializing template modernization...').start();
  
  try {
    // Display welcome message
    console.log('\n' + chalk.cyan('🚀 Xaheen CLI - Template Modernization System'));
    console.log(chalk.gray('   EPIC 13 Story 13.2: Semantic UI System Template Modernization'));
    console.log(chalk.gray('   Compliance: WCAG AAA + Norwegian Government Standards\n'));

    // Show configuration
    console.log(chalk.yellow('📋 Configuration:'));
    console.log(`   Target Pattern: ${chalk.green(options.target || '**/*.hbs')}`);
    console.log(`   Output Directory: ${chalk.green(options.output || './modernized-templates')}`);
    console.log(`   WCAG Level: ${chalk.green(options.wcagLevel || 'AAA')}`);
    console.log(`   NSM Classification: ${chalk.green(options.nsmClassification || 'OPEN')}`);
    ...
    if (options.examples) {
      ...
      console.log('\n' + chalk.cyan('💡 Examples show the transformation from legacy HTML to semantic components'));
      return;
    }
    ...
    const analyses = await Promise.all(
      templateFiles.map(file => templateModernizationService.analyzeTemplate(file))
    );

    const needsModernization = analyses.filter(a => a.modernizationNeeded);
    spinner.succeed(`✅ Analysis complete: ${needsModernization.length}/${templateFiles.length} need modernization`);
    ...
    console.log(`   🔴 High Priority: ${chalk.red(priorityCounts.high)} templates`);
    console.log(`   🟡 Medium Priority: ${chalk.yellow(priorityCounts.medium)} templates`);
    console.log(`   🟢 Low Priority: ${chalk.green(priorityCounts.low)} templates`);

    const totalIssues = needsModernization.reduce((sum, a) => sum + a.issues.length, 0);
    const totalFixTime = needsModernization.reduce((sum, a) => sum + a.estimatedFixTime, 0);

    console.log(`   📋 Total Issues: ${chalk.cyan(totalIssues)}`);
    console.log(`   ⏱️  Estimated Fix Time: ${chalk.cyan(Math.round(totalFixTime / 60))} hours`);
    ...
    if (options.verbose && needsModernization.length > 0) {
      ...
      needsModernization.slice(0, 10).forEach((analysis, index) => {
        ...
        console.log(`      HTML Elements: ${chalk.red(analysis.htmlElementCount)}`);
        console.log(`      Semantic Components: ${chalk.green(analysis.semanticComponentCount)}`);
        console.log(`      Accessibility Score: ${getScoreColor(analysis.accessibilityScore)}`);
        console.log(`      Norwegian Compliance: ${getScoreColor(analysis.norwegianComplianceScore)}`);
        console.log(`      Fix Time: ${chalk.cyan(analysis.estimatedFixTime)} minutes`);
      });
    ...
    if (options.analyze) {
      console.log('\n' + chalk.green('✅ Analysis complete! Use --report to generate detailed reports.'));
      return;
    }
    ...
    if (options.dryRun) {
      ...
      for (const analysis of needsModernization.slice(0, 5)) {
        ...
        console.log(`      Would apply ${chalk.green(analysis.issues.filter(i => i.automaticFix).length)} automatic fixes`);
        console.log(`      Would require ${chalk.yellow(analysis.issues.filter(i => !i.automaticFix).length)} manual fixes`);
      }

      console.log('\n' + chalk.green('✅ Dry run complete! Remove --dry-run to apply changes.'));
      return;
    }
    ...
    const { results, summary } = await templateModernizationService.modernizeTemplates(
    ...
    console.log(`   ✅ Templates Modernized: ${chalk.green(summary.templatesModernized)}`);
    console.log(`   🔧 Issues Fixed: ${chalk.green(summary.issuesFixed)}`);
    console.log(`   📈 Average Improvement: ${chalk.green(summary.improvementPercentage + '%')}`);
    console.log(`   ⏱️  Time Saved: ${chalk.green(summary.timeSaved)} hours`);
    console.log(`   ♿ WCAG Compliance: ${chalk.green(summary.accessibilityCompliance)}`);
    console.log(`   🇳🇴 Norwegian Compliance: ${chalk.green(summary.norwegianCompliance ? 'Full' : 'Partial')}`);

    // Show key improvements
    console.log('\n' + chalk.yellow('🚀 Key Improvements Applied:'));
    console.log('   ✅ 100% Semantic Component Migration - All raw HTML elements replaced');
    console.log('   ✅ WCAG AAA Accessibility - Full screen reader and keyboard support');
    console.log('   ✅ Norwegian Government Compliance - NSM classification and i18n');
    console.log('   ✅ Design Token Integration - Consistent styling system');
    console.log('   ✅ Comprehensive i18n - Multi-language support (nb, nn, se, en)');
    console.log('   ✅ Performance Optimization - React.memo and optimization patterns');
    ...
    // Report generation
    if (options.report) {
      const reportSpinner = ora('📊 Generating comprehensive reports...').start();
      ...
      console.log(`   📄 ${chalk.cyan(join(options.output || './modernized-templates', 'modernization-report.md'))}`);
      console.log(`   📖 ${chalk.cyan(join(options.output || './modernized-templates', 'examples/'))}`);
    }

    // Show warnings if any
    const remainingIssues = results.reduce((sum, r) => sum + r.remainingIssues.length, 0);
    ...
    console.log(chalk.gray('   Generated with Xaheen CLI - EPIC 13 Story 13.2\n'));
