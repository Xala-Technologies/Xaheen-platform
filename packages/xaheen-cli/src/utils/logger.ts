...
export const cliLogger = {
	...
	legacy: (oldCommand: string, newCommand: string) => {
		logger.warn(chalk.yellow(`⚠ Legacy command detected: '${oldCommand}'`));
		logger.info(chalk.blue(`ℹ Consider using: '${newCommand}'`));
	},

	migration: (from: string, to: string) => {
		logger.info(chalk.blue(`🔄 Migrating from ${from} to ${to}`));
	},
