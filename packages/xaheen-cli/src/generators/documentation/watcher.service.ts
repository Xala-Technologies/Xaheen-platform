...
export class DocumentationWatcherService extends EventEmitter {
	...
	addProcessor(processor: FileProcessor): void {
		...
		console.log(
			`📄 Added processor: ${processor.name} (pattern: ${processor.pattern})`,
		);
	}
	...
	getStats(): WatcherStats {
		...
		const completedJobs = Array.from(this.jobQueue.values()).filter(
		...
		return {
			...this.stats,
			jobsQueued: Array.from(this.jobQueue.values()).filter(
				(job) => job.status === "pending",
			).length,
			jobsProcessing: this.processingJobs.size,
			jobsCompleted: completedJobs.length,
			jobsFailed: Array.from(this.jobQueue.values()).filter(
	...
	getJobQueue(): readonly ProcessingJob[] {
		return Array.from(this.jobQueue.values()).sort(
	...
	async processFile(
		filePath: string,
		eventType: WatchEvent["type"] = "change",
	...
	clearJobHistory(): void {
		const activeJobs = Array.from(this.jobQueue.values()).filter(
		...
		try {
			...
			for (const processor of processors) {
				try {
					...
					console.warn(
						`Processor ${processor.name} failed for ${filePath}:`,
						error,
					);
				}
			}
	...
	private async waitForJobsToComplete(timeoutMs: number): Promise<void> {
		...
		if (this.processingJobs.size > 0) {
			console.warn(
				`⚠️ ${this.processingJobs.size} jobs still processing after timeout`,
			);
		}
	}
 ...
 */
export function createDocumentationWatcher(
	config: WatcherConfig,
): DocumentationWatcherService {
			...
			if (exports.length > 0) {
				changes.push({
					...
					metadata: {
						...
						hasAPIDecorators:
							content.includes("@api") || content.includes("@ApiProperty"),
					},
				});
			}
				...
				// OpenAPI specifications
					...
					changes.push({
						...
						metadata: {
							...
							componentCount: Object.keys(data.components?.schemas || {})
								.length,
						},
					});
				}
...
export const createDefaultWatcherHooks = (): readonly WatchHook[] => [
	{
		...
		handler: async (context: WatchContext) => {
			console.log(
				`🔄 Processing file: ${relative(process.cwd(), context.filePath)}`,
			);
		},
	},

	{
		...
		handler: async (context: WatchContext) => {
			if (context.changes && context.changes.length > 0) {
				console.log(
					`✅ Processed ${relative(process.cwd(), context.filePath)}: ${context.changes.length} changes`,
				);
			}
		},
	},

	{
		...
		handler: async (context: WatchContext) => {
			console.error(
				`❌ Error processing ${relative(process.cwd(), context.filePath)}:`,
