...
export interface DeveloperWorkflowGeneratorOptions
...
export class DeveloperWorkflowGenerator extends BaseGenerator<DeveloperWorkflowGeneratorOptions> {
	async generate(
		options: DeveloperWorkflowGeneratorOptions,
	): Promise<DocumentationResult> {
		try {
			this.logger.info(
				`Generating developer workflow documentation for ${options.projectName}`,
			);
			...
			this.logger.error(
				"Failed to generate developer workflow documentation",
				error,
			);
			return {
				success: false,
