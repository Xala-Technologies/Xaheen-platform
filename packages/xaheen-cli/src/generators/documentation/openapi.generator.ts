...
export class OpenAPIGenerator extends BaseGenerator<OpenAPIGeneratorOptions> {
  ...
  async generate(options: OpenAPIGeneratorOptions): Promise<DocumentationResult> {
    try {
      await this.validateOptions(options);
      
      const projectContext = await this.analyzer.analyze(process.cwd());
      const openAPISpec = await this.generateOpenAPISpec(options, projectContext);
  ...
  private async validateOptions(options: OpenAPIGeneratorOptions): Promise<void> {
  ...
  private async generateOpenAPISpec(
    ...
    const spec = {
      ...
      info: {
        ...
        description: options.apiDescription || `API documentation for ${options.projectName}`,
  ...
  private async generateSpecification(spec: any, options: OpenAPIGeneratorOptions): Promise<void> {
  ...
  private async generateSwaggerUI(options: OpenAPIGeneratorOptions): Promise<void> {
    const swaggerConfig = {
  ...
  private async generateRedocDocs(options: OpenAPIGeneratorOptions): Promise<void> {
    const redocConfig = {
      specUrl: './openapi.yaml',
  ...
  private async generatePostmanCollection(spec: any, options: OpenAPIGeneratorOptions): Promise<void> {
    const collection = {
      info: {
  ...
  private async generateClientExamples(spec: any, options: OpenAPIGeneratorOptions): Promise<void> {
  ...
  private async generateValidationMiddleware(spec: any, options: OpenAPIGeneratorOptions): Promise<void> {
  ...
  private generatePaths(endpoints: readonly APIEndpoint[]): Record<string, any> {
  ...
  private generateSchemas(endpoints: readonly APIEndpoint[]): Record<string, APISchema> {
  ...
  private generateExamples(endpoints: readonly APIEndpoint[]): Record<string, any> {
  ...
  private generateLinks(endpoints: readonly APIEndpoint[]): Record<string, any> {
  ...
  private generateGlobalSecurity(securitySchemes?: Record<string, SecurityScheme>): SecurityRequirement[] {
    if (!securitySchemes) {
      return [{ bearerAuth: [] }];
    }
  ...
  private generateTagsFromEndpoints(endpoints: readonly APIEndpoint[]): APITag[] {
  ...
  private generatePostmanAuth(securitySchemes?: Record<string, SecurityScheme>): any {
    if (!securitySchemes) {
      return {
        type: 'bearer',
    ...
    if (firstScheme.type === 'apiKey') {
      return {
        type: 'apikey',
        apikey: [
          { key: 'key', value: firstScheme.name || 'X-API-Key', type: 'string' },
  ...
  private generatePostmanVariables(servers: any[]): any[] {
    const variables = [
      { key: 'baseUrl', value: servers[0]?.url || 'https://api.example.com', type: 'string' }
    ];
  ...
  private generatePostmanBody(operation: any): any {
    if (operation.requestBody?.content?.['application/json']) {
      return {
        mode: 'raw',
        raw: JSON.stringify(this.generateExampleFromSchema(operation.requestBody.content['application/json'].schema), null, 2),
        options: {
          raw: {
            language: 'json'
          }
