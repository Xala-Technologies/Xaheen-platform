...
export interface GettingStartedGeneratorOptions
...
export class GettingStartedGenerator extends BaseGenerator<GettingStartedGeneratorOptions> {
	async generate(
		options: GettingStartedGeneratorOptions,
	): Promise<DocumentationResult> {
		try {
			this.logger.info(
				`Generating getting started documentation for ${options.projectName}`,
			);
			...
			this.logger.error(
				"Failed to generate getting started documentation",
				error,
			);
			return {
				success: false,
