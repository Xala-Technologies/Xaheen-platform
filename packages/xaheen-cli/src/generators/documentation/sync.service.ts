...
export class DocumentationSyncService {
	...
	async startWatching(): Promise<void> {
		...
		console.log(
			`📚 Documentation sync service started watching ${watchPatterns.join(", ")}`,
		);
	}
	...
	async processFile(
		filePath: string,
		eventType: "add" | "change" | "unlink" | "manual",
			...
			if (this.shouldExecuteTrigger(trigger, filePath, eventType)) {
				for (const action of trigger.actions) {
					try {
						...
						console.error(
							`Failed to execute sync action ${action.type}:`,
							error,
						);
					}
				}
	...
	private shouldExecuteTrigger(
		...
		if (
			trigger.event === "file-change" &&
			!["add", "change", "unlink"].includes(eventType)
		) {
			return false;
		}
	...
	private async regenerateDocumentation(
		...
		console.log(
			`🔄 Regenerating documentation${target ? ` for ${target}` : ""}...`,
		);
	...
	private async updateAPIDocumentation(
		filePath: string,
		config?: Record<string, any>,
	): Promise<void> {
	...
	private async refreshExamples(
		...
		try {
			...
			const examplesDir =
				config?.examplesDir || join(this.options.docsRoot, "examples");
	...
	private async notifyTeam(
		...
		console.log(
			`📢 Notifying team about ${changes.length} documentation changes...`,
		);
	...
	private async runHooks(
		event: "before-process" | "after-process" | "error",
 ...
 */
export function createDocumentationSyncService(
