...
export interface LayoutGeneratorOptions extends BaseGeneratorOptions {
	...
	readonly layoutType?: 'admin' | 'auth' | 'dashboard' | 'landing' | 'blog' | 'ecommerce' | 'mobile' | 'custom';
...
export class LayoutGenerator extends BaseGenerator<LayoutGeneratorOptions> {
	private readonly layoutTypes = [
		{ value: 'admin', label: 'Admin Layout', hint: 'Dashboard with sidebar navigation' },
		{ value: 'auth', label: 'Authentication Layout', hint: 'Login/signup centered layout' },
		{ value: 'dashboard', label: 'Dashboard Layout', hint: 'Analytics and metrics layout' },
		{ value: 'landing', label: 'Landing Page Layout', hint: 'Marketing page layout' },
		{ value: 'blog', label: 'Blog Layout', hint: 'Article reading layout' },
		{ value: 'ecommerce', label: 'E-commerce Layout', hint: 'Product catalog layout' },
		{ value: 'mobile', label: 'Mobile Layout', hint: 'Mobile-first responsive layout' },
		{ value: 'custom', label: 'Custom Layout', hint: 'Blank template to customize' },
	];

	private readonly layoutFeatures = [
		{ value: 'header', label: 'Header Navigation', hint: 'Top navigation bar' },
		{ value: 'sidebar', label: 'Sidebar Navigation', hint: 'Side navigation panel' },
		{ value: 'footer', label: 'Footer', hint: 'Page footer with links' },
		{ value: 'breadcrumbs', label: 'Breadcrumbs', hint: 'Navigation breadcrumbs' },
		{ value: 'search', label: 'Search Bar', hint: 'Global search functionality' },
		{ value: 'notifications', label: 'Notifications', hint: 'Toast/alert notifications' },
		{ value: 'theme-toggle', label: 'Dark Mode Toggle', hint: 'Light/dark theme switcher' },
		{ value: 'user-menu', label: 'User Menu', hint: 'Profile dropdown menu' },
		{ value: 'mobile-menu', label: 'Mobile Menu', hint: 'Hamburger mobile navigation' },
	];
	...
	async generate(options: LayoutGeneratorOptions): Promise<GeneratorResult> {
		...
		const layoutFile = await this.generateLayoutComponent(layoutData, completeOptions);
		...
		this.logger.success(`Layout ${chalk.green(completeOptions.name)} generated successfully!`);
	...
	private async parseFeatures(featuresInput: string[]): Promise<LayoutFeature[]> {
		if (featuresInput.length === 0) {
			...
			return (selectedFeatures as string[]).map(feature => {
				const featureData = this.layoutFeatures.find(f => f.value === feature);
				return {
					name: feature,
	...
	private async generateStories(layoutData: any, options: LayoutGeneratorOptions): Promise<string | null> {
		const placement = this.getFilePlacement('layout', layoutData.name);
	...
	private async generateTests(layoutData: any, options: LayoutGeneratorOptions): Promise<string | null> {
		const placement = this.getFilePlacement('layout', layoutData.name);
	...
	private async generateStyles(layoutData: any, options: LayoutGeneratorOptions): Promise<string | null> {
	...
	protected async promptForMissingOptions(options: Partial<LayoutGeneratorOptions>): Promise<LayoutGeneratorOptions> {
		...
		if (!result.accessibility) {
			const accessibility = await select({
				...
				options: [
					{ value: 'A', label: 'WCAG A', hint: 'Basic accessibility compliance' },
					{ value: 'AA', label: 'WCAG AA', hint: 'Standard accessibility compliance' },
					{ value: 'AAA', label: 'WCAG AAA', hint: 'Highest accessibility compliance' },
				],
			});
