...
import { BackendGenerator, type BackendGeneratorOptions, type BackendGeneratorResult, type GeneratedFile, type BackendFeature } from '../index';

export class NestJSGenerator extends BackendGenerator {
  readonly framework = 'nestjs';
  readonly defaultPort = 3000;
  readonly supportedDatabases = ['postgresql', 'mysql', 'mongodb', 'sqlite', 'supabase'] as const;
  ...
  protected getBaseDependencies(options: BackendGeneratorOptions): Record<string, string> {
    return {
  ...
  protected getDevDependencies(options: BackendGeneratorOptions): Record<string, string> {
    const devDeps = {
      '@nestjs/cli': '^10.0.0',
  ...
  protected getScripts(options: BackendGeneratorOptions): Record<string, string> {
    return {
      'build': 'nest build',
  ...
  protected getEnvironmentVariables(options: BackendGeneratorOptions): Record<string, string> {
    ...
    switch (options.database) {
      case 'postgresql':
        envVars['DATABASE_URL'] = 'postgresql://postgres:postgres@localhost:5432/xaheen_dev';
        break;
      case 'mysql':
        envVars['DATABASE_URL'] = 'mysql://root:password@localhost:3306/xaheen_dev';
        break;
      case 'mongodb':
      ...
      case 'sqlite':
        ...
        break;
      case 'supabase':
        envVars['DATABASE_URL'] = 'postgresql://postgres:[YOUR-PASSWORD]@db.[YOUR-PROJECT-REF].supabase.co:5432/postgres';
    ...
    if (options.authentication === 'bankid') {
      ...
      envVars['BANKID_REDIRECT_URI'] = 'http://localhost:3000/auth/bankid/callback';
    }

    return envVars;
  }

  protected getFeatureDependencies(features: readonly BackendFeature[]): Record<string, string> {
  ...
  protected getDatabaseDependencies(database: string, orm: string): Record<string, string> {
    const deps: Record<string, string> = {};
...
bootstrap().catch((error) => {
  ...
  private generateAppModule(options: BackendGeneratorOptions): GeneratedFile {
    ...
    const moduleImports = [
      ...
      ...(options.features.includes('caching') ? ['CacheModule.register({ isGlobal: true })'] : []),
      ...(options.features.includes('cron') ? ['ScheduleModule.forRoot()'] : []),
      'DatabaseModule',
      ...(options.authentication ? ['AuthModule'] : []),
      ...options.features.map(feature => `${this.getFeatureModuleName(feature)}Module`)
    ];
...
export class AppService {
  ...
  private generateConfigModule(options: BackendGeneratorOptions): GeneratedFile[] {
...
export class AppConfigModule {}`;
  ...
  private generateDatabaseConfig(options: BackendGeneratorOptions): GeneratedFile[] {
  ...
  private generatePrismaConfig(options: BackendGeneratorOptions): GeneratedFile[] {
...
export class PrismaService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
  ...
  private generateTypeOrmConfig(options: BackendGeneratorOptions): GeneratedFile[] {
    // TypeORM configuration implementation
    return [];
  }

  private generateAuthModule(options: BackendGeneratorOptions): GeneratedFile[] {
    // Authentication module implementation
    return [];
  }

  private generateFeatureModule(feature: BackendFeature, options: BackendGeneratorOptions): GeneratedFile[] {
  ...
  private generateDocumentationFiles(options: BackendGeneratorOptions): GeneratedFile[] {
    // Documentation generation
    return [];
  }

  private generateDeploymentFiles(options: BackendGeneratorOptions): GeneratedFile[] {
    // Deployment configuration
    return [];
  }
