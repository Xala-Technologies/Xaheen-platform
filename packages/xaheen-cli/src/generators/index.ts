...
const GENERATOR_CATEGORIES = {
	...
	// Meta-generator category
	meta: ["meta-generator", "generator-template", "generator-composition", "generator-workflow"],
 ...
 */
async function executeMetaGenerator(
	context: GeneratorContext,
): Promise<GeneratorResult> {
	const { type, name, options } = context;
	...
	try {
		switch (type) {
			case "meta-generator":
				const { MetaGenerator } = await import("./meta/index");
				const metaGenerator = new MetaGenerator();
				
				const result = await metaGenerator.generate({
					name,
					templateId: options.templateId || 'basic',
					targetGenerator: {
						id: name.toLowerCase().replace(/[^a-z0-9]/g, '-'),
						name: name,
						version: '1.0.0',
						description: options.description || `Generated ${name} generator`,
						author: options.author || 'Generated by Xaheen Meta-Generator',
					...
					publishToMarketplace: options.publishToMarketplace || false,
			...
			case "generator-template":
				const { TemplateEngine } = await import("./meta/index");
				...
				return {
					success: true,
					message: `Generator template '${name}' created successfully`,
					files: [template.path, `${template.path.replace('.hbs', '.json')}`],
					commands: [],
					nextSteps: [
						'Customize the template content',
						'Add template variables and blocks',
						'Test the template with sample data',
						'Use the template in meta-generator'
					]
				};

			case "generator-composition":
				const { GeneratorComposer, GeneratorRegistry } = await import("./meta/index");
				const registry = new GeneratorRegistry();
				const composer = new GeneratorComposer(registry);

				const composition = {
					generators: options.generators || [],
					execution: options.execution || 'sequential' as const,
					errorHandling: options.errorHandling || 'fail-fast' as const,
					rollback: options.rollback || 'files' as const,
					conditions: options.conditions || []
				};

				const compositionResult = await composer.executeComposition(composition, options);

				return {
					success: compositionResult.success,
					message: `Generator composition '${name}' executed successfully`,
					files: compositionResult.results.flatMap(r => r.files || []),
					commands: compositionResult.results.flatMap(r => r.commands || []),
					nextSteps: [
						'Review composition execution results',
						'Check individual generator outputs',
						'Test the combined functionality',
						'Save successful compositions for reuse'
					]
				};

			case "generator-workflow":
				const { MetaGenerator: WorkflowGenerator } = await import("./meta/index");
				const workflowGenerator = new WorkflowGenerator();

				const workflow = {
					id: name.toLowerCase().replace(/[^a-z0-9]/g, '-'),
					name,
					description: options.description || `Generated workflow: ${name}`,
					steps: options.steps || [],
					triggers: options.triggers || [],
					variables: options.variables || []
				};

				const workflowResult = await workflowGenerator.executeWorkflow(workflow, options);

				return {
					success: workflowResult.success,
					message: workflowResult.message,
					files: workflowResult.files || [],
					commands: workflowResult.commands || [],
			...
			default:
				return {
					success: false,
					message: `Unknown meta-generator type: ${type}`,
					error: `Meta-generator type '${type}' is not supported`,
				};
		}
	} catch (error) {
		return {
			success: false,
			message: `Failed to execute meta-generator ${type}: ${error instanceof Error ? error.message : "Unknown error"}`,
			error: error instanceof Error ? error.message : "Unknown error",
		};
	}
}
		...
		switch (category) {
			...
			case "backend":
				if (type === "middleware") {
					...
					return await middlewareGenerator.generate({
						...
						type: context.options.middlewareType,
						features: context.options.features ? context.options.features.split(',') : undefined,
						async: context.options.async,
						norwegian: context.options.norwegian,
			...
			case "database":
				if (type === "migration") {
					...
					return await migrationGenerator.generate({
						name: context.name,
						orm: context.options.orm,
						action: context.options.action || 'create',
						table: context.options.table,
						fields: context.options.fields ? context.options.fields.split(',') : undefined,
						operations: context.options.operations ? context.options.operations.split(',') : undefined,
						rollback: context.options.rollback,
						norwegian: context.options.norwegian,
						dataCompliance: context.options.dataCompliance,
						typescript: context.options.typescript !== false,
						dryRun: context.options.dryRun,
						force: context.options.force,
					});
					...
					return await seedGenerator.generate({
						name: context.name,
						orm: context.options.orm,
						tables: context.options.tables ? context.options.tables.split(',') : undefined,
						dataType: context.options.dataType,
						recordCount: context.options.recordCount ? parseInt(context.options.recordCount) : undefined,
						locale: context.options.locale,
						relationships: context.options.relationships,
						gdprCompliant: context.options.gdprCompliant,
						anonymized: context.options.anonymized,
						typescript: context.options.typescript !== false,
						dryRun: context.options.dryRun,
						force: context.options.force,
					});
			...
			case "fullstack":
				if (type === "scaffold") {
					...
					return await scaffoldGenerator.generate({
						name: context.name,
						fields: context.options.fields ? context.options.fields.split(',') : undefined,
						features: context.options.features ? context.options.features.split(',') : undefined,
						frontend: context.options.frontend,
						backend: context.options.backend,
						database: context.options.database,
						styling: context.options.styling,
						auth: context.options.auth,
						api: context.options.api,
						tests: context.options.tests,
						norwegian: context.options.norwegian,
						accessibility: context.options.accessibility,
						typescript: context.options.typescript !== false,
						dryRun: context.options.dryRun,
						force: context.options.force,
					});
			...
			case "testing":
				if (type === "test") {
					...
					return await testGenerator.generate({
						name: context.name,
						targetFile: context.options.targetFile,
						testTypes: context.options.testTypes ? context.options.testTypes.split(',') as any : undefined,
						framework: context.options.framework,
						testingLibrary: context.options.testingLibrary,
						coverage: context.options.coverage,
						mocks: context.options.mocks,
						fixtures: context.options.fixtures,
						norwegian: context.options.norwegian,
						accessibility: context.options.accessibility,
						typescript: context.options.typescript !== false,
						dryRun: context.options.dryRun,
			...
			case "meta":
				return await executeMetaGenerator(context);
...
export function getGeneratorHelp(type: GeneratorType): string {
	const helpTexts: Record<GeneratorType, string> = {
		...
		// Meta-generators
		"meta-generator":
			"Generate a new generator that can generate other generators (Rails-style meta-programming)",
		"generator-template":
			"Create reusable templates for generator composition with inheritance and variables",
		"generator-composition":
			"Compose multiple generators into complex workflows with dependency management",
		"generator-workflow":
			"Create sophisticated generator workflows with conditional logic and parallel execution",
	};

	return helpTexts[type] || `Generate ${type} with best practices`;
}

// Export new generator classes
export { PageGenerator } from './page.generator.js';
