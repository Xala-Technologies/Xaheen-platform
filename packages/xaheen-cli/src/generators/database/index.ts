...
export interface DatabaseField {
  readonly name: string;
  readonly type: 'string' | 'number' | 'boolean' | 'date' | 'uuid' | 'json' | 'text' | 'enum';
 ...
 */
function createDatabaseGenerator(orm: DatabaseGeneratorOptions['orm']): DatabaseGenerator {
  switch (orm) {
    case 'prisma':
...
export abstract class DatabaseGenerator {
  ...
  protected convertFieldType(field: DatabaseField, database: string): string {
    switch (field.type) {
      case 'string':
        if (database === 'postgresql') return field.length ? `VARCHAR(${field.length})` : 'TEXT';
        if (database === 'mysql') return field.length ? `VARCHAR(${field.length})` : 'TEXT';
        return 'TEXT';
...
export class PrismaGenerator extends DatabaseGenerator {
  readonly orm = 'prisma';
  readonly supportedDatabases = ['postgresql', 'mysql', 'sqlite', 'mongodb'] as const;
  ...
  private generateMigration(options: DatabaseGeneratorOptions): GeneratedDatabaseFile[] {
  ...
  private generateSeed(options: DatabaseGeneratorOptions): GeneratedDatabaseFile {
...
main();`;
  ...
  private generateSchema(options: DatabaseGeneratorOptions): GeneratedDatabaseFile {
  ...
  private convertPrismaFieldType(field: DatabaseField): string {
    switch (field.type) {
      ...
      case 'text':
        return field.required ? 'String' : 'String?';
      case 'enum':
        return field.required ? `${this.capitalize(field.name)}Enum` : `${this.capitalize(field.name)}Enum?`;
      default:
        return field.required ? 'String' : 'String?';
    }
  }
...
export class TypeORMGenerator extends DatabaseGenerator {
  ...
  readonly supportedTypes = ['migration', 'seed', 'schema', 'repository'] as const;
