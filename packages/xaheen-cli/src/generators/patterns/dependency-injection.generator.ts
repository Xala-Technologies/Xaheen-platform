...
export interface DIGeneratorOptions extends GeneratorOptions {
  ...
  readonly framework?: "nestjs" | "inversify" | "awilix" | "tsyringe" | "generic";
}
...
export class DependencyInjectionGenerator {
  ...
  constructor(projectPath: string) {
    ...
    this.templatesPath = join(__dirname, "../../templates/patterns/dependency-injection");
  }
  ...
  private async generateDIContainer(
    ...
    const containerContent = this.generateDIContainerClass(name, services, providers, configuration, framework);
    files.push(await this.writeFile(this.getDIContainerPath(name), containerContent));

    // Generate container interface
    const containerInterfaceContent = this.generateDIContainerInterface(name, services);
    files.push(await this.writeFile(this.getDIContainerInterfacePath(name), containerInterfaceContent));

    // Generate service registry
    const registryContent = this.generateServiceRegistry(services);
    files.push(await this.writeFile(this.getServiceRegistryPath(), registryContent));
    ...
    for (const service of services) {
      ...
      files.push(await this.writeFile(this.getServiceDefinitionPath(service.name), serviceContent));

      if (service.interface) {
        const interfaceContent = this.generateServiceInterface(service);
        files.push(await this.writeFile(this.getServiceInterfacePath(service.name), interfaceContent));
      }
    }
    ...
    for (const provider of providers) {
      ...
      files.push(await this.writeFile(this.getProviderPath(provider.name), providerContent));
    }

    // Generate modules
    for (const module of modules) {
      const moduleContent = this.generateModuleClass(module, framework);
      files.push(await this.writeFile(this.getModulePath(module.name), moduleContent));
    }

    // Generate container builder
    const builderContent = this.generateContainerBuilder(name, services, providers, modules);
    files.push(await this.writeFile(this.getContainerBuilderPath(name), builderContent));

    // Generate tests if requested
    if (includeTests) {
      const containerTestContent = this.generateContainerTest(name, services);
      files.push(await this.writeFile(this.getContainerTestPath(name), containerTestContent));

      for (const service of services) {
        const serviceTestContent = this.generateServiceTest(service);
        files.push(await this.writeFile(this.getServiceTestPath(service.name), serviceTestContent));
      }
    }

    // Generate documentation
    const docContent = this.generateDIDocumentation(name, services, providers, modules);
  ...
  private async generateServiceLocator(
    ...
    const serviceLocatorContent = this.generateServiceLocatorClass(name, services);
    files.push(await this.writeFile(this.getServiceLocatorPath(name), serviceLocatorContent));

    const serviceLocatorInterfaceContent = this.generateServiceLocatorInterface(name);
    files.push(await this.writeFile(this.getServiceLocatorInterfacePath(name), serviceLocatorInterfaceContent));

    if (includeTests) {
      const testContent = this.generateServiceLocatorTest(name, services);
      files.push(await this.writeFile(this.getServiceLocatorTestPath(name), testContent));
    }
  ...
  private async generateFactory(
    ...
    files.push(await this.writeFile(this.getFactoryInterfacePath(name), factoryInterfaceContent));

    if (includeTests) {
      const testContent = this.generateFactoryTest(name, services);
      files.push(await this.writeFile(this.getFactoryTestPath(name), testContent));
    }
  ...
  private async generateProvider(
    ...
    if (providers.length === 0) {
      ...
      files.push(await this.writeFile(this.getProviderPath(name), providerContent));

      if (includeTests) {
        const testContent = this.generateProviderTest(basicProvider);
        files.push(await this.writeFile(this.getProviderTestPath(name), testContent));
      }
    } else {
      for (const provider of providers) {
        const providerContent = this.generateProviderClass(provider);
        ...
        if (includeTests) {
          ...
          files.push(await this.writeFile(this.getProviderTestPath(provider.name), testContent));
        }
      }
    }
  ...
  private async generateAdapter(
    ...
    if (adapters.length === 0) {
      ...
      files.push(await this.writeFile(this.getAdapterPath(name), adapterContent));

      const sourceInterfaceContent = this.generateAdapterSourceInterface(basicAdapter);
      files.push(await this.writeFile(this.getAdapterSourceInterfacePath(name), sourceInterfaceContent));

      const targetInterfaceContent = this.generateAdapterTargetInterface(basicAdapter);
      files.push(await this.writeFile(this.getAdapterTargetInterfacePath(name), targetInterfaceContent));

      if (includeTests) {
        const testContent = this.generateAdapterTest(basicAdapter);
        files.push(await this.writeFile(this.getAdapterTestPath(name), testContent));
      }
    } else {
      for (const adapter of adapters) {
        const adapterContent = this.generateAdapterClass(adapter);
        files.push(await this.writeFile(this.getAdapterPath(adapter.name), adapterContent));

        const sourceInterfaceContent = this.generateAdapterSourceInterface(adapter);
        files.push(await this.writeFile(this.getAdapterSourceInterfacePath(adapter.name), sourceInterfaceContent));

        const targetInterfaceContent = this.generateAdapterTargetInterface(adapter);
        files.push(await this.writeFile(this.getAdapterTargetInterfacePath(adapter.name), targetInterfaceContent));

        if (includeTests) {
          const testContent = this.generateAdapterTest(adapter);
          files.push(await this.writeFile(this.getAdapterTestPath(adapter.name), testContent));
        }
      }
    }
  ...
  private async generateDecorator(
    ...
    const { decorators = [], includeTests = true, framework = "generic" } = options;
    const files: string[] = [];
    ...
    if (decorators.length === 0) {
      ...
      const decoratorContent = this.generateDecoratorClass(basicDecorator, framework);
      files.push(await this.writeFile(this.getDecoratorPath(name), decoratorContent));

      if (includeTests) {
        const testContent = this.generateDecoratorTest(basicDecorator);
        files.push(await this.writeFile(this.getDecoratorTestPath(name), testContent));
      }
    } else {
      for (const decorator of decorators) {
        const decoratorContent = this.generateDecoratorClass(decorator, framework);
        files.push(await this.writeFile(this.getDecoratorPath(decorator.name), decoratorContent));

        if (includeTests) {
          const testContent = this.generateDecoratorTest(decorator);
          files.push(await this.writeFile(this.getDecoratorTestPath(decorator.name), testContent));
        }
      }
    }
   ...
   */
  private async generateProxy(name: string, options: DIGeneratorOptions): Promise<GeneratorResult> {
    const files: string[] = [];
    ...
    return {
      ...
      nextSteps: ["Implement proxy logic", "Add proxy interceptors", "Configure proxy behavior"],
    };
  }

  private async generateBridge(name: string, options: DIGeneratorOptions): Promise<GeneratorResult> {
    const files: string[] = [];
    ...
    return {
      ...
      files,
      commands: ["npm run type-check"],
      nextSteps: ["Implement bridge logic", "Add bridge abstraction", "Configure bridge implementations"],
    };
  }

  private async generateFacade(name: string, options: DIGeneratorOptions): Promise<GeneratorResult> {
    const files: string[] = [];
    ...
    return {
      ...
      files,
      commands: ["npm run type-check"],
      nextSteps: ["Implement facade methods", "Add subsystem coordination", "Configure facade dependencies"],
    };
  }

  private async generateRegistry(name: string, options: DIGeneratorOptions): Promise<GeneratorResult> {
    const files: string[] = [];
    const registryContent = this.generateRegistryClass(name);
    files.push(await this.writeFile(this.getRegistryPath(name), registryContent));
    ...
    return {
      ...
      files,
      commands: ["npm run type-check"],
      nextSteps: ["Implement registry storage", "Add registry queries", "Configure registry persistence"],
    };
  }

  private async generateModuleSystem(name: string, options: DIGeneratorOptions): Promise<GeneratorResult> {
    const { modules = [] } = options;
    const files: string[] = [];

    const moduleSystemContent = this.generateModuleSystemClass(name, modules);
    files.push(await this.writeFile(this.getModuleSystemPath(name), moduleSystemContent));
    ...
    return {
      ...
      files,
      commands: ["npm run type-check"],
      nextSteps: ["Configure module loading", "Add module dependencies", "Implement module lifecycle"],
    };
  }

  private async generateInterceptor(name: string, options: DIGeneratorOptions): Promise<GeneratorResult> {
    const { interceptors = [], includeTests = true, framework = "generic" } = options;
    const files: string[] = [];
    ...
    if (interceptors.length === 0) {
      ...
      const interceptorContent = this.generateInterceptorClass(basicInterceptor, framework);
      files.push(await this.writeFile(this.getInterceptorPath(name), interceptorContent));

      if (includeTests) {
        const testContent = this.generateInterceptorTest(basicInterceptor);
        files.push(await this.writeFile(this.getInterceptorTestPath(name), testContent));
      }
    } else {
      for (const interceptor of interceptors) {
        const interceptorContent = this.generateInterceptorClass(interceptor, framework);
        files.push(await this.writeFile(this.getInterceptorPath(interceptor.name), interceptorContent));

        if (includeTests) {
          const testContent = this.generateInterceptorTest(interceptor);
          files.push(await this.writeFile(this.getInterceptorTestPath(interceptor.name), testContent));
        }
      }
    }
  ...
  private async generateMiddleware(name: string, options: DIGeneratorOptions): Promise<GeneratorResult> {
    const { middleware = [], includeTests = true, framework = "generic" } = options;
    const files: string[] = [];
    ...
    if (middleware.length === 0) {
      ...
      const middlewareContent = this.generateMiddlewareClass(basicMiddleware, framework);
      files.push(await this.writeFile(this.getMiddlewarePath(name), middlewareContent));

      if (includeTests) {
        const testContent = this.generateMiddlewareTest(basicMiddleware);
        files.push(await this.writeFile(this.getMiddlewareTestPath(name), testContent));
      }
    } else {
      for (const middlewareItem of middleware) {
        const middlewareContent = this.generateMiddlewareClass(middlewareItem, framework);
        files.push(await this.writeFile(this.getMiddlewarePath(middlewareItem.name), middlewareContent));

        if (includeTests) {
          const testContent = this.generateMiddlewareTest(middlewareItem);
          files.push(await this.writeFile(this.getMiddlewareTestPath(middlewareItem.name), testContent));
        }
      }
    }
  ...
  private getDIContainerPath(name: string): string {
    return join(this.getDIBasePath(), "container", `${this.kebabCase(name)}.container.ts`);
  }

  private getDIContainerInterfacePath(name: string): string {
    return join(this.getDIBasePath(), "interfaces", `${this.kebabCase(name)}.container.interface.ts`);
  }
  ...
  private getServiceDefinitionPath(serviceName: string): string {
    return join(this.getDIBasePath(), "services", `${this.kebabCase(serviceName)}.service.ts`);
  }

  private getServiceInterfacePath(serviceName: string): string {
    return join(this.getDIBasePath(), "interfaces", `${this.kebabCase(serviceName)}.service.interface.ts`);
  }

  private getProviderPath(providerName: string): string {
    return join(this.getDIBasePath(), "providers", `${this.kebabCase(providerName)}.provider.ts`);
  }

  private getModulePath(moduleName: string): string {
    return join(this.getDIBasePath(), "modules", `${this.kebabCase(moduleName)}.module.ts`);
  }

  private getContainerBuilderPath(name: string): string {
    return join(this.getDIBasePath(), "builders", `${this.kebabCase(name)}.builder.ts`);
  }

  private getServiceLocatorPath(name: string): string {
    return join(this.getDIBasePath(), "locators", `${this.kebabCase(name)}.locator.ts`);
  }

  private getServiceLocatorInterfacePath(name: string): string {
    return join(this.getDIBasePath(), "interfaces", `${this.kebabCase(name)}.locator.interface.ts`);
  }

  private getFactoryPath(name: string): string {
    return join(this.getDIBasePath(), "factories", `${this.kebabCase(name)}.factory.ts`);
  }

  private getFactoryInterfacePath(name: string): string {
    return join(this.getDIBasePath(), "interfaces", `${this.kebabCase(name)}.factory.interface.ts`);
  }

  private getAdapterPath(name: string): string {
    return join(this.getDIBasePath(), "adapters", `${this.kebabCase(name)}.adapter.ts`);
  }

  private getAdapterSourceInterfacePath(name: string): string {
    return join(this.getDIBasePath(), "interfaces", `${this.kebabCase(name)}.source.interface.ts`);
  }

  private getAdapterTargetInterfacePath(name: string): string {
    return join(this.getDIBasePath(), "interfaces", `${this.kebabCase(name)}.target.interface.ts`);
  }

  private getDecoratorPath(name: string): string {
    return join(this.getDIBasePath(), "decorators", `${this.kebabCase(name)}.decorator.ts`);
  }

  private getProxyPath(name: string): string {
    return join(this.getDIBasePath(), "proxies", `${this.kebabCase(name)}.proxy.ts`);
  }

  private getBridgePath(name: string): string {
    return join(this.getDIBasePath(), "bridges", `${this.kebabCase(name)}.bridge.ts`);
  }

  private getFacadePath(name: string): string {
    return join(this.getDIBasePath(), "facades", `${this.kebabCase(name)}.facade.ts`);
  }

  private getRegistryPath(name: string): string {
    return join(this.getDIBasePath(), "registries", `${this.kebabCase(name)}.registry.ts`);
  }

  private getModuleSystemPath(name: string): string {
    return join(this.getDIBasePath(), "systems", `${this.kebabCase(name)}.system.ts`);
  }

  private getInterceptorPath(name: string): string {
    return join(this.getDIBasePath(), "interceptors", `${this.kebabCase(name)}.interceptor.ts`);
  }

  private getMiddlewarePath(name: string): string {
    return join(this.getDIBasePath(), "middleware", `${this.kebabCase(name)}.middleware.ts`);
  }
  ...
  private getContainerTestPath(name: string): string {
    return join(this.getDIBasePath(), "container", `${this.kebabCase(name)}.container.spec.ts`);
  }

  private getServiceTestPath(serviceName: string): string {
    return join(this.getDIBasePath(), "services", `${this.kebabCase(serviceName)}.service.spec.ts`);
  }

  private getServiceLocatorTestPath(name: string): string {
    return join(this.getDIBasePath(), "locators", `${this.kebabCase(name)}.locator.spec.ts`);
  }

  private getFactoryTestPath(name: string): string {
    return join(this.getDIBasePath(), "factories", `${this.kebabCase(name)}.factory.spec.ts`);
  }

  private getProviderTestPath(name: string): string {
    return join(this.getDIBasePath(), "providers", `${this.kebabCase(name)}.provider.spec.ts`);
  }

  private getAdapterTestPath(name: string): string {
    return join(this.getDIBasePath(), "adapters", `${this.kebabCase(name)}.adapter.spec.ts`);
  }

  private getDecoratorTestPath(name: string): string {
    return join(this.getDIBasePath(), "decorators", `${this.kebabCase(name)}.decorator.spec.ts`);
  }

  private getInterceptorTestPath(name: string): string {
    return join(this.getDIBasePath(), "interceptors", `${this.kebabCase(name)}.interceptor.spec.ts`);
  }

  private getMiddlewareTestPath(name: string): string {
    return join(this.getDIBasePath(), "middleware", `${this.kebabCase(name)}.middleware.spec.ts`);
  }
  ...
  private generateDIContainerClass(
    ...
    const servicesCode = services.map(service => 
...
`;
  ...
  private generateDIContainerInterface(name: string, services: readonly ServiceDefinition[]): string {
    return `/**
...
`;
  ...
  private generateServiceRegistry(services: readonly ServiceDefinition[]): string {
...
`;
  ...
  private generateServiceDefinition(service: ServiceDefinition): string {
    const dependenciesCode = service.dependencies.map(dep => 
...
`;
  ...
  private generateModuleClass(module: ModuleDefinition, framework: string): string {
    const imports = module.imports?.map(imp => this.pascalCase(imp)).join(', ') || '';
    const providers = module.providers.map(prov => this.pascalCase(prov)).join(', ');
    const exports = module.exports?.map(exp => this.pascalCase(exp)).join(', ') || '';
    const controllers = module.controllers?.map(ctrl => this.pascalCase(ctrl)).join(', ') || '';
...
`;
  ...
  private generateServiceLocatorClass(name: string, services: readonly ServiceDefinition[]): string {
    return `/**
...
`;
  ...
  private generateFactoryClass(name: string, services: readonly ServiceDefinition[]): string {
    return `/**
 * ${name} Factory
 * Generated by Xaheen CLI - Dependency Injection Generator
 */
...
`;
  ...
  private generateAdapterClass(adapter: AdapterDefinition): string {
    const methodsCode = adapter.methods.map(method => 
...
`;
  ...
  private generateDecoratorClass(decorator: DecoratorDefinition, framework: string): string {
...
`;
  ...
  private generateInterceptorClass(interceptor: InterceptorDefinition, framework: string): string {
    if (framework === "nestjs") {
      return `/**
...
`;
  ...
  private generateMiddlewareClass(middleware: MiddlewareDefinition, framework: string): string {
    if (framework === "nestjs") {
      return `/**
...
`;
  ...
  private generateModuleSystemClass(name: string, modules: readonly ModuleDefinition[]): string {
...
`;
  ...
  private generateContainerTest(name: string, services: readonly ServiceDefinition[]): string {
  ...
  private generateServiceLocatorTest(name: string, services: readonly ServiceDefinition[]): string {
    return `describe('${this.pascalCase(name)} Service Locator', () => {\n  it('should locate services', () => {\n    // TODO: Implement service locator tests\n  });\n});`;
  }

  private generateFactoryTest(name: string, services: readonly ServiceDefinition[]): string {
