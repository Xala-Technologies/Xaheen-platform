...
export const PATTERN_HELP = {
	ddd: {
		description:
			"Generate Domain-Driven Design patterns including bounded contexts, aggregates, entities, domain services, and repositories",
		patterns: [
			"bounded-context",
			"aggregate",
			"entity",
	...
	"clean-architecture": {
		description:
			"Generate Clean Architecture patterns with layered architecture, use cases, and interfaces",
		patterns: [
			"complete-feature",
			"use-case",
			"repository-pattern",
	...
	cqrs: {
		description:
			"Generate CQRS and Event Sourcing patterns including commands, queries, events, and projections",
		patterns: [
			"complete-cqrs",
			"command",
			"query",
	...
	di: {
		description:
			"Generate Dependency Injection and Adapter patterns including containers, services, and decorators",
		patterns: [
			"container",
			"service-locator",
			"factory",
 ...
 */
export function getPatternHelp(
	pattern: string,
): (typeof PATTERN_HELP)[keyof typeof PATTERN_HELP] | null {
	...
	for (const [key, help] of Object.entries(PATTERN_HELP)) {
		if (
			...
			help.patterns.includes(normalizedPattern)
		) {
			return help;
		}
	}
 ...
 */
export function validatePatternOptions(
	pattern: string,
	options: any,
): string[] {
	...
	for (const conflict of rule.conflicts) {
		if (conflict in options && options[conflict]) {
			errors.push(
				`Conflicting option: ${conflict} cannot be used with ${pattern}`,
			);
		}
	}
