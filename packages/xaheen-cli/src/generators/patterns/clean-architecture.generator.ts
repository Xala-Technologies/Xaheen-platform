...
export class CleanArchitectureGenerator {
	...
	constructor(projectPath: string) {
		...
		this.templatesPath = join(
			__dirname,
			"../../templates/patterns/clean-architecture",
		);
	}
	...
	private async generateCompleteFeature(
		...
		files.push(
			await this.writeFile(
				this.getEntityPath(featureName, entityName),
				entityContent,
			),
		);
		...
		for (const useCase of useCases) {
			...
			files.push(
				await this.writeFile(
					this.getUseCasePath(featureName, useCase.name),
					useCaseContent,
				),
			);
			...
			files.push(
				await this.writeFile(
					this.getUseCaseInterfacePath(featureName, useCase.name),
		...
		const repositoryInterfaceContent = this.generateRepositoryInterface(
			entityName,
			fields,
		);
		files.push(
			await this.writeFile(
				this.getRepositoryInterfacePath(featureName, entityName),
				repositoryInterfaceContent,
			),
		);

		// Generate Repository Implementation (Infrastructure Layer)
		const repositoryImplContent = this.generateRepositoryImplementation(
			entityName,
			fields,
			options,
		);
		files.push(
			await this.writeFile(
				this.getRepositoryImplementationPath(featureName, entityName),
				repositoryImplContent,
			),
		);

		// Generate Controller (Interface Adapters Layer)
		const controllerContent = this.generateController(
			entityName,
			useCases,
			options,
		);
		files.push(
			await this.writeFile(
				this.getControllerPath(featureName, entityName),
				controllerContent,
			),
		);
		...
		files.push(
			await this.writeFile(
				this.getCreateDTOPath(featureName, entityName),
				createDtoContent,
			),
		);
		...
		files.push(
			await this.writeFile(
				this.getUpdateDTOPath(featureName, entityName),
				updateDtoContent,
			),
		);
		...
		files.push(
			await this.writeFile(
				this.getResponseDTOPath(featureName, entityName),
				responseDtoContent,
			),
		);
		...
		files.push(
			await this.writeFile(
				this.getMapperPath(featureName, entityName),
				mapperContent,
			),
		);

		// Generate Module (Framework Layer)
		const moduleContent = this.generateFeatureModule(
			featureName,
			entityName,
			useCases,
			options,
		...
		files.push(
			await this.writeFile(this.getModulePath(featureName), moduleContent),
		);
		...
		if (includeTests) {
			for (const useCase of useCases) {
				...
				files.push(
					await this.writeFile(
						this.getUseCaseTestPath(featureName, useCase.name),
						testContent,
					),
				);
			}

			const controllerTestContent = this.generateControllerTest(
				entityName,
				useCases,
			);
			files.push(
				await this.writeFile(
					this.getControllerTestPath(featureName, entityName),
					controllerTestContent,
				),
			);

			const repositoryTestContent = this.generateRepositoryTest(
				entityName,
				fields,
			);
			files.push(
				await this.writeFile(
					this.getRepositoryTestPath(featureName, entityName),
		...
		const archDocContent = this.generateArchitectureDocumentation(
			featureName,
			entityName,
			useCases,
		);
		files.push(
			await this.writeFile(
				this.getArchitectureDocPath(featureName),
				archDocContent,
			),
		);
	...
	private async generateUseCase(
		...
		if (useCases.length === 0) {
			...
			files.push(
				await this.writeFile(
					this.getUseCasePath(featureName, name),
					useCaseContent,
				),
			);

			const useCaseInterfaceContent =
				this.generateUseCaseInterface(basicUseCase);
			files.push(
				await this.writeFile(
					this.getUseCaseInterfacePath(featureName, name),
					useCaseInterfaceContent,
				),
			);
			...
			files.push(
				await this.writeFile(
					this.getUseCaseInputDTOPath(featureName, name),
					inputDtoContent,
				),
			);
			...
			files.push(
				await this.writeFile(
					this.getUseCaseOutputDTOPath(featureName, name),
					outputDtoContent,
				),
			);
			...
			if (includeTests) {
				...
				files.push(
					await this.writeFile(
						this.getUseCaseTestPath(featureName, name),
						testContent,
					),
				);
			}
	...
	private async generateRepositoryPattern(
		...
		const repositoryFactoryContent = this.generateRepositoryFactory(
			entityName,
			options,
		);
		files.push(
			await this.writeFile(
				this.getRepositoryFactoryPath(featureName, entityName),
				repositoryFactoryContent,
			),
		);
	...
	private async generateGatewayPattern(
		...
		const gatewayInterfaceContent = this.generateGatewayInterface(
			name,
			interfaces,
		);
		files.push(
			await this.writeFile(
				this.getGatewayInterfacePath(featureName, name),
				gatewayInterfaceContent,
			),
		);

		// Generate Gateway Implementation
		const gatewayImplContent = this.generateGatewayImplementation(
			name,
			interfaces,
			options,
		);
		files.push(
			await this.writeFile(
				this.getGatewayImplementationPath(featureName, name),
				gatewayImplContent,
			),
		);
		...
		files.push(
			await this.writeFile(
				this.getGatewayConfigPath(featureName, name),
		...
		if (includeTests) {
			...
			files.push(
				await this.writeFile(
					this.getGatewayTestPath(featureName, name),
					gatewayTestContent,
				),
			);
		}
	...
	private async generateAdapterPattern(
		...
		for (const adapter of adapters) {
			...
			files.push(
				await this.writeFile(
					this.getAdapterInterfacePath(featureName, adapter.name),
					adapterInterfaceContent,
				),
			);

			// Generate Adapter Implementation
			const adapterImplContent = this.generateAdapterImplementation(
				adapter,
				options,
			);
			files.push(
				await this.writeFile(
					this.getAdapterImplementationPath(featureName, adapter.name),
					adapterImplContent,
				),
			);
			...
			if (includeTests) {
				...
				files.push(
					await this.writeFile(
						this.getAdapterTestPath(featureName, adapter.name),
						adapterTestContent,
					),
				);
			}
	...
	private async generateDependencyInjection(
		...
		const diConfigContent = this.generateDIConfiguration(
			featureName,
			dependencies,
			options,
		);
		files.push(
			await this.writeFile(this.getDIConfigPath(featureName), diConfigContent),
		);

		// Generate Service Locator
		const serviceLocatorContent = this.generateServiceLocator(
			featureName,
			dependencies,
		);
		files.push(
			await this.writeFile(
				this.getServiceLocatorPath(featureName),
		...
		files.push(
			await this.writeFile(this.getDIFactoryPath(featureName), factoryContent),
		);
	...
	private async generateEntityLayer(
		name: string,
		options: CleanArchitectureGeneratorOptions,
	): Promise<GeneratorResult> {
		...
		files.push(
			await this.writeFile(
				this.getEntityPath(featureName, name),
				entityContent,
			),
		);
		...
		if (includeTests) {
			...
			files.push(
				await this.writeFile(
					this.getEntityTestPath(featureName, name),
					entityTestContent,
				),
			);
		}
	...
	private async generateRepositoryInterfaceLayer(
		name: string,
		options: CleanArchitectureGeneratorOptions,
	): Promise<GeneratorResult> {
	...
	private async generateRepositoryImplementationLayer(
		name: string,
		options: CleanArchitectureGeneratorOptions,
	): Promise<GeneratorResult> {
		const {
	...
	private async generateControllerLayer(
		...
		files.push(
			await this.writeFile(
				this.getControllerPath(featureName, name),
				controllerContent,
			),
		);
		...
		if (includeTests) {
			...
			files.push(
				await this.writeFile(
					this.getControllerTestPath(featureName, name),
					controllerTestContent,
				),
			);
		}
	...
	private async generatePresenterLayer(
		name: string,
		options: CleanArchitectureGeneratorOptions,
	): Promise<GeneratorResult> {
		const { featureName, fields = [], includeTests = true } = options;
		...
		files.push(
			await this.writeFile(
				this.getPresenterPath(featureName, name),
				presenterContent,
			),
		);

		const presenterInterfaceContent = this.generatePresenterInterface(
			name,
			fields,
		);
		files.push(
			await this.writeFile(
				this.getPresenterInterfacePath(featureName, name),
				presenterInterfaceContent,
			),
		);
		...
		if (includeTests) {
			...
			files.push(
				await this.writeFile(
					this.getPresenterTestPath(featureName, name),
					presenterTestContent,
				),
			);
		}
	...
	private async generateGatewayLayer(
		name: string,
		options: CleanArchitectureGeneratorOptions,
	): Promise<GeneratorResult> {
		const { featureName, interfaces = [], includeTests = true } = options;
	...
	private async generateExternalInterfaceLayer(
		name: string,
		options: CleanArchitectureGeneratorOptions,
	): Promise<GeneratorResult> {
		...
		for (const interfaceSpec of interfaces) {
			...
			files.push(
				await this.writeFile(
					this.getExternalInterfacePath(featureName, interfaceSpec.name),
					interfaceContent,
				),
			);
		}
	...
	private async generateDTOLayer(
		name: string,
		options: CleanArchitectureGeneratorOptions,
	): Promise<GeneratorResult> {
		...
		files.push(
			await this.writeFile(
				this.getCreateDTOPath(featureName, name),
				createDtoContent,
			),
		);
		...
		files.push(
			await this.writeFile(
				this.getUpdateDTOPath(featureName, name),
				updateDtoContent,
			),
		);
		...
		files.push(
			await this.writeFile(
				this.getResponseDTOPath(featureName, name),
				responseDtoContent,
			),
		);
	...
	private async generateMapperLayer(
		name: string,
		options: CleanArchitectureGeneratorOptions,
	): Promise<GeneratorResult> {
		const { featureName, fields = [], includeTests = true } = options;
		...
		files.push(
			await this.writeFile(
				this.getMapperPath(featureName, name),
				mapperContent,
			),
		);
		...
		if (includeTests) {
			...
			files.push(
				await this.writeFile(
					this.getMapperTestPath(featureName, name),
					mapperTestContent,
				),
			);
		}
	...
	private getEntityPath(featureName: string, entityName: string): string {
		return join(
			this.getFeaturePath(featureName),
			"domain",
			"entities",
	...
	private getUseCasePath(featureName: string, useCaseName: string): string {
		return join(
			...
			"application",
			"use-cases",
			`${this.kebabCase(useCaseName)}.use-case.ts`,
		);
	}

	private getUseCaseInterfacePath(
		featureName: string,
		useCaseName: string,
	): string {
		return join(
			...
			"application",
			"interfaces",
			`${this.kebabCase(useCaseName)}.interface.ts`,
		);
	}

	private getRepositoryInterfacePath(
		featureName: string,
		entityName: string,
	): string {
		return join(
			...
			"application",
			"interfaces",
			`${entityName.toLowerCase()}.repository.interface.ts`,
		);
	}

	private getRepositoryImplementationPath(
		featureName: string,
		entityName: string,
	): string {
		return join(
	...
	private getCreateDTOPath(featureName: string, entityName: string): string {
		return join(
			...
			"interface-adapters",
			"dto",
			`create-${entityName.toLowerCase()}.dto.ts`,
		);
	}
	...
	private getUpdateDTOPath(featureName: string, entityName: string): string {
		return join(
			...
			"interface-adapters",
			"dto",
			`update-${entityName.toLowerCase()}.dto.ts`,
		);
	}
	...
	private getMapperPath(featureName: string, entityName: string): string {
		return join(
			...
			"interface-adapters",
			"mappers",
			`${entityName.toLowerCase()}.mapper.ts`,
		);
	}
	...
	private getModulePath(featureName: string): string {
		return join(
			this.getFeaturePath(featureName),
			`${featureName.toLowerCase()}.module.ts`,
		);
	}

	private getGatewayInterfacePath(
		featureName: string,
		gatewayName: string,
	): string {
		return join(
			...
			"application",
			"interfaces",
			`${gatewayName.toLowerCase()}.gateway.interface.ts`,
		);
	}

	private getGatewayImplementationPath(
		featureName: string,
		gatewayName: string,
	): string {
		return join(
			...
			"infrastructure",
			"gateways",
			`${gatewayName.toLowerCase()}.gateway.ts`,
		);
	}

	private getGatewayConfigPath(
		featureName: string,
		gatewayName: string,
	): string {
		return join(
			...
			"infrastructure",
			"config",
			`${gatewayName.toLowerCase()}.config.ts`,
		);
	}

	private getAdapterInterfacePath(
		featureName: string,
		adapterName: string,
	): string {
		return join(
			...
			"application",
			"interfaces",
			`${adapterName.toLowerCase()}.adapter.interface.ts`,
		);
	}

	private getAdapterImplementationPath(
		featureName: string,
		adapterName: string,
	): string {
		return join(
			...
			"infrastructure",
			"adapters",
			`${adapterName.toLowerCase()}.adapter.ts`,
		);
	}
	...
	private getPresenterPath(featureName: string, presenterName: string): string {
		return join(
			...
			"interface-adapters",
			"presenters",
			`${presenterName.toLowerCase()}.presenter.ts`,
		);
	}

	private getPresenterInterfacePath(
		featureName: string,
		...
		return join(
			...
			"application",
			"interfaces",
			`${presenterName.toLowerCase()}.presenter.interface.ts`,
		);
	}

	private getExternalInterfacePath(
		featureName: string,
		...
		return join(
			...
			"infrastructure",
			"external",
			`${interfaceName.toLowerCase()}.interface.ts`,
		);
	}
	...
	private getRepositoryFactoryPath(
		featureName: string,
		entityName: string,
	): string {
		return join(
			...
			"infrastructure",
			"factories",
			`${entityName.toLowerCase()}-repository.factory.ts`,
		);
	}

	private getUseCaseInputDTOPath(
		featureName: string,
		useCaseName: string,
	): string {
		return join(
			...
			"application",
			"dto",
			`${this.kebabCase(useCaseName)}-input.dto.ts`,
		);
	}

	private getUseCaseOutputDTOPath(
		featureName: string,
		useCaseName: string,
	): string {
		return join(
			...
			"application",
			"dto",
			`${this.kebabCase(useCaseName)}-output.dto.ts`,
		);
	}
	...
	private getEntityTestPath(featureName: string, entityName: string): string {
		return join(
			...
			"domain",
			"entities",
			`${entityName.toLowerCase()}.entity.spec.ts`,
		);
	}
	...
	private getUseCaseTestPath(featureName: string, useCaseName: string): string {
		return join(
			...
			"application",
			"use-cases",
			`${this.kebabCase(useCaseName)}.use-case.spec.ts`,
		);
	}

	private getRepositoryTestPath(
		featureName: string,
		entityName: string,
	): string {
		return join(
			...
			"infrastructure",
			"repositories",
			`${entityName.toLowerCase()}.repository.spec.ts`,
		);
	}

	private getControllerTestPath(
		featureName: string,
		entityName: string,
	): string {
		return join(
			...
			"interface-adapters",
			"controllers",
			`${entityName.toLowerCase()}.controller.spec.ts`,
		);
	}
	...
	private getGatewayTestPath(featureName: string, gatewayName: string): string {
		return join(
			...
			"infrastructure",
			"gateways",
			`${gatewayName.toLowerCase()}.gateway.spec.ts`,
		);
	}
	...
	private getAdapterTestPath(featureName: string, adapterName: string): string {
		return join(
			...
			"infrastructure",
			"adapters",
			`${adapterName.toLowerCase()}.adapter.spec.ts`,
		);
	}

	private getPresenterTestPath(
		featureName: string,
		presenterName: string,
	): string {
		return join(
			...
			"interface-adapters",
			"presenters",
			`${presenterName.toLowerCase()}.presenter.spec.ts`,
		);
	}
	...
	private getMapperTestPath(featureName: string, entityName: string): string {
		return join(
			...
			"interface-adapters",
			"mappers",
			`${entityName.toLowerCase()}.mapper.spec.ts`,
		);
	}
	 ...
	 * Ensure clean architecture structure exists
	 */
	private async ensureCleanArchitectureStructure(
		featureName: string,
	): Promise<void> {
...
`;
	...
	private generateUseCaseClass(
		useCase: UseCaseSpec,
		entityName: string,
	): string {
		const dependenciesCode = useCase.dependencies
			.map(
				(dep) =>
...
`;
	...
	private generateController(
		...
		const constructorDeps = useCases
			.map(
				(useCase) =>
...
`;
	...
	private generateUseCaseInputDTO(
		name: string,
		fields: readonly CleanArchitectureField[],
	): string {
	...
	private generateUseCaseOutputDTO(
		name: string,
		fields: readonly CleanArchitectureField[],
	): string {
	...
	private generateRepositoryFactory(
		entityName: string,
		options: CleanArchitectureGeneratorOptions,
	): string {
	...
	private generateGatewayInterface(
		name: string,
	...
	private generateGatewayImplementation(
		name: string,
		interfaces: readonly InterfaceSpec[],
		options: CleanArchitectureGeneratorOptions,
	): string {
	...
	private generateAdapterImplementation(
		adapter: AdapterSpec,
		options: CleanArchitectureGeneratorOptions,
	): string {
	...
	private generatePresenterInterface(
		name: string,
		fields: readonly CleanArchitectureField[],
	): string {
	...
	private generateServiceLocator(
		featureName: string,
		dependencies: readonly DependencySpec[],
	): string {
...
`;
	...
	private generateRepositoryTest(
		entityName: string,
		fields: readonly CleanArchitectureField[],
	): string {
	...
	private generateControllerTest(
		entityName: string,
		useCases: readonly UseCaseSpec[],
	): string {
