...
import { SubscriptionManagementOptions, GenerationResult } from './types';
...
export class SubscriptionManagementGenerator extends BaseGenerator<SubscriptionManagementOptions> {
  async generate(options: SubscriptionManagementOptions): Promise<GenerationResult> {
    try {
      await this.validateOptions(options);
      this.logger.info(`Generating Subscription Management System: ${options.name}`);
      ...
      this.logger.success(`Subscription Management System generated successfully: ${options.name}`);
      return result;

    } catch (error: any) {
      this.logger.error(`Failed to generate Subscription Management System: ${error.message}`, error);
      throw error;
    }
  }
  ...
  private async createSubscriptionStructure(
    ...
    const moduleTemplate = await this.loadTemplate('subscription/subscription.module.ts.hbs');
    const moduleContent = moduleTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const modulePath = path.join(baseDir, 'src/subscription/SubscriptionModule.ts');
  ...
  private async generateBillingProviderIntegration(
    ...
    const providerFactoryTemplate = await this.loadTemplate('subscription/billing/billing-provider.factory.ts.hbs');
    const providerFactoryContent = providerFactoryTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const providerFactoryPath = path.join(baseDir, 'src/billing/providers/BillingProviderFactory.ts');
    ...
    const billingServiceTemplate = await this.loadTemplate('subscription/billing/billing.service.ts.hbs');
    const billingServiceContent = billingServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const billingServicePath = path.join(baseDir, 'src/billing/services/BillingService.ts');
  ...
  private async generateStripeIntegration(
    ...
    const stripeServiceTemplate = await this.loadTemplate('subscription/billing/providers/stripe.service.ts.hbs');
    const stripeServiceContent = stripeServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const stripeServicePath = path.join(baseDir, 'src/billing/providers/StripeService.ts');
  ...
  private async generateVippsIntegration(
    ...
    const vippsServiceTemplate = await this.loadTemplate('subscription/billing/providers/vipps.service.ts.hbs');
    const vippsServiceContent = vippsServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const vippsServicePath = path.join(baseDir, 'src/billing/providers/VippsService.ts');
  ...
  private async generatePayPalIntegration(
    ...
    const paypalServiceTemplate = await this.loadTemplate('subscription/billing/providers/paypal.service.ts.hbs');
    const paypalServiceContent = paypalServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const paypalServicePath = path.join(baseDir, 'src/billing/providers/PayPalService.ts');
  ...
  private async generateSubscriptionPlans(
    ...
    const planServiceTemplate = await this.loadTemplate('subscription/services/subscription-plan.service.ts.hbs');
    const planServiceContent = planServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const planServicePath = path.join(baseDir, 'src/subscription/services/SubscriptionPlanService.ts');
    await fs.writeFile(planServicePath, planServiceContent);
    result.files.push(planServicePath);

    // Generate pricing calculator
    const pricingTemplate = await this.loadTemplate('subscription/utils/pricing-calculator.service.ts.hbs');
    const pricingContent = pricingTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const pricingPath = path.join(baseDir, 'src/utils/pricing/PricingCalculator.ts');
  ...
  private async generateBillingCycleManagement(
    ...
    const cycleServiceTemplate = await this.loadTemplate('subscription/services/billing-cycle.service.ts.hbs');
    const cycleServiceContent = cycleServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const cycleServicePath = path.join(baseDir, 'src/subscription/services/BillingCycleService.ts');
    ...
    if (options.prorationHandling) {
      ...
      const prorationTemplate = await this.loadTemplate('subscription/services/proration.service.ts.hbs');
      const prorationContent = prorationTemplate({
        ...options,
        timestamp: new Date().toISOString()
      });

      const prorationPath = path.join(baseDir, 'src/subscription/services/ProrationService.ts');
  ...
  private async generateUsageTracking(
    ...
    if (!options.features.includes('usage-tracking') && !options.features.includes('metering')) return;

    const baseDir = path.join(process.cwd(), 'subscription-system');

    // Generate usage tracking service
    const usageTrackingTemplate = await this.loadTemplate('subscription/usage/usage-tracking.service.ts.hbs');
    const usageTrackingContent = usageTrackingTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const usageTrackingPath = path.join(baseDir, 'src/usage/tracking/UsageTrackingService.ts');
    await fs.writeFile(usageTrackingPath, usageTrackingContent);
    result.files.push(usageTrackingPath);

    // Generate metering service
    const meteringTemplate = await this.loadTemplate('subscription/usage/metering.service.ts.hbs');
    const meteringContent = meteringTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const meteringPath = path.join(baseDir, 'src/usage/metering/MeteringService.ts');
  ...
  private async generateDunningManagement(
    ...
    const dunningTemplate = await this.loadTemplate('subscription/services/dunning.service.ts.hbs');
    const dunningContent = dunningTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const dunningPath = path.join(baseDir, 'src/subscription/services/DunningService.ts');
  ...
  private async generateTaxHandling(
    ...
    const taxTemplate = await this.loadTemplate('subscription/utils/tax-calculator.service.ts.hbs');
  ...
  private async generateWebhookHandlers(
    ...
    const webhookTemplate = await this.loadTemplate('subscription/webhooks/billing-webhook.controller.ts.hbs');
    const webhookContent = webhookTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const webhookPath = path.join(baseDir, 'src/billing/webhooks/BillingWebhookController.ts');
  ...
  private async generateAnalyticsReporting(
    ...
    const analyticsTemplate = await this.loadTemplate('subscription/reporting/subscription-analytics.service.ts.hbs');
    const analyticsContent = analyticsTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const analyticsPath = path.join(baseDir, 'src/reporting/services/SubscriptionAnalyticsService.ts');
    await fs.writeFile(analyticsPath, analyticsContent);
    result.files.push(analyticsPath);

    // Generate report generator
    const reportTemplate = await this.loadTemplate('subscription/reporting/report-generator.service.ts.hbs');
    const reportContent = reportTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const reportPath = path.join(baseDir, 'src/reporting/generators/ReportGenerator.ts');
  ...
  private async generateCustomerPortal(
    ...
    const portalTemplate = await this.loadTemplate('subscription/customer/customer-portal.controller.ts.hbs');
    const portalContent = portalTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const portalPath = path.join(baseDir, 'src/customer/portal/CustomerPortalController.ts');
  ...
  private async generateConfigurations(
    ...
    const configTemplate = await this.loadTemplate('subscription/config/subscription.config.ts.hbs');
    const configContent = configTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const configPath = path.join(baseDir, 'config/subscription/subscription.config.ts');
    await fs.writeFile(configPath, configContent);
    result.files.push(configPath);

    // Generate environment variables
    const envTemplate = await this.loadTemplate('subscription/config/env.example.hbs');
    ...
    result.nextSteps.push('Configure billing provider credentials in .env file');
    result.nextSteps.push('Set up webhook endpoints in your billing provider dashboard');
...
export class SubscriptionService {
  ...
  protected async validateOptions(options: SubscriptionManagementOptions): Promise<void> {
    if (!options.name) {
