...
import { GenerationResult, SingleTenantOptions } from "./types";
...
export class SingleTenantGenerator extends BaseGenerator<SingleTenantOptions> {
	async generate(options: SingleTenantOptions): Promise<GenerationResult> {
		try {
			...
			this.logger.success(
				`Single-Tenant Application generated successfully: ${options.name}`,
			);
			return result;
		} catch (error: any) {
			this.logger.error(
				`Failed to generate Single-Tenant Application: ${error.message}`,
				error,
			);
			throw error;
		}
	...
	private async createSingleTenantStructure(
		...
		const baseDir = path.join(
			process.cwd(),
			`single-tenant-${options.tenantId}`,
		);
		...
		const moduleTemplate = await this.loadTemplate(
			"single-tenant/app.module.ts.hbs",
		);
	...
	private async generateKubernetesInfrastructure(
		...
		const deploymentTemplate = await this.loadTemplate(
			"single-tenant/kubernetes/deployment.yaml.hbs",
		);
		...
		const serviceTemplate = await this.loadTemplate(
			"single-tenant/kubernetes/service.yaml.hbs",
		);
		...
		if (options.features.includes("custom-domain")) {
			const ingressTemplate = await this.loadTemplate(
				"single-tenant/kubernetes/ingress.yaml.hbs",
			);
	...
	private async generateDockerInfrastructure(
		...
		// Generate Dockerfile
		const dockerfileTemplate = await this.loadTemplate(
			"single-tenant/docker/Dockerfile.hbs",
		);
		...
		const composeTemplate = await this.loadTemplate(
			"single-tenant/docker/docker-compose.yaml.hbs",
		);
	...
	private async generateServerlessInfrastructure(
		...
		const serverlessTemplate = await this.loadTemplate(
			"single-tenant/serverless/serverless.yml.hbs",
		);
		...
		const serverlessPath = path.join(
			baseDir,
			"deployments/serverless/serverless.yml",
		);
	...
	private async generateTenantConfiguration(
		...
		const configTemplate = await this.loadTemplate(
			"single-tenant/config/tenant.config.ts.hbs",
		);
		...
		if (options.customization.branding) {
			const brandingTemplate = await this.loadTemplate(
				"single-tenant/customization/branding.config.ts.hbs",
			);
			...
			const brandingPath = path.join(
				baseDir,
				"src/customization/branding/BrandingConfig.ts",
			);
		...
		if (options.customization.themes) {
			const themeTemplate = await this.loadTemplate(
				"single-tenant/customization/theme.config.ts.hbs",
			);
			...
			const themePath = path.join(
				baseDir,
				"src/customization/themes/ThemeConfig.ts",
			);
		...
		if (options.customization.features) {
			const featureTemplate = await this.loadTemplate(
				"single-tenant/customization/feature.config.ts.hbs",
			);
			...
			const featurePath = path.join(
				baseDir,
				"src/customization/features/FeatureConfig.ts",
			);
	...
	private async generateEnhancedSecurity(
		...
		const securityTemplate = await this.loadTemplate(
			"single-tenant/security/enhanced-security.service.ts.hbs",
		);
		...
		const securityPath = path.join(
			baseDir,
			"src/security/tenant/EnhancedSecurityService.ts",
		);
		await fs.writeFile(securityPath, securityContent);
		result.files.push(securityPath);

		// Generate security policies
		const policyTemplate = await this.loadTemplate(
			"single-tenant/security/security-policies.ts.hbs",
		);
		...
		const policyPath = path.join(
			baseDir,
			"src/security/tenant/SecurityPolicies.ts",
		);
	...
	private async generateCustomDomainHandling(
		...
		const domainTemplate = await this.loadTemplate(
			"single-tenant/services/custom-domain.service.ts.hbs",
		);
		...
		const domainPath = path.join(
			baseDir,
			"src/domain/services/CustomDomainService.ts",
		);
	...
	private async generateBackupAndRecovery(
		...
		const backupTemplate = await this.loadTemplate(
			"single-tenant/backup/backup.service.ts.hbs",
		);
		...
		const backupPath = path.join(
			baseDir,
			"src/backup/services/BackupService.ts",
		);
		await fs.writeFile(backupPath, backupContent);
		result.files.push(backupPath);

		// Generate backup configuration
		const backupConfigTemplate = await this.loadTemplate(
			"single-tenant/backup/backup.config.ts.hbs",
		);
	...
	private async generateComplianceFeatures(
		...
		const complianceTemplate = await this.loadTemplate(
			"single-tenant/compliance/compliance.service.ts.hbs",
		);
	...
	private async generateMonitoring(
		...
		const monitoringTemplate = await this.loadTemplate(
			"single-tenant/monitoring/tenant-monitoring.service.ts.hbs",
		);
	...
	private async generateDeploymentConfigurations(
		...
		const deployScriptTemplate = await this.loadTemplate(
			"single-tenant/scripts/deploy.sh.hbs",
		);
		...
		result.nextSteps.push(
			`Configure tenant-specific settings in config/environments/.env.${options.tenantId || "production"}`,
		);
		...
		result.nextSteps.push(
			"Deploy dedicated infrastructure using provided scripts",
		);
		result.nextSteps.push(
