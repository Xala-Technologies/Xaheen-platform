...
import { MultiTenantOptions, GenerationResult } from './types';
...
export class MultiTenantGenerator extends BaseGenerator<MultiTenantOptions> {
  async generate(options: MultiTenantOptions): Promise<GenerationResult> {
    try {
      ...
      this.logger.success(`Multi-Tenant Application generated successfully: ${options.name}`);
      return result;

    } catch (error: any) {
      this.logger.error(`Failed to generate Multi-Tenant Application: ${error.message}`, error);
      throw error;
    }
  }
  ...
  private async createMultiTenantStructure(
    ...
    const moduleTemplate = await this.loadTemplate('multi-tenancy/tenant.module.ts.hbs');
  ...
  private async generateDatabasePerTenant(
    ...
    const dbManagerTemplate = await this.loadTemplate('multi-tenancy/database/database-per-tenant.service.ts.hbs');
    const dbManagerContent = dbManagerTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const dbManagerPath = path.join(baseDir, 'src/database/tenant/DatabasePerTenantService.ts');
    await fs.writeFile(dbManagerPath, dbManagerContent);
    result.files.push(dbManagerPath);

    // Generate tenant database factory
    const dbFactoryTemplate = await this.loadTemplate('multi-tenancy/database/tenant-database.factory.ts.hbs');
    const dbFactoryContent = dbFactoryTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const dbFactoryPath = path.join(baseDir, 'src/database/tenant/TenantDatabaseFactory.ts');
  ...
  private async generateSchemaPerTenant(
    ...
    const schemaServiceTemplate = await this.loadTemplate('multi-tenancy/database/schema-per-tenant.service.ts.hbs');
    const schemaServiceContent = schemaServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const schemaServicePath = path.join(baseDir, 'src/database/tenant/SchemaPerTenantService.ts');
  ...
  private async generateRowLevelSecurity(
    ...
    const rlsServiceTemplate = await this.loadTemplate('multi-tenancy/database/row-level-security.service.ts.hbs');
    const rlsServiceContent = rlsServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const rlsServicePath = path.join(baseDir, 'src/database/tenant/RowLevelSecurityService.ts');
  ...
  private async generateHybridIsolation(
    ...
    const hybridServiceTemplate = await this.loadTemplate('multi-tenancy/database/hybrid-isolation.service.ts.hbs');
    const hybridServiceContent = hybridServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const hybridServicePath = path.join(baseDir, 'src/database/tenant/HybridIsolationService.ts');
  ...
  private async generateTenantMiddleware(
    ...
    const middlewareTemplate = await this.loadTemplate('multi-tenancy/middleware/tenant-context.middleware.ts.hbs');
    const middlewareContent = middlewareTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const middlewarePath = path.join(baseDir, 'src/tenant/middleware/TenantContextMiddleware.ts');
    await fs.writeFile(middlewarePath, middlewareContent);
    result.files.push(middlewarePath);

    // Generate tenant validation guard
    const guardTemplate = await this.loadTemplate('multi-tenancy/guards/tenant-validation.guard.ts.hbs');
    const guardContent = guardTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const guardPath = path.join(baseDir, 'src/tenant/guards/TenantValidationGuard.ts');
  ...
  private async generateTenantAuthentication(
    ...
    const authServiceTemplate = await this.loadTemplate('multi-tenancy/services/tenant-auth.service.ts.hbs');
    const authServiceContent = authServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const authServicePath = path.join(baseDir, 'src/tenant/services/TenantAuthService.ts');
  ...
  private async generateTenantServices(
    ...
    const tenantServiceTemplate = await this.loadTemplate('multi-tenancy/services/tenant-management.service.ts.hbs');
    const tenantServiceContent = tenantServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const tenantServicePath = path.join(baseDir, 'src/tenant/services/TenantManagementService.ts');
  ...
  private async generateTenantDiscovery(
    ...
    const discoveryServiceTemplate = await this.loadTemplate('multi-tenancy/services/tenant-discovery.service.ts.hbs');
    const discoveryServiceContent = discoveryServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const discoveryServicePath = path.join(baseDir, 'src/tenant/services/TenantDiscoveryService.ts');
  ...
  private async generateCachingLayer(
    ...
    const cacheServiceTemplate = await this.loadTemplate('multi-tenancy/cache/tenant-cache.service.ts.hbs');
    const cacheServiceContent = cacheServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const cacheServicePath = path.join(baseDir, 'src/cache/tenant/TenantCacheService.ts');
    await fs.writeFile(cacheServicePath, cacheServiceContent);
    result.files.push(cacheServicePath);

    result.commands.push(`npm install ${options.caching === 'redis' ? 'redis' : 'memcached'}`);
  }
  ...
  private async generateMonitoring(
    ...
    const monitoringServiceTemplate = await this.loadTemplate('multi-tenancy/monitoring/tenant-monitoring.service.ts.hbs');
    const monitoringServiceContent = monitoringServiceTemplate({
      ...options,
      timestamp: new Date().toISOString()
    });

    const monitoringServicePath = path.join(baseDir, 'src/monitoring/tenant/TenantMonitoringService.ts');
  ...
  private async generateConfigurations(
    ...
    const configTemplate = await this.loadTemplate('multi-tenancy/config/tenant.config.ts.hbs');
    ...
    const envTemplate = await this.loadTemplate('multi-tenancy/config/env.example.hbs');
