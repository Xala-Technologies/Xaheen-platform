...
export interface InfrastructureGeneratorOptions {
  ...
  readonly platform: 'docker' | 'kubernetes' | 'github-actions' | 'azure-devops' | 'gitlab-ci' | 'aws' | 'azure' | 'gcp';
...
export interface InfrastructureService {
  ...
  readonly type: 'web' | 'api' | 'database' | 'cache' | 'queue' | 'storage' | 'monitoring';
 ...
 */
function createInfrastructureGenerator(type: InfrastructureGeneratorOptions['type']): InfrastructureGenerator {
  switch (type) {
    case 'docker':
...
export abstract class InfrastructureGenerator {
   ...
   */
  protected generateEnvironmentConfig(environment: string, services: readonly InfrastructureService[]): Record<string, string> {
   ...
   */
  protected generateResourceLimits(environment: string, serviceType: string): ServiceResources {
    const baseResources: Record<string, ServiceResources> = {
      development: {
    ...
    return baseResources[environment]?.[serviceType] || { cpu: '100m', memory: '128Mi' };
  }
...
export class DockerGenerator extends InfrastructureGenerator {
  ...
  private generateDockerfile(service: InfrastructureService, options: InfrastructureGeneratorOptions): GeneratedInfrastructureFile {
...
CMD ["npm", "start"]`;
  ...
  private generateDockerCompose(options: InfrastructureGeneratorOptions): GeneratedInfrastructureFile {
    const services: Record<string, unknown> = {};
    
    for (const service of options.services) {
      services[service.name] = this.generateDockerComposeService(service, options);
    }
  ...
  private generateDockerComposeService(service: InfrastructureService, options: InfrastructureGeneratorOptions): Record<string, unknown> {
    ...
    switch (service.type) {
      ...
      case 'api':
        ...
        serviceConfig.ports = [`${service.port || 3000}:${service.port || 3000}`];
        serviceConfig.environment = this.generateEnvironmentConfig(options.environment, [service]);
        break;
...
export class KubernetesGenerator extends InfrastructureGenerator {
  ...
  private generateNamespace(options: InfrastructureGeneratorOptions): GeneratedInfrastructureFile {
...
metadata:
  ...
  private generateServiceManifests(service: InfrastructureService, options: InfrastructureGeneratorOptions): GeneratedInfrastructureFile[] {
    const files: GeneratedInfrastructureFile[] = [];
  ...
  private generateDeployment(service: InfrastructureService, options: InfrastructureGeneratorOptions): GeneratedInfrastructureFile {
    const resources = service.resources || this.generateResourceLimits(options.environment, service.type);
...
spec:
  ...
  private generateK8sService(service: InfrastructureService, options: InfrastructureGeneratorOptions): GeneratedInfrastructureFile {
    const content = `apiVersion: v1
kind: Service
metadata:
...
export class CIGenerator extends InfrastructureGenerator {
  readonly type = 'ci';
  readonly supportedPlatforms = ['github-actions', 'azure-devops', 'gitlab-ci'] as const;
  ...
  private generateGitHubActions(options: InfrastructureGeneratorOptions): GeneratedInfrastructureFile {
...
jobs:
  ...
  private generateAzureDevOps(options: InfrastructureGeneratorOptions): GeneratedInfrastructureFile {
...
- stage: Build
  ...
  private generateGitLabCI(options: InfrastructureGeneratorOptions): GeneratedInfrastructureFile {
