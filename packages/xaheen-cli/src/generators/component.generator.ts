...
export class ComponentGenerator extends BaseGenerator<ComponentGeneratorOptions> {
	private readonly componentTypes = [
		{
			...
			hint: "Modern React functional component with hooks",
		},
		{
			value: "class",
			label: "Class Component",
			hint: "Traditional React class component",
		},
	];
	...
	private readonly frameworks = [
		{
			value: "react",
			label: "React",
			hint: "React with TypeScript/JavaScript",
		},
		...
		{ value: "angular", label: "Angular", hint: "Angular with TypeScript" },
		{
			value: "svelte",
			label: "Svelte",
			hint: "Svelte with TypeScript support",
		},
	];
	...
	protected async validateOptions(
		...
		if (!/^[A-Za-z][A-Za-z0-9]*$/.test(options.name)) {
			throw new Error(
				"Component name must be alphanumeric and start with a letter",
			);
		}
	}
}
