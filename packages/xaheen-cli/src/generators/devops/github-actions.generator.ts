...
export class GitHubActionsGenerator extends BaseGenerator<GitHubActionsGeneratorOptions> {
	...
	private async generateDependencyCheckWorkflow(
		options: GitHubActionsGeneratorOptions,
	): Promise<void> {
		const workflow = {
	...
	private async generateReleaseWorkflow(
		options: GitHubActionsGeneratorOptions,
	): Promise<void> {
		const workflow = {
			name: "Release",
	...
	private async generateReusableWorkflows(
		options: GitHubActionsGeneratorOptions,
	): Promise<void> {
	...
	private async generateCompositeActions(
		options: GitHubActionsGeneratorOptions,
	): Promise<void> {
		// Setup action
		const setupAction = {
	...
	private async generateWorkflowTemplates(
		options: GitHubActionsGeneratorOptions,
	): Promise<void> {
		const template = {
	...
	private generateGlobalEnvironmentVariables(
		options: GitHubActionsGeneratorOptions,
			...
			steps.push({
				...
				with: {
					...
					key:
						...
						this.getDefaultDependencyPath(options) +
						"') }}",
				},
			});
		}
	...
	private generateDependencyCheckSteps(
		options: GitHubActionsGeneratorOptions,
	): WorkflowStep[] {
		return [
			{
	...
	private generatePerformanceSteps(
		options: GitHubActionsGeneratorOptions,
	): WorkflowStep[] {
		return [
			{
	...
	private generateDeploymentSteps(
		...
		const target = options.deploymentTargets.find(
			(t) => t.environment === environment.name,
		);
		if (target) {
			switch (target.type) {
				case "vercel":
	...
	private generateSetupActionSteps(
		options: GitHubActionsGeneratorOptions,
	): WorkflowStep[] {
		return [
			{
	...
	private generateBuildActionSteps(
		options: GitHubActionsGeneratorOptions,
	): WorkflowStep[] {
		return [
			{
	...
	private getDefaultDependencyPath(
		options: GitHubActionsGeneratorOptions,
	): string {
		const paths = {
	...
	private getDependencyCheckCommand(
		options: GitHubActionsGeneratorOptions,
	): string {
