...
export class DockerGenerator extends BaseGenerator {
    ...
    async generateDockerfile(options, config, projectContext) {
        ...
        await this.templateManager.renderTemplate('devops/docker/Dockerfile.hbs', 'Dockerfile', templateData);
    }
    async generateDockerCompose(options, config) {
        ...
        await this.templateManager.renderTemplate('devops/docker/docker-compose.yml.hbs', 'docker-compose.yml', templateData);
    }
    async generateDockerIgnore(options, config) {
        ...
        await this.templateManager.renderTemplate('devops/docker/dockerignore.hbs', '.dockerignore', templateData);
    }
    async generateDevContainer(options, config) {
        ...
        await this.templateManager.renderTemplate('devops/docker/devcontainer.json.hbs', '.devcontainer/devcontainer.json', devContainerConfig);
    }
    async generateBuildScripts(options, config) {
        ...
        for (const [scriptName, scriptContent] of Object.entries(scripts)) {
            await this.templateManager.renderTemplate('devops/docker/scripts/docker-script.sh.hbs', `scripts/docker-${scriptName}.sh`, {
                scriptName,
                scriptContent,
                executable: true
            });
    ...
    async generateSecurityScanning(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker/security/security-scan.yml.hbs', 'security/docker-security-scan.yml', securityConfig);
        await this.templateManager.renderTemplate('devops/docker/security/trivy-config.yaml.hbs', 'security/trivy-config.yaml', securityConfig);
    }
    async generateHealthCheck(options) {
        const healthCheckScript = this.getHealthCheckScript(options);
        await this.templateManager.renderTemplate('devops/docker/health/healthcheck.sh.hbs', 'docker/healthcheck.sh', {
    ...
    async generatePrometheusConfig(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker/monitoring/prometheus.yml.hbs', 'prometheus.yml', prometheusConfig);
    }
...
`;
    ...
    async generateTracingConfig(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker/tracing/jaeger-config.yml.hbs', 'tracing/jaeger-config.yml', tracingConfig);
        await this.templateManager.renderTemplate('devops/docker/tracing/otel-config.yml.hbs', 'tracing/otel-config.yml', tracingConfig);
    }
    /**
    ...
    async generateLoggingConfig(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker/logging/fluentd.conf.hbs', 'logging/fluentd.conf', loggingConfig);
        await this.templateManager.renderTemplate('devops/docker/logging/logstash.conf.hbs', 'logging/logstash.conf', loggingConfig);
    }
    /**
    ...
    async generateSecretsManagement(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker/secrets/secrets-config.yml.hbs', 'secrets/secrets-config.yml', secretsConfig);
        await this.templateManager.renderTemplate('devops/docker/secrets/docker-secrets.sh.hbs', 'scripts/docker-secrets.sh', {
            ...secretsConfig,
            executable: true
        });
    }
    ...
    async generateVulnerabilityScanning(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker/security/vulnerability-scan.yml.hbs', 'security/vulnerability-scan.yml', scanningConfig);
        await this.templateManager.renderTemplate('devops/docker/security/scan-script.sh.hbs', 'scripts/vulnerability-scan.sh', {
            ...scanningConfig,
    ...
    async generateMultiArchSupport(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker/multiarch/buildx-config.yml.hbs', 'multiarch/buildx-config.yml', multiArchConfig);
        await this.templateManager.renderTemplate('devops/docker/multiarch/build-multiarch.sh.hbs', 'scripts/build-multiarch.sh', {
            ...multiArchConfig,
            executable: true
        });
    }
    ...
    async generateCachingConfig(options, config) {
        ...
        await this.templateManager.renderTemplate('devops/docker/caching/cache-config.yml.hbs', 'caching/cache-config.yml', cachingConfig);
        await this.templateManager.renderTemplate('devops/docker/caching/build-with-cache.sh.hbs', 'scripts/build-with-cache.sh', {
            ...cachingConfig,
            imageName: options.imageName,
            imageTag: options.imageTag,
            executable: true
    ...
    async generateRegistryIntegration(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker/registry/registry-config.yml.hbs', 'registry/registry-config.yml', registryConfig);
        await this.templateManager.renderTemplate('devops/docker/registry/push-to-registry.sh.hbs', 'scripts/push-to-registry.sh', {
            ...registryConfig,
            executable: true
        });
        await this.templateManager.renderTemplate('devops/docker/registry/cleanup-registry.sh.hbs', 'scripts/cleanup-registry.sh', {
            ...registryConfig,
            executable: true
        });
    }
    ...
    async generateObservabilityStack(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker/observability/observability-stack.yml.hbs', 'observability/observability-stack.yml', observabilityConfig);
        await this.templateManager.renderTemplate('devops/docker/observability/prometheus-rules.yml.hbs', 'observability/prometheus-rules.yml', {
            ...observabilityConfig,
            serviceName: options.imageName
        });
        await this.templateManager.renderTemplate('devops/docker/observability/grafana-dashboard.json.hbs', 'observability/grafana-dashboard.json', {
            ...observabilityConfig,
            serviceName: options.imageName
        });
    }
