...
export class GitLabCIGenerator extends BaseGenerator<GitLabCIGeneratorOptions> {
	...
	private async generateAutoDevOpsPipeline(
		options: GitLabCIGeneratorOptions,
	): Promise<void> {
		const autoDevOps = {
	...
	private async generateIncludeTemplates(
		options: GitLabCIGeneratorOptions,
	): Promise<void> {
		// Build template
		const buildTemplate = {
	...
	private async generatePagesConfiguration(
		options: GitLabCIGeneratorOptions,
	): Promise<void> {
		const pagesJob = {
			pages: {
	...
	private async generateVariablesTemplate(
		options: GitLabCIGeneratorOptions,
	): Promise<void> {
		const variables = {
			// CI/CD variables
	...
	private getPagesScript(options: GitLabCIGeneratorOptions): readonly string[] {
		const scripts = {
			web: ["npm ci", "npm run build", "cp -r dist/* public/"],
			api: ["npm ci", "npm run docs", "cp -r docs/* public/"],
		};

		return (
			scripts[options.projectType] ||
			([
				"mkdir public",
