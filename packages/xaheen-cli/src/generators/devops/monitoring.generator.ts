...
export class MonitoringGenerator extends BaseGenerator<MonitoringGeneratorOptions> {
	...
	private async validateOptions(
		options: MonitoringGeneratorOptions,
	): Promise<void> {
	...
	private async createMonitoringStructure(
		options: MonitoringGeneratorOptions,
	): Promise<void> {
		const directories = [
			"monitoring",
						...
						containers: [
							{
								...
								securityContext: {
									allowPrivilegeEscalation:
										options.security.allowPrivilegeEscalation,
									readOnlyRootFilesystem:
										options.security.readOnlyRootFilesystem,
									capabilities: {
										...
										queries: [
											{
												name: "Sample query",
												query:
													"sum(rate(traces_spanmetrics_latency_bucket[$__interval]))",
											},
										],
									},
									serviceMap: {
								...
								env: [
									...
									},
									{
										...
										valueFrom: options.authentication.grafana.secretName
											...
											: {
													value:
														options.authentication.grafana.adminPassword ||
														"admin",
												},
									},
									// GDPR compliance
	...
	private async generateLokiConfig(
		options: MonitoringGeneratorOptions,
	): Promise<void> {
	...
	private async generateAlertManagerConfig(
		options: MonitoringGeneratorOptions,
	): Promise<void> {
	...
	private async generateDashboards(
		options: MonitoringGeneratorOptions,
	): Promise<void> {
	...
	private async generateMonitoringResources(
		options: MonitoringGeneratorOptions,
	): Promise<void> {
	...
	private async generateExporters(
		options: MonitoringGeneratorOptions,
	): Promise<void> {
	...
	private async generateNetworkPolicies(
		options: MonitoringGeneratorOptions,
	): Promise<void> {
		const networkPolicy = {
	...
	private async generateHelmChart(
		...
		const helmChart = {
			...
			description:
				"Comprehensive monitoring stack with Prometheus 3.0, Grafana LGTM, and OpenTelemetry",
			type: "application",
			version: "1.0.0",
			appVersion: "1.0.0",
			keywords: [
					...
					grafana: {
						...
						environment: {
							...
							GF_SECURITY_ADMIN_PASSWORD:
								options.authentication.grafana.adminPassword || "admin",
						},
						volumes: [
	...
	private async generatePrometheusOperator(
		options: MonitoringGeneratorOptions,
	): Promise<void> {
