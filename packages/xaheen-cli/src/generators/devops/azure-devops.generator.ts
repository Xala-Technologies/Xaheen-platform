...
export class AzureDevOpsGenerator extends BaseGenerator<AzureDevOpsGeneratorOptions> {
  ...
  private async validateOptions(options: AzureDevOpsGeneratorOptions): Promise<void> {
  ...
  private async generateCDPipeline(options: AzureDevOpsGeneratorOptions): Promise<void> {
    const pipeline = {
  ...
  private async generateSecurityPipeline(options: AzureDevOpsGeneratorOptions): Promise<void> {
    const pipeline = {
  ...
  private async generateVariableTemplates(options: AzureDevOpsGeneratorOptions): Promise<void> {
  ...
  private async generateJobTemplates(options: AzureDevOpsGeneratorOptions): Promise<void> {
    // Build job template
    const buildJob = {
      parameters: {
        vmImage: {
  ...
  private async generateStageTemplates(options: AzureDevOpsGeneratorOptions): Promise<void> {
    // CI stage template
    const ciStage = {
      parameters: {
        vmImage: {
  ...
  private async generateDeploymentTemplates(options: AzureDevOpsGeneratorOptions): Promise<void> {
  ...
  private async generateARMTemplates(options: AzureDevOpsGeneratorOptions): Promise<void> {
  ...
  private generateSecurityVariables(options: AzureDevOpsGeneratorOptions): any[] {
    return [
      { name: 'securityScanResults', value: '$(Agent.TempDirectory)/security-results' }
    ];
  }
    ...
    if (options.enableCD) {
      for (const env of options.environments) {
        stages.push({
          ...
          dependsOn: 'CI',
          condition: env.conditions.length > 0 ? env.conditions.join(' and ') : 'succeeded()',
          jobs: [
            {
              deployment: 'Deploy',
              displayName: `Deploy to ${env.displayName}`,
  ...
  private async generateDeploymentStages(options: AzureDevOpsGeneratorOptions): Promise<any[]> {
  ...
  private generateBuildSteps(options: AzureDevOpsGeneratorOptions): PipelineStep[] {
  ...
  private generateTestSteps(options: AzureDevOpsGeneratorOptions): PipelineStep[] {
    const steps: PipelineStep[] = [];
  ...
  private generateSecuritySteps(options: AzureDevOpsGeneratorOptions): PipelineStep[] {
    const steps: PipelineStep[] = [];
  ...
  private generateDeploymentSteps(options: AzureDevOpsGeneratorOptions): PipelineStep[] {
    return [
      {
  ...
  private generateDeploymentStrategy(env: AzureEnvironment): any {
    const strategies = {
      ...
      canary: {
        ...
        postRouteTraffic: {
          steps: [
            { script: 'echo "Post-route traffic validation"' }
          ]
        }
      }
    };
  ...
  private getTestCommand(options: AzureDevOpsGeneratorOptions, framework: string): string {
    const commands = {
