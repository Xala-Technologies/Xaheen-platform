...
export class KubernetesGenerator extends BaseGenerator {
    templateManager;
    analyzer;
    constructor() {
        super();
    ...
    async generateManifests(options) {
        const commonLabels = {
            app: options.appName,
    ...
    createDeploymentManifest(options, labels, annotations) {
        return {
            apiVersion: 'apps/v1',
            kind: 'Deployment',
            metadata: {
    ...
    createServiceManifest(options, labels) {
        return {
            apiVersion: 'v1',
            kind: 'Service',
            metadata: {
    ...
    createIngressManifest(options, labels) {
        return {
    ...
    createHPAManifest(options, labels) {
        return {
    ...
    createConfigMapManifest(options, labels) {
        return {
            apiVersion: 'v1',
            kind: 'ConfigMap',
            metadata: {
    ...
    createSecretsManifest(options, labels) {
        return {
            apiVersion: 'v1',
            kind: 'Secret',
            metadata: {
    ...
    createNetworkPolicyManifest(options, labels) {
        return {
            apiVersion: 'networking.k8s.io/v1',
            kind: 'NetworkPolicy',
            metadata: {
    ...
    createPodSecurityPolicyManifest(options) {
        return {
    ...
    createServiceAccountManifest(options, labels) {
        return {
            apiVersion: 'v1',
            kind: 'ServiceAccount',
            metadata: {
    ...
    createRBACManifest(options, labels) {
        return {
            role: {
    ...
    async generateNamespace(options) {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/namespace.yaml.hbs', `k8s/${options.namespace}-namespace.yaml`, namespaceManifest);
    }
    async generateDeployment(options, manifests, projectContext) {
        await this.templateManager.renderTemplate('devops/kubernetes/deployment.yaml.hbs', `k8s/${options.appName}-deployment.yaml`, {
            manifest: manifests.deployment,
            options,
            projectContext
        });
    }
    async generateService(options, manifests) {
        await this.templateManager.renderTemplate('devops/kubernetes/service.yaml.hbs', `k8s/${options.appName}-service.yaml`, {
            manifest: manifests.service,
            options
        });
    }
    async generateIngress(options, manifests) {
        if (!manifests.ingress)
            return;
        await this.templateManager.renderTemplate('devops/kubernetes/ingress.yaml.hbs', `k8s/${options.appName}-ingress.yaml`, {
            manifest: manifests.ingress,
            options
        });
    }
    async generateHPA(options, manifests) {
        if (!manifests.hpa)
            return;
        await this.templateManager.renderTemplate('devops/kubernetes/hpa.yaml.hbs', `k8s/${options.appName}-hpa.yaml`, {
            manifest: manifests.hpa,
            options
        });
    }
    async generateConfigMap(options, manifests) {
        if (!manifests.configMap)
            return;
        await this.templateManager.renderTemplate('devops/kubernetes/configmap.yaml.hbs', `k8s/${options.appName}-configmap.yaml`, {
            manifest: manifests.configMap,
            options
        });
    }
    async generateSecrets(options, manifests) {
        if (!manifests.secrets)
            return;
        await this.templateManager.renderTemplate('devops/kubernetes/secrets.yaml.hbs', `k8s/${options.appName}-secrets.yaml`, {
            manifest: manifests.secrets,
            options
        });
    }
    async generateNetworkPolicy(options) {
        const networkPolicy = this.createNetworkPolicyManifest(options, {});
        await this.templateManager.renderTemplate('devops/kubernetes/network-policy.yaml.hbs', `k8s/${options.appName}-network-policy.yaml`, {
            manifest: networkPolicy,
            options
        });
    }
    async generatePodSecurityPolicy(options) {
        const psp = this.createPodSecurityPolicyManifest(options);
        await this.templateManager.renderTemplate('devops/kubernetes/pod-security-policy.yaml.hbs', `k8s/${options.appName}-psp.yaml`, {
            manifest: psp,
            options
        });
    }
    async generateServiceMesh(options) {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/istio/virtual-service.yaml.hbs', `k8s/istio/${options.appName}-virtual-service.yaml`, { manifest: virtualService, options });
        await this.templateManager.renderTemplate('devops/kubernetes/istio/destination-rule.yaml.hbs', `k8s/istio/${options.appName}-destination-rule.yaml`, { manifest: destinationRule, options });
    }
    async generateMonitoring(options) {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/monitoring/service-monitor.yaml.hbs', `k8s/monitoring/${options.appName}-service-monitor.yaml`, { manifest: serviceMonitor, options });
    }
    async generateLogging(options) {
        const fluentdConfig = {
            apiVersion: 'v1',
            kind: 'ConfigMap',
            metadata: {
...
<match kubernetes.**>
  ...
  index_name ${options.appName}
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/logging/fluentd-config.yaml.hbs', `k8s/logging/${options.appName}-fluentd-config.yaml`, { manifest: fluentdConfig, options });
    }
    async generateHelmChart(options, manifests) {
        // Generate Chart.yaml
        const chartYaml = {
            apiVersion: 'v2',
            name: options.appName,
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/helm/Chart.yaml.hbs', `helm/${options.appName}/Chart.yaml`, chartYaml);
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/helm/values.yaml.hbs', `helm/${options.appName}/values.yaml`, valuesYaml);
        // Generate templates
        const templateData = { manifests, options };
        await this.templateManager.renderTemplate('devops/kubernetes/helm/templates/deployment.yaml.hbs', `helm/${options.appName}/templates/deployment.yaml`, templateData);
        await this.templateManager.renderTemplate('devops/kubernetes/helm/templates/service.yaml.hbs', `helm/${options.appName}/templates/service.yaml`, templateData);
    }
    async generateKustomization(options) {
        const kustomization = {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/kustomization.yaml.hbs', 'k8s/kustomization.yaml', kustomization);
    }
...
`;
        await this.templateManager.renderTemplate('devops/kubernetes/scripts/deploy.sh.hbs', 'scripts/k8s-deploy.sh', { script: deployScript, executable: true });
        await this.templateManager.renderTemplate('devops/kubernetes/scripts/undeploy.sh.hbs', 'scripts/k8s-undeploy.sh', { script: undeployScript, executable: true });
    }
     ...
     */
    async generateVPA(options, manifests) {
        if (!manifests.vpa)
            return;
        await this.templateManager.renderTemplate('devops/kubernetes/vpa.yaml.hbs', `k8s/${options.appName}-vpa.yaml`, {
     ...
     */
    async generatePDB(options, manifests) {
        if (!manifests.pdb)
            return;
        await this.templateManager.renderTemplate('devops/kubernetes/pdb.yaml.hbs', `k8s/${options.appName}-pdb.yaml`, {
     ...
     */
    async generateKedaScaler(options, manifests) {
        if (!manifests.kedaScaler)
            return;
        await this.templateManager.renderTemplate('devops/kubernetes/keda/scaledobject.yaml.hbs', `k8s/keda/${options.appName}-scaledobject.yaml`, {
     ...
     */
    async generateCertManager(options) {
        const certificate = {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/cert-manager/certificate.yaml.hbs', `k8s/cert-manager/${options.appName}-certificate.yaml`, { manifest: certificate, options });
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/cert-manager/cluster-issuer.yaml.hbs', `k8s/cert-manager/cluster-issuer.yaml`, { manifest: clusterIssuer, options });
    }
    /**
     * Generate External Secrets configuration
     */
    async generateExternalSecrets(options) {
        const secretStore = {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/external-secrets/secret-store.yaml.hbs', `k8s/external-secrets/${options.appName}-secret-store.yaml`, { manifest: secretStore, options });
        await this.templateManager.renderTemplate('devops/kubernetes/external-secrets/external-secret.yaml.hbs', `k8s/external-secrets/${options.appName}-external-secret.yaml`, { manifest: externalSecret, options });
    }
    /**
     * Generate ArgoCD Application
     */
    async generateArgoApplication(options, manifests) {
        if (!manifests.argoApplication)
            return;
        await this.templateManager.renderTemplate('devops/kubernetes/argocd/application.yaml.hbs', `argocd/${options.appName}-application.yaml`, {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/argocd/appproject.yaml.hbs', `argocd/${options.appName}-appproject.yaml`, { manifest: appProject, options });
    }
    /**
     * Generate FluxCD Kustomization
     */
    async generateFluxKustomization(options, manifests) {
        if (!manifests.fluxKustomization)
            return;
        await this.templateManager.renderTemplate('devops/kubernetes/flux/kustomization.yaml.hbs', `flux/${options.appName}-kustomization.yaml`, {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/flux/gitrepository.yaml.hbs', `flux/${options.appName}-gitrepository.yaml`, { manifest: gitRepository, options });
    }
    /**
    ...
    async generateTracingConfig(options) {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/tracing/jaeger.yaml.hbs', `k8s/tracing/jaeger.yaml`, { manifest: jaegerConfig, options });
  ...
  pipelines:
    traces:
      ...
      exporters: [jaeger]
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/tracing/otel-collector.yaml.hbs', `k8s/tracing/otel-collector.yaml`, { manifest: otelCollector, options });
    }
    /**
     * Generate multi-cluster configuration
     */
    async generateMultiClusterConfig(options) {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/multicluster/multicluster-service.yaml.hbs', `k8s/multicluster/${options.appName}-multicluster-service.yaml`, { manifest: admiraltyConfig, options });
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/multicluster/cluster-role.yaml.hbs', `k8s/multicluster/cluster-role.yaml`, { manifest: clusterRole, options });
    }
    /**
     * Generate GitOps workflows
     */
    async generateGitOpsWorkflows(options) {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/gitops/github-workflow.yml.hbs', `.github/workflows/${options.appName}-gitops.yml`, workflowConfig);
        await this.templateManager.renderTemplate('devops/kubernetes/gitops/gitlab-ci.yml.hbs', `.gitlab-ci.yml`, workflowConfig);
    }
    /**
     * Generate security policies
     */
    async generateSecurityPolicies(options) {
...
violation[{"msg": msg}] {
  ...
  msg := sprintf("Missing required label: %v", [missing])
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/security/constraint-template.yaml.hbs', `k8s/security/constraint-template.yaml`, { manifest: constraintTemplate, options });
...
- rule: Suspicious Activity in ${options.appName}
  ...
  tags: [container, ${options.appName}]
      `
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/security/falco-rules.yaml.hbs', `k8s/security/falco-rules.yaml`, falcoRules);
    }
    /**
     * Generate disaster recovery configuration
     */
    async generateDisasterRecovery(options) {
        const veleroConfig = {
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/disaster-recovery/velero-schedule.yaml.hbs', `k8s/disaster-recovery/${options.appName}-backup-schedule.yaml`, { manifest: veleroConfig, options });
        ...
        await this.templateManager.renderTemplate('devops/kubernetes/disaster-recovery/runbook.md.hbs', `docs/disaster-recovery-runbook.md`, runbook);
    }
    // Create the new manifest methods
    createVPAManifest(options, labels) {
        return {
    ...
    createPDBManifest(options, labels) {
        return {
    ...
    createPVCManifest(options, labels) {
        return {
            apiVersion: 'v1',
    ...
    createKedaScalerManifest(options, labels) {
        return {
    ...
    createCertManagerManifest(options, labels) {
        return {
            apiVersion: 'cert-manager.io/v1',
            kind: 'Certificate',
            metadata: {
    ...
    createExternalSecretsManifest(options, labels) {
        return {
            apiVersion: 'external-secrets.io/v1beta1',
            kind: 'ExternalSecret',
            metadata: {
    ...
    createArgoApplicationManifest(options, labels) {
        return {
            apiVersion: 'argoproj.io/v1alpha1',
            kind: 'Application',
            metadata: {
    ...
    createFluxKustomizationManifest(options, labels) {
        return {
...
//# sourceMappingURL=kubernetes.generator.js.map