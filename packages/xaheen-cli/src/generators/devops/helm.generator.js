...
export class HelmGenerator extends BaseGenerator {
    ...
    async createChartStructure(options) {
        ...
        for (const dir of directories) {
            await this.templateManager.renderTemplate('devops/helm/structure/.gitkeep.hbs', `${dir}/.gitkeep`, {});
        }
    }
    async generateChartYaml(options) {
        ...
        await this.templateManager.renderTemplate('devops/helm/Chart.yaml.hbs', `helm/${options.chartName}/Chart.yaml`, chartData);
    }
    async generateValuesYaml(options, projectContext) {
        const values = {
            ...
            securityContext: options.security?.enabled ? {
                allowPrivilegeEscalation: options.security.allowPrivilegeEscalation ?? false,
                readOnlyRootFilesystem: options.security.readOnlyRootFilesystem ?? true,
                runAsNonRoot: options.security.runAsNonRoot ?? true,
                runAsUser: options.security.runAsUser ?? 1000,
                capabilities: {
            ...
            autoscaling: {
                ...
                targetCPUUtilizationPercentage: options.autoscaling?.targetCPUUtilizationPercentage ?? 80,
        ...
        await this.templateManager.renderTemplate('devops/helm/values.yaml.hbs', `helm/${options.chartName}/values.yaml`, values);
    }
    async generateValuesSchema(options) {
        ...
        await this.templateManager.renderTemplate('devops/helm/values.schema.json.hbs', `helm/${options.chartName}/values.schema.json`, schema);
    }
    async generateTemplates(options) {
        ...
        for (const template of templates) {
            await this.templateManager.renderTemplate(`devops/helm/templates/${template}.hbs`, `helm/${options.chartName}/templates/${template}`, {
                options,
        ...
        await this.templateManager.renderTemplate('devops/helm/templates/rbac.yaml.hbs', `helm/${options.chartName}/templates/rbac.yaml`, { options });
        // Generate Network Policy if needed
        await this.templateManager.renderTemplate('devops/helm/templates/networkpolicy.yaml.hbs', `helm/${options.chartName}/templates/networkpolicy.yaml`, { options });
        // Generate Pod Disruption Budget
        await this.templateManager.renderTemplate('devops/helm/templates/poddisruptionbudget.yaml.hbs', `helm/${options.chartName}/templates/poddisruptionbudget.yaml`, { options });
    }
...
{{- end }}`
        ...
        await this.templateManager.renderTemplate('devops/helm/templates/_helpers.tpl.hbs', `helm/${options.chartName}/templates/_helpers.tpl`, helpers);
    }
    async generateNotes(options) {
        const notes = {
            ...
            ingressHost: options.ingress?.hosts?.[0]?.host || `${options.chartName}.example.com`,
            monitoringEnabled: options.monitoring?.enabled ?? false
        };
        await this.templateManager.renderTemplate('devops/helm/templates/NOTES.txt.hbs', `helm/${options.chartName}/templates/NOTES.txt`, notes);
    }
    async generateHooks(options) {
        ...
        for (const hook of options.hooks) {
            ...
            await this.templateManager.renderTemplate('devops/helm/templates/hooks/hook.yaml.hbs', `helm/${options.chartName}/templates/hooks/${hook.name}.yaml`, hookData);
        }
    }
    async generateTests(options) {
        ...
        for (const test of options.tests) {
            ...
            await this.templateManager.renderTemplate('devops/helm/templates/tests/test.yaml.hbs', `helm/${options.chartName}/templates/tests/${test.name}.yaml`, testData);
        }
        ...
        await this.templateManager.renderTemplate('devops/helm/templates/tests/test.yaml.hbs', `helm/${options.chartName}/templates/tests/connection-test.yaml`, connectionTest);
    }
    async generateEnvironmentValues(options) {
        const environments = ['development', 'staging', 'production'];
        for (const env of environments) {
            const envValues = this.getEnvironmentSpecificValues(options, env);
            await this.templateManager.renderTemplate('devops/helm/values/values-env.yaml.hbs', `helm/${options.chartName}/values/values-${env}.yaml`, {
                environment: env,
                values: envValues
            });
        }
    ...
    async generateDeploymentScripts(options) {
        ...
        for (const [scriptName, scriptContent] of Object.entries(scripts)) {
            await this.templateManager.renderTemplate('devops/helm/scripts/script.sh.hbs', `helm/${options.chartName}/scripts/${scriptName}.sh`, {
                scriptName,
                scriptContent,
                executable: true,
                chartName: options.chartName
    ...
    async generateCIPipelines(options) {
        ...
        await this.templateManager.renderTemplate('devops/helm/ci/github-workflow.yml.hbs', `.github/workflows/${options.chartName}-helm.yml`, githubWorkflow);
        // Generate GitLab CI
        const gitlabCI = {
        ...
        await this.templateManager.renderTemplate('devops/helm/ci/gitlab-ci.yml.hbs', `.gitlab-ci.yml`, gitlabCI);
    }
    async generateMonitoring(options) {
        ...
        await this.templateManager.renderTemplate('devops/helm/monitoring/grafana-dashboard.json.hbs', `helm/${options.chartName}/dashboards/${options.chartName}-dashboard.json`, dashboard);
        ...
        await this.templateManager.renderTemplate('devops/helm/monitoring/prometheus-rules.yaml.hbs', `helm/${options.chartName}/rules/${options.chartName}-rules.yaml`, prometheusRules);
    }
    async generateBackup(options) {
        ...
        await this.templateManager.renderTemplate('devops/helm/backup/velero-schedule.yaml.hbs', `helm/${options.chartName}/backup/velero-schedule.yaml`, { manifest: backupConfig, options });
    }
    async generateSecurity(options) {
        ...
        const psp = {
            ...
            spec: {
                privileged: false,
                allowPrivilegeEscalation: options.security.allowPrivilegeEscalation ?? false,
                requiredDropCapabilities: options.security.capabilities?.drop ?? ['ALL'],
        ...
        await this.templateManager.renderTemplate('devops/helm/security/pod-security-policy.yaml.hbs', `helm/${options.chartName}/security/pod-security-policy.yaml`, { manifest: psp, options });
        ...
        await this.templateManager.renderTemplate('devops/helm/security/network-policy.yaml.hbs', `helm/${options.chartName}/security/network-policy.yaml`, { manifest: networkPolicy, options });
    }
    async generateReadme(options) {
        ...
        await this.templateManager.renderTemplate('devops/helm/README.md.hbs', `helm/${options.chartName}/README.md`, readme);
    }
    // Helper methods
