...
export class DockerComposeGenerator extends BaseGenerator {
    ...
    async generate(options) {
        try {
            ...
            this.logger.success('Docker Compose configuration generated successfully');
        }
        catch (error) {
            this.logger.error('Failed to generate Docker Compose configuration', error);
            throw error;
        }
    }
    async validateOptions(options) {
        ...
        for (const service of options.services) {
            if (service.dependsOn) {
                const deps = Array.isArray(service.dependsOn) ? service.dependsOn : Object.keys(service.dependsOn);
    ...
    async generateMainCompose(options, projectContext) {
        const composeData = {
        ...
        await this.templateManager.renderTemplate('devops/docker-compose/docker-compose.yml.hbs', 'docker-compose.yml', composeData);
    }
    async generateEnvironmentOverrides(options) {
        const environments = ['development', 'staging', 'production'];
        for (const env of environments) {
            const overrideData = {
                version: options.version,
                services: this.getEnvironmentServiceOverrides(options.services, env),
                networks: env === 'production' ? this.getProductionNetworks() : undefined,
                environment: env
            };
            await this.templateManager.renderTemplate('devops/docker-compose/docker-compose.override.yml.hbs', `docker-compose.${env}.yml`, overrideData);
        }
    }
    async generateDevelopmentCompose(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker-compose/docker-compose.dev.yml.hbs', 'docker-compose.dev.yml', devData);
    }
    async generateProductionCompose(options) {
        const prodData = {
            version: options.version,
        ...
        await this.templateManager.renderTemplate('devops/docker-compose/docker-compose.prod.yml.hbs', 'docker-compose.prod.yml', prodData);
    }
    async generateTraefikConfig(options) {
        const traefikConfig = {
            api: {
                dashboard: true,
        ...
        await this.templateManager.renderTemplate('devops/docker-compose/traefik/traefik.yml.hbs', 'traefik/traefik.yml', traefikConfig);
        ...
        await this.templateManager.renderTemplate('devops/docker-compose/traefik/docker-compose.traefik.yml.hbs', 'docker-compose.traefik.yml', traefikService);
    }
    async generateObservabilityStack(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker-compose/observability/docker-compose.observability.yml.hbs', 'docker-compose.observability.yml', observabilityStack);
        ...
        const prometheusConfig = {
            ...
            scrape_configs: options.services.map(service => ({
                ...
                static_configs: [{
                        targets: [`${service.name}:${this.extractPort(service.ports?.[0]) || 3000}`]
                    }],
        ...
        await this.templateManager.renderTemplate('devops/docker-compose/observability/prometheus.yml.hbs', 'observability/prometheus.yml', prometheusConfig);
    }
    async generateLoggingStack(options) {
        const loggingServices = {
            ...
            fluentd: {
                ...
                volumes: [
                    './logging/fluentd:/fluentd/etc:ro'
                ],
                ports: ['24224:24224'],
                networks: ['logging'],
                dependsOn: ['elasticsearch']
        ...
        await this.templateManager.renderTemplate('devops/docker-compose/logging/docker-compose.logging.yml.hbs', 'docker-compose.logging.yml', loggingStack);
    }
    async generateBackupConfig(options) {
        const backupConfig = {
            services: {
                backup: {
                    image: 'alpine:latest',
        ...
        await this.templateManager.renderTemplate('devops/docker-compose/backup/docker-compose.backup.yml.hbs', 'docker-compose.backup.yml', backupConfig);
...
cleanup_old_backups
`;
        await this.templateManager.renderTemplate('devops/docker-compose/backup/backup-script.sh.hbs', 'backup/backup-script.sh', { script: backupScript, executable: true });
    }
    async generateSSLConfig(options) {
        ...
        await this.templateManager.renderTemplate('devops/docker-compose/ssl/docker-compose.ssl.yml.hbs', 'docker-compose.ssl.yml', { services: sslConfig, volumes: { letsencrypt: {}, certbot_www: {} } });
    }
    async generateHelperScripts(options) {
        const scripts = {
        ...
        for (const [scriptName, scriptContent] of Object.entries(scripts)) {
            await this.templateManager.renderTemplate('devops/docker-compose/scripts/script.sh.hbs', `scripts/${scriptName}.sh`, {
                scriptName,
                scriptContent,
                executable: true
            });
    ...
    async generateEnvironmentFiles(options) {
        const environments = ['development', 'staging', 'production'];
        for (const env of environments) {
            const envVars = this.getEnvironmentVariables(options, env);
            await this.templateManager.renderTemplate('devops/docker-compose/env/env.hbs', `.env.${env}`, { variables: envVars, environment: env });
        }
        // Generate .env.example
        const exampleVars = this.getExampleEnvironmentVariables(options);
        await this.templateManager.renderTemplate('devops/docker-compose/env/env.example.hbs', '.env.example', { variables: exampleVars });
    }
    async generateSwarmConfig(options) {
        const swarmConfig = {
            version: options.version,
        ...
        await this.templateManager.renderTemplate('devops/docker-compose/swarm/docker-compose.swarm.yml.hbs', 'docker-compose.swarm.yml', swarmConfig);
...
`;
        await this.templateManager.renderTemplate('devops/docker-compose/swarm/deploy-swarm.sh.hbs', 'scripts/deploy-swarm.sh', { script: deployScript, executable: true });
    }
    ...
    getEnvironmentServiceOverrides(services, env) {
    ...
    getProductionServices(services) {
        return services.reduce((acc, service) => {
            acc[service.name] = {
                ...service,
                restart: 'unless-stopped',
    ...
    extractPort(portMapping) {
        if (!portMapping)
            return undefined;
        const match = portMapping.match(/(\d+):/);
        return match ? parseInt(match[1], 10) : undefined;
    }
    generateStartScript(options) {
        return `#!/bin/bash
set -e
...
`;
    ...
    getEnvironmentVariables(options, env) {
        const baseVars = {
    ...
    getExampleEnvironmentVariables(options) {
        return {
            PROJECT_NAME: options.projectName,
...
//# sourceMappingURL=docker-compose.generator.js.map