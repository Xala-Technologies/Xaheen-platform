...
import type { UnifiedConfig, XaheenConfig, XalaConfig, MigrationResult } from '../../types/index.js';
import { UnifiedConfigSchema } from '../../types/index.js';
...
export class UnifiedConfigManager {
  ...
  public async loadConfig(): Promise<UnifiedConfig> {
    ...
    if (await fs.pathExists(this.configPaths.unified)) {
      logger.debug('Found unified configuration file');
    ...
    if (migrationResult.success) {
      logger.info(`Successfully migrated from ${migrationResult.source} to unified config`);
   ...
   */
  public async saveConfig(config: UnifiedConfig): Promise<void> {
    try {
      // Validate configuration before saving
      const validatedConfig = UnifiedConfigSchema.parse(config);
      
      // Ensure directory exists
      await fs.ensureDir(path.dirname(this.configPaths.unified));
      
      // Save unified config
      await fs.writeJson(this.configPaths.unified, validatedConfig, { spaces: 2 });
   ...
   */
  public async updateConfig(updates: Partial<UnifiedConfig>): Promise<UnifiedConfig> {
   ...
   */
  public async addService(serviceId: string, config: { provider: string; version?: string; config?: Record<string, any> }): Promise<void> {
    const currentConfig = await this.loadConfig();
   ...
   */
  public async updateDesignConfig(designConfig: Partial<UnifiedConfig['design']>): Promise<void> {
    const currentConfig = await this.loadConfig();
   ...
   */
  public async updateAIConfig(aiConfig: Partial<UnifiedConfig['ai']>): Promise<void> {
    const currentConfig = await this.loadConfig();
  ...
  public async getMonorepoInfo(): Promise<{
    ...
    try {
      ...
      if (packageJson.workspaces) {
        ...
        return {
          isMonorepo: true,
          structure: apps.length > 0 && packages.length > 0 ? 'apps-packages' : 'workspaces',
          apps,
          packages
        };
      }
      ...
      // Check for nx
      if (await fs.pathExists(path.join(path.dirname(this.configPaths.packageJson), 'nx.json'))) {
        return {
          isMonorepo: true,
          structure: 'nx',
  ...
  private async loadUnifiedConfig(): Promise<UnifiedConfig> {
    try {
      ...
      logger.error('Failed to load unified configuration:', error);
      throw error;
    }
  }
  ...
  private async migrateFromXaheen(): Promise<MigrationResult> {
    try {
      ...
      const xaheenConfig: XaheenConfig = await fs.readJson(this.configPaths.xaheen);
      ...
      const unifiedConfig: UnifiedConfig = {
        ...
        design: {
          platform: this.inferPlatformFromFramework(xaheenConfig.project.framework)
        },
        compliance: {
  ...
  private async createDefaultConfig(): Promise<UnifiedConfig> {
    const projectInfo = await this.extractProjectInfoFromPackageJson();
  ...
  private async extractProjectInfoFromPackageJson(): Promise<{
    ...
    try {
      ...
      return {
        ...
        framework: this.detectFrameworkFromDependencies(packageJson.dependencies || {}),
  ...
  private detectFrameworkFromDependencies(dependencies: Record<string, string>): string {
  ...
  private async getDirectoryContents(dirName: string): Promise<string[]> {
    try {
      const dirPath = path.join(path.dirname(this.configPaths.packageJson), dirName);
  ...
  private mergeConfigs(base: UnifiedConfig, updates: Partial<UnifiedConfig>): UnifiedConfig {
    return {
      ...base,
      ...updates,
      ...
      compliance: updates.compliance ? {
        accessibility: updates.compliance.accessibility || base.compliance?.accessibility || 'AAA' as const,
        norwegian: updates.compliance.norwegian ?? base.compliance?.norwegian ?? false,
