...
export interface StackConfig {
  language: 'typescript' | 'javascript' | 'python' | 'csharp' | 'java' | 'php' | 'ruby' | 'go';
  framework: string;
  orm?: string;
  ui?: string;
  testFramework?: string;
  packageManager?: 'npm' | 'yarn' | 'pnpm' | 'bun' | 'pip' | 'composer' | 'gem' | 'cargo' | 'go' | 'nuget' | 'maven' | 'gradle';
  buildTool?: string;
  features?: string[];
}
...
export abstract class BaseStackAdapter implements StackAdapter {
  ...
  abstract generateController(context: GeneratorContext): Promise<GeneratedFile[]>;
  abstract generateService(context: GeneratorContext): Promise<GeneratedFile[]>;
  abstract generateComponent(context: GeneratorContext): Promise<GeneratedFile[]>;
  abstract generatePage(context: GeneratorContext): Promise<GeneratedFile[]>;
  abstract generateMigration(context: GeneratorContext): Promise<GeneratedFile[]>;
  abstract generateValidation(context: GeneratorContext): Promise<GeneratedFile[]>;
  ...
  validateContext(context: GeneratorContext): ValidationResult {
    ...
    if (context.name && !/^[a-zA-Z][a-zA-Z0-9]*$/.test(context.name)) {
      warnings.push('Name should start with a letter and contain only alphanumeric characters');
    }
