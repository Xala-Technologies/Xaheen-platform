...
export default class PageDomain {
  ...
  private initializePageTemplates(): Map<string, PageTemplate> {
    ...
    templates.set('auth-register', {
      ...
      type: 'auth',
      features: ['Registration Form', 'Email Verification', 'Terms Acceptance', 'Social Auth'],
      dependencies: ['@xala-technologies/ui-system', 'react-hook-form']
    });
    ...
    templates.set('profile', {
      ...
      type: 'dynamic',
      features: ['Profile Info', 'Avatar Upload', 'Settings', 'Activity History'],
      dependencies: ['@xala-technologies/ui-system']
    });
    ...
    templates.set('blog-post', {
      ...
      type: 'dynamic',
      features: ['Article Content', 'Author Info', 'Comments', 'Related Posts', 'Share'],
    ...
    templates.set('404', {
      ...
      type: 'static',
      features: ['Error Message', 'Search', 'Navigation Links', 'Contact Support'],
      dependencies: ['@xala-technologies/ui-system']
    });
    ...
    templates.set('settings', {
      ...
      type: 'dynamic',
      features: ['Settings Tabs', 'Profile Settings', 'Notifications', 'Security', 'Billing'],
      dependencies: ['@xala-technologies/ui-system']
    });
  ...
  async create(command: CLICommand): Promise<void> {
    ...
    try {
      ...
      logger.success(`✅ Page created successfully at: ${chalk.cyan(outputPath)}`);
  ...
  async list(command: CLICommand): Promise<void> {
    ...
    templatesByType.forEach((templates, type) => {
      ...
      templates.forEach(template => {
        ...
        logger.info(`      Features: ${chalk.cyan(template.features.join(', '))}`);
        logger.info('');
      });
    });

    logger.info(chalk.gray('\n  Usage:'));
    logger.info(chalk.gray('    xaheen page create <name> --template <template-key>'));
    logger.info(chalk.gray('    xaheen page generate "description of your page"'));
  }
  ...
  private analyzePageType(description: string): string {
    ...
    if (desc.includes('login') || desc.includes('signin') || desc.includes('auth')) {
      return 'auth';
    }
    if (desc.includes('landing') || desc.includes('hero') || desc.includes('marketing')) {
      return 'landing';
    }
    if (desc.includes('blog') || desc.includes('article') || desc.includes('post')) {
      return 'dynamic';
    }
  ...
  private findBestTemplate(description: string, pageType: string): PageTemplate | undefined {
    const desc = description.toLowerCase();
