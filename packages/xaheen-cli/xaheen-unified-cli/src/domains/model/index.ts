...
export default class ModelDomain {
  ...
  private initializeCommonModels(): void {
    ...
    this.modelSchemas.set('user', {
      ...
      fields: [
        ...
        { name: 'posts', type: 'Relation', required: false, relation: { type: 'hasMany', model: 'Post' } }
      ],
      timestamps: true,
      softDelete: true,
    ...
    this.modelSchemas.set('post', {
      ...
      fields: [
        ...
        { name: 'author', type: 'Relation', required: false, relation: { type: 'belongsTo', model: 'User', foreignKey: 'authorId' } },
        { name: 'tags', type: 'Relation', required: false, relation: { type: 'belongsToMany', model: 'Tag' } }
      ],
      timestamps: true,
    ...
    this.modelSchemas.set('product', {
      ...
      fields: [
        ...
        { name: 'stock', type: 'Int', required: true, default: 0, validation: { min: 0 } },
        { name: 'categoryId', type: 'String', required: false },
        { name: 'category', type: 'Relation', required: false, relation: { type: 'belongsTo', model: 'Category' } },
  ...
  async scaffold(command: CLICommand): Promise<void> {
    ...
    logger.info(`🚀 Scaffolding complete CRUD for model: ${chalk.green(modelName)}`);
    ...
    try {
      ...
      logger.success(`✅ Complete CRUD scaffolding for "${schema.name}" created!`);
      this.showGeneratedFiles(schema);
      this.showNextSteps(schema);
    } catch (error) {
  ...
  private async parseModelSchema(modelName: string, options: any): Promise<ModelSchema> {
  ...
  private async generateArtifacts(schema: ModelSchema): Promise<GeneratedArtifacts> {
    return {
    ...
    const fields = schema.fields
      ...
      .map(field => {
        ...
        if (field.default !== undefined) {
          ...
          } else if (field.type === 'Boolean' || field.type === 'Int' || field.type === 'Float') {
            fieldDef += ` @default(${field.default})`;
...
${schema.indexes ? schema.indexes.map(idx => 
...
export class ${modelName}Service {
  async findAll(params: PaginationParams): Promise<PaginatedResult<${modelName}>> {
    ...
    if (search) {
      where.OR = [
        ${schema.fields
          .filter(f => f.type === 'String' && f.name !== 'id' && f.name !== 'password')
  ...
  async delete(id: string): Promise<void> {
    ${schema.softDelete ? `await prisma.${modelNameLower}.update({
      where: { id },
...
export default function ${modelNamePlural}Page() {
  ...
  private generateFrontendForm(schema: ModelSchema): string {
    ...
    const formFields = schema.fields
      .filter(f => f.type !== 'Relation' && f.name !== 'id' && !['createdAt', 'updatedAt', 'deletedAt'].includes(f.name))
      .map(field => {
        ...
        <div className="form-group">
          ...
          >
            ...
            ${field.validation.enum.map(val => 
              `<option value="${val}">${val}</option>`
            ).join('\n            ')}
          </Select>
        </div>`;
        } else {
          const inputType = field.type === 'Int' || field.type === 'Float' ? 'number' : 
...
export function ${modelName}Form({ initialData, onSubmit, onCancel }: ${modelName}FormProps) {
  ...
  private generateFrontendTable(schema: ModelSchema): string {
    const modelName = schema.name;
    const columns = schema.fields
      .filter(f => f.type !== 'Relation' && !['password', 'deletedAt'].includes(f.name))
  ...
  return (
    <Table>
      <thead>
        <tr>
          ${columns.map(col => 
            `<th>${this.toTitleCase(col.name)}</th>`
          ).join('\n          ')}
          <th>Actions</th>
        </tr>
      </thead>
...
export interface Paginated${modelName}Response {
  ...
  private async writeArtifacts(schema: ModelSchema, artifacts: GeneratedArtifacts): Promise<void> {
    ...
    try {
      ...
      logger.warn('  ! Prisma schema file not found, creating models directory');
      await fs.mkdir('./models', { recursive: true });
      await fs.writeFile(`./models/${modelNameLower}.prisma`, artifacts.prismaModel);
    }
    ...
    await fs.mkdir(path.join(frontendDir, 'pages', this.pluralize(modelNameLower)), { recursive: true });
    await fs.mkdir(path.join(frontendDir, 'types'), { recursive: true });

    await fs.writeFile(
      path.join(frontendDir, 'pages', this.pluralize(modelNameLower), 'page.tsx'),
      artifacts.frontendPage
    );

    await fs.writeFile(
      path.join(frontendDir, 'pages', this.pluralize(modelNameLower), `${modelNameLower}-form.tsx`),
      artifacts.frontendForm
    );

    await fs.writeFile(
      path.join(frontendDir, 'pages', this.pluralize(modelNameLower), `${modelNameLower}-table.tsx`),
...
export default function ${modelName}AdminDashboard() {
  ...
  private showGeneratedFiles(schema: ModelSchema): void {
    ...
    logger.info(`     • ${chalk.cyan(`prisma/schema.prisma`)} (model definition)`);
    logger.info(`     • ${chalk.cyan(`src/api/controllers/${modelNameLower}.controller.ts`)}`);
    logger.info(`     • ${chalk.cyan(`src/api/services/${modelNameLower}.service.ts`)}`);
    logger.info(`     • ${chalk.cyan(`src/api/validators/${modelNameLower}.validator.ts`)}`);
    
    logger.info(chalk.gray('\n   Frontend:'));
    logger.info(`     • ${chalk.cyan(`src/pages/${modelNamePlural}/page.tsx`)}`);
    logger.info(`     • ${chalk.cyan(`src/pages/${modelNamePlural}/${modelNameLower}-form.tsx`)}`);
    logger.info(`     • ${chalk.cyan(`src/pages/${modelNamePlural}/${modelNameLower}-table.tsx`)}`);
  ...
  private showNextSteps(schema: ModelSchema): void {
    ...
    logger.info(`      ${chalk.cyan(`import ${schema.name.toLowerCase()}Routes from './routes/${schema.name.toLowerCase()}';`)}`);
  ...
  private showAvailableTemplates(): void {
    ...
    this.modelSchemas.forEach((schema, key) => {
      logger.info(`  • ${chalk.green(key)}: ${schema.fields.length} fields, ${schema.traits?.join(', ') || 'no traits'}`);
    });
    logger.info('\n💡 Usage examples:');
    logger.info('  xaheen model create user --template user');
    logger.info('  xaheen model generate Product --fields "name:String,price:Float,stock:Int"');
